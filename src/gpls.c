...
int adsp_pls_add(uintptr_t type, uintptr_t key, int size, int (*ctor)(void *ctx, void *data),
                 void *ctx, void (*dtor)(void *), void **ppo)
{
	return pls_add(&gpls, type, key, size, ctor, ctx, dtor, ppo);
}

int adsp_pls_add_lookup(uintptr_t type, uintptr_t key, int size, int (*ctor)(void *ctx, void *data),
                        void *ctx, void (*dtor)(void *), void **ppo)
{
	return pls_add_lookup_singleton(&gpls, type, key, size, ctor, ctx, dtor, ppo);
}
