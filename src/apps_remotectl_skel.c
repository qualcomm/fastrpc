...
static const Method methods[2] = { { REMOTE_SCALARS_MAKEX(0, 0, 0x2, 0x2, 0x0, 0x0), 0x8, 0x8, 6, 4,
...
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(apps_remotectl_slim)
    = { 2, &(methodArrays[0]), 0, 0, &(methodStringsArrays[0]), methodStrings, strings };
...
static __inline int _skel_method(int (*_pfn)(int, char *, int, int *), uint32_t _sc,
                                 remote_arg *_pra)
{
...
static __inline int _skel_method_1(int (*_pfn)(const char *, int *, char *, int, int *),
                                   uint32_t _sc, remote_arg *_pra)
{
	...
	_TRY(_nErr, _pfn((const char *)*_in0, (int *)_rout1, (char *)*_rout2, (int)*_rout2Len,
	                 (int *)_rout3));
...
__QAIC_SKEL_EXPORT int __QAIC_SKEL(apps_remotectl_skel_invoke)(uint32_t _sc, remote_arg *_pra)
