...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: remote register fd fails for fd %d, "
			     "size %zu\n",
			     nErr, fd, size);
		}
	}
...
remote_register_buf_common(void *buf, size_t size, int fd, int attr)
{
	...
	if(fd != -1) {
		...
		if(freefd) {
			...
			/**
			 * When buf deregister size mismatch with register
			 * size, deregister buf fails leaving stale fd in
			 * fdlist. Bad fd can be attached to other shared
			 * buffers in next invoke calls.
			 */
			FARF(ERROR,
			     "FATAL: Size mismatch between deregister buf "
			     "(%p) size (%zu) and "
			     "registered buf size (%zu) fd %d, bad fd can be "
			     "attached to other "
			     "shared buffers. Use same buffer size as "
			     "registered buffer",
...
bail:
	if(nErr != AEE_SUCCESS) {
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: remote_register_buf failed buf %p, "
			     "size %zu, fd 0x%x",
			     nErr, buf, size, fd);
		}
	}
...
remote_register_dma_handle_attr(int fd, uint32_t len, uint32_t attr)
{
	...
	for(i = 0; i < dma_handle_count; i++) {
		if(dhandles[i].used && dhandles[i].fd == fd) {
			/* If fd already present in handle list, then just
			 * update attribute only if its zero */
			if(!dhandles[i].attr) {
	...
	if(i == dma_handle_count) {
		if(dma_handle_count >= MAX_DMA_HANDLES) {
			FARF(ERROR,
			     "Error: %s: DMA handle list is already full "
			     "(count %d)",
...
bail:
	if(nErr) {
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: %s failed for fd 0x%x, len %d, attr "
			     "0x%x",
			...
			if(STD_BETWEEN(buf, tofd->buf,
				...
				if(STD_BETWEEN((unsigned long)buf + bufLen - 1,
					...
					*attr = tofd->attr;
					...
					pthread_mutex_unlock(&fdlist.mut);
					FARF(ERROR,
					     "Error 0x%x: Mismatch in buffer "
					     "address(%p) or size(%x) to the "
					     "registered FD(0x%x), "
					     "address(%p) and size(%zu)\n",
...
bail:
	...
	if(nErr) {
		...
		FARF(ERROR,
		     "Error 0x%x: %s failed to map buffer fd %d, addr %p, "
		     "length 0x%zx, "
...
bail:
	...
	if(nErr) {
		...
		FARF(ERROR,
		     "Error 0x%x: %s failed fd %d, vaddr %p, length 0x%zx, "
		     "domain %d, "
...
bail:
	...
	if(nErr) {
		...
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: %s failed to unmap buffer addr "
			     "0x%llx size 0x%zx "
			     "domain %d errno %s",
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		nErr = convert_kernel_to_user_error(nErr, errno);
		FARF(ERROR,
		     "Error 0x%x: %s failed for fd 0x%x of size %lld (flags "
		     "0x%x, vaddrin "
		     "0x%llx) errno %s\n",
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		nErr = convert_kernel_to_user_error(nErr, errno);
		FARF(ERROR,
		     "Error 0x%x: %s failed for size %lld (vaddrout 0x%llx) "
		     "errno %s\n",
...
fastrpc_unmap_fd(void *buf, size_t size, int fd, int attr)
{
	...
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for size %zu fd %d errno %s\n",
...
try_map_buffer(struct mem_to_fd *tofd)
{
	...
	if(errcnt) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for fd %d buf %p size 0x%zx "
		     "errcnt %d",
...
try_unmap_buffer(struct mem_to_fd *tofd)
{
	...
	FOR_EACH_EFFECTIVE_DOMAIN_ID(domain)
	{
		...
		if(!nErr) {
			...
			errcnt++;
			//@TODO: Better way to handle error? probably prevent
			// same FD getting re-used with FastRPC library.
		}
	}
	if(errcnt) {
		FARF(ERROR,
...
fastrpc_buffer_ref(int domain, int fd, int ref, void **va, size_t *size)
{
	...
	if(ref == 1) {
		...
		if(map->refs == 1) {
			FARF(ERROR,
			     "%s: Attempting to remove last reference to "
			     "buffer %d on domain %d",
			     __func__, fd, domain);
			nErr = AEE_EBADPARM;
			goto bail;
		}
