...
static void *remote_register_fd_attr(int fd, size_t size, int attr)
{
	...
	QNode_CtorZ(&tofd->qn);
	VERIFYM((void *)-1 != (buf = mmap(0, size, PROT_NONE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0)),
...
void *remote_register_fd2(int fd, size_t size) { return remote_register_fd_attr(fd, size, 0); }

static int remote_register_buf_common(void *buf, size_t size, int fd, int attr)
{
	int nErr = 0;
	...
	if (fd != -1) {
		...
		if (freefd) {
			if (freefd->attr & FASTRPC_ATTR_KEEP_MAP) {
				fastrpc_unmap_fd(freefd->buf, freefd->size, freefd->fd,
				                 freefd->attr);
			}
			...
			if (STD_BETWEEN(buf, tofd->buf, (unsigned long)tofd->buf + tofd->size)) {
				if (STD_BETWEEN((unsigned long)buf + bufLen - 1, tofd->buf,
					...
					*attr = tofd->attr;
					...
					pthread_mutex_unlock(&fdlist.mut);
					FARF(ERROR,
					     "Error 0x%x: Mismatch in buffer address(%p) or "
					     "size(%x) to the "
					     "registered FD(0x%x), address(%p) and size(%zu)\n",
					     AEE_EBADPARM, buf, bufLen, tofd->fd, tofd->buf,
					     tofd->size);
					return AEE_EBADPARM;
				}
			}
                 ...
                 enum fastrpc_map_flags flags)
{
	...
	FARF(RUNTIME_RPC_HIGH, "%s: domain %d fd %d addr %p length 0x%zx flags 0x%x offset 0x%x",
	...
	VERIFYC(flags >= 0 && flags < FASTRPC_MAP_MAX && flags != FASTRPC_MAP_RESERVED,
	        AEE_EBADPARM);
	...
	map.m.vaddrout = 0;
	mNode->map = map.m;
	iocErr = ioctl_mmap(dev, MEM_MAP, flags, attrs, fd, offset, length, (uint64_t)vaddr,
	                    &vaddrout);
	if (!iocErr) {
		...
		pthread_mutex_unlock(&smaplst[domain].mut);
		mNode = NULL;
	} else if (errno == ENOTTY || iocErr == (int)(DSP_AEE_EOFFSET | AEE_EUNSUPPORTED)) {
...
bail:
	...
	if (nErr) {
		...
		FARF(ERROR,
		     ...
		     nErr, __func__, fd, vaddr, length, domain, flags, iocErr, strerror(errno));
	}
	if (mNode) {
		free(mNode);
		mNode = NULL;
...
int fastrpc_munmap(int domain, int fd, void *vaddr, size_t length)
{
	...
	FARF(RUNTIME_RPC_HIGH, "%s: domain %d fd %d vaddr %p length 0x%zx", __func__, domain, fd,
	     vaddr, length);
	if (domain == -1) {
		domain = get_current_domain();
	}
	...
	QLIST_NEXTSAFE_FOR_ALL(&smaplst[domain].ql, pn, pnn)
	{
		...
		if (mNode->map.fd == fd) {
			FARF(RUNTIME_RPC_HIGH, "%s: unmap found for fd %d domain %d", __func__, fd,
			     domain);
			break;
		}
	}
	...
	if (mNode->refs > 1) {
		FARF(ERROR, "%s: Attempt to unmap FD %d with %d outstanding references", __func__,
		     fd, mNode->refs - 1);
		nErr = AEE_EBADPARM;
		goto bail;
	}
	...
	iocErr = ioctl_munmap(dev, MEM_UNMAP, 0, 0, fd, mNode->map.length, mNode->map.vaddrout);
	pthread_mutex_lock(&smaplst[domain].mut);
	locked = 1;
	if (iocErr == 0) {
...
int remote_mem_map(int domain, int fd, int flags, uint64_t vaddr, size_t size, uint64_t *raddr)
{
	int nErr = 0;
	int dev = -1, ref = 0;
	uint64_t vaddrout = 0;
	...
	FARF(RUNTIME_RPC_HIGH, "%s: domain %d fd %d addr 0x%llx size 0x%zx flags 0x%x", __func__,
	     domain, fd, vaddr, size, flags);

	VERIFYC(fd >= 0, AEE_EBADPARM);
	VERIFYC(size >= 0, AEE_EBADPARM);
	VERIFYC(flags >= 0 && flags < REMOTE_MAP_MAX_FLAG && raddr != NULL, AEE_EBADPARM);
	if (domain == -1) {
		domain = get_current_domain();
	}
	VERIFYC(IS_VALID_EFFECTIVE_DOMAIN_ID(domain), AEE_EBADPARM);
...
int remote_mem_unmap(int domain, uint64_t raddr, size_t size)
{
	...
	FARF(RUNTIME_RPC_HIGH, "%s: domain %d addr 0x%llx size 0x%zx", __func__, domain, raddr,
	     size);
	if (domain == -1) {
		domain = get_current_domain();
	}
...
bail:
	...
	if (nErr != AEE_SUCCESS) {
		nErr = convert_kernel_to_user_error(nErr, errno);
		FARF(ERROR, "Error 0x%x: %s failed for size %lld (vaddrout 0x%llx) errno %s\n",
...
static int fastrpc_unmap_fd(void *buf, size_t size, int fd, int attr)
{
	...
	FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
	{
		...
		if (nErr)
			FARF(RUNTIME_RPC_LOW, "unmap_fd: device found %d for domain %d returned %d",
			     dev, ii, nErr);
		fastrpc_session_put(ii);
	}
	if (nErr != AEE_SUCCESS) {
...
static __inline void try_map_buffer(struct mem_to_fd *tofd)
{
	...
	{
		nErr = fastrpc_mmap(domain, tofd->fd, tofd->buf, 0, tofd->size, FASTRPC_MAP_STATIC);
		if (!nErr) {
	...
	if (errcnt) {
		FARF(ERROR, "Error 0x%x: %s failed for fd %d buf %p size 0x%zx errcnt %d", nErr,
		     __func__, tofd->fd, tofd->buf, tofd->size, errcnt);
	}
}
...
int fastrpc_mem_open(int domain)
{
	...
	{
		tofd = STD_RECOVER_REC(struct mem_to_fd, qn, pn);
		if (tofd->attr & FASTRPC_ATTR_TRY_MAP_STATIC && tofd->mapped[domain] == false) {
...
bail:
	pthread_mutex_unlock(&smaplst[domain].mut);
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed (domain %d, fd %d, ref %d)", nErr, __func__,
		     domain, fd, ref);
	}
	return nErr;
}
