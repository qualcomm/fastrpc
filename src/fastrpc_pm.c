...
bail:
	...
	if (nErr) {
		FARF(ERROR, "Error 0x%x (%d): %s failed for %s, fd %d (errno %s)\n", nErr, ret,
		     __func__, WAKE_LOCK_FILE, wakelock.lock, strerror(errno));
	}
	return nErr;
}
...
bail:
	if (nErr) {
		wakelock.count++;
		FARF(ERROR, "Error 0x%x (%d): %s failed for %s, fd %d (errno %s)\n", nErr, ret,
		     __func__, WAKE_UNLOCK_FILE, wakelock.unlock, strerror(errno));
	}
	pthread_mutex_unlock(&wakelock.wmut);
	return nErr;
}
...
int fastrpc_wake_lock_init()
{
	...
	VERIFYC(0 < (wakelock.lock = open(WAKE_LOCK_FILE, O_RDWR | O_CLOEXEC)), AEE_ERPC);
	VERIFYC(0 < (wakelock.unlock = open(WAKE_UNLOCK_FILE, O_RDWR | O_CLOEXEC)), AEE_ERPC);
...
bail:
	if (nErr) {
		...
		if (wakelock.lock > 0) {
			...
			if (ret) {
				FARF(ERROR,
				     "Error %d: %s: failed to close %s with fd %d (errno %s)", ret,
				     __func__, WAKE_LOCK_FILE, wakelock.lock, strerror(errno));
			} else {
				wakelock.lock = 0;
			}
...
int fastrpc_wake_lock_deinit()
{
	...
	if (wakelock.lock > 0) {
		...
		if (nErr) {
			FARF(ERROR, "Error %d: %s: failed to close %s with fd %d (errno %s)", nErr,
			     __func__, WAKE_LOCK_FILE, wakelock.lock, strerror(errno));
		} else {
			wakelock.lock = 0;
		}
