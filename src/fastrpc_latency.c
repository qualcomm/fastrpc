		...
		if (invoke) {
			...
			if (qp->vote == FASTRPC_LATENCY_VOTE_OFF) {
				...
				if (nErr == AEE_SUCCESS) {
					...
					FARF(ERROR,
					     "Error %d: %s: PM QoS ON request failed with errno %d "
					     "(%s)",
			...
			if (qp->vote == FASTRPC_LATENCY_VOTE_ON) {
				...
				if (nErr == AEE_SUCCESS) {
					...
					FARF(ERROR,
					     "Error %d: %s: PM QoS OFF request failed with errno "
					     "%d (%s)",
					     nErr, __func__, errno, strerror(errno));
				}
			}
...
int fastrpc_set_pm_qos(struct fastrpc_latency *qos, uint32_t enable, uint32_t latency)
{
	...
	if (enable && state == FASTRPC_LATENCY_STOP) {
		...
		VERIFY(AEE_SUCCESS
		       == (nErr = pthread_create(&qos->thread, 0, fastrpc_latency_thread_handler,
		                                 (void *)qos)));
	}
bail:
	return nErr;
