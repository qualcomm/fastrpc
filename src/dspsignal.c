...
static AEEResult init_domain_signals(int domain)
{
	...
	if ((pthread_once(&signals_once, init_process_signals_once) != 0) || (signals == NULL)) {
...
AEEResult dspsignal_create(int domain, uint32_t id, uint32_t flags)
{
	...
	VERIFYC((ds = (struct dspsignal_domain_signals *)signals->domain_signals[domain]) != NULL,
	        AEE_EBADSTATE);
	errno = 0;
...
bail:
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed (domain %d, ID %u, flags 0x%x) errno %s", nErr,
		     __func__, domain, id, flags, strerror(errno));
	}
	return nErr;
}
...
bail:
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed (domain %d, ID %u) errno %s", nErr, __func__,
		     domain, id, strerror(errno));
	}
	return nErr;
}
...
bail:
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed (domain %d, ID %u, timeout %u) errno %s", nErr,
		     __func__, domain, id, timeout_usec, strerror(errno));
	}
	return nErr;
}
