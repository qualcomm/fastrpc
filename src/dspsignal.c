...
dspsignal_create(int domain, uint32_t id, uint32_t flags)
{
	...
	if(nErr) {
		if(errno == ENOTTY) {
			FARF(HIGH, "dspsignal support not present in the "
			           "FastRPC driver");
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed (domain %d, ID %u, flags 0x%x) "
		     "errno %s",
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed (domain %d, ID %u) errno %s", nErr,
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed (domain %d, ID %u, timeout %u) "
		     "errno %s",
