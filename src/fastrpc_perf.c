...
#define IS_KEY_ENABLED(name)                                                                       \
	(!strncmp((name), "perf_invoke_count", 17) || !strncmp((name), "perf_mod_invoke", 15)      \
	 || !strncmp((name), "perf_rsp", 8) || !strncmp((name), "perf_hdr_sync_flush", 19)         \
	 || !strncmp((name), "perf_sync_flush", 15) || !strncmp((name), "perf_hdr_sync_inv", 17)   \
	 || !strncmp((name), "perf_clean_cache", 16) || !strncmp((name), "perf_sync_inv", 13))
...
static int perf_kernel_getkeys(int dev)
{
	...
	FARF(RUNTIME_RPC_HIGH, "adsprpc:apps:keys: numkeys %d keys: %s", numkeys, p->kernel.keys);
...
static void get_perf_kernel(int dev, remote_handle handle, uint32_t sc)
{
	...
	case PERF_KERNEL_NUM_KEYS:
		FARF(ALWAYS,
		     ...
		     p->kernel.data[3], p->kernel.data[4], p->kernel.data[5], p->kernel.data[6],
		     p->kernel.data[7], p->kernel.data[8]);
		break;
	default:
...
bail:
	if (nErr)
		VERIFY_WPRINTF("Warning: %s: Failed to get perf data from kernel, nErr 0x%x\n",
		               __func__, nErr);
	return;
}
...
static void get_perf_adsp(remote_handle handle, uint32_t sc)
{
	...
	if (gperf.adsp_perf_handle != INVALID_HANDLE) {
		VERIFY(0
		       == (nErr = adsp_perf1_get_usecs(gperf.adsp_perf_handle, pdsp->data,
		                                       PERF_MAX_NUM_KEYS)));
	} else {
...
static int perf_dsp_enable(int domain)
{
	...
	VERIFY(0 == (nErr = adsp_perf_get_keys(keys, PERF_KEY_STR_MAX, &maxLen, &numKeys)));
	if ((gperf.adsp_perf_handle = get_adsp_perf1_handle(domain)) != INVALID_HANDLE) {
		nErr = adsp_perf1_get_keys(gperf.adsp_perf_handle, keys, PERF_KEY_STR_MAX, &maxLen,
		                           &numKeys);
		if (nErr) {
			FARF(ALWAYS,
	...
	FARF(RUNTIME_RPC_HIGH, "keys enable done maxLen %d numKeys %d", maxLen, numKeys);
bail:
	if (nErr) {
...
int fastrpc_perf_init(int dev, int domain)
{
	...
	pd->enable
	    = fastrpc_get_property_int(FASTRPC_PERF_ADSP, 0) || fastrpc_config_is_perfdsp_enabled();
	...
	p->process_trace_enabled = fastrpc_get_property_int(FASTRPC_ENABLE_SYSTRACE, 0);
	if (p->perf_on) {
	...
	if (pk->enable) {
		...
		if (!perf_v2_kernel) {
			VERIFYC(NULL != (pk->keys = (char *)calloc(sizeof(char), PERF_KEY_STR_MAX)),
			        AEE_ENOMEMORY);
