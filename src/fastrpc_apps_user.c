...
fastrpc_get_property_int(fastrpc_properties UserPropKey, int defValue)
{
	if(((int)UserPropKey > NO_ENV_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR,
		     "%s: Index %d out-of-bound for ENV_DEBUG_VAR_NAME array "
		     "of len %d",
	...
	if(((int)UserPropKey > NO_ANDROIDP_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR,
		     "%s: Index %d out-of-bound for ANDROIDP_DEBUG_VAR_NAME "
		     "array of len %d",
		     __func__, UserPropKey,
	...
	if(((int)UserPropKey > NO_ANDROID_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR,
		     "%s: Index %d out-of-bound for ANDROID_DEBUG_VAR_NAME "
		     "array of len %d",
		     __func__, UserPropKey,
...
trace_marker_init(int domain)
{
	...
	if(IS_QTF_TRACING_ENABLED(hlist[domain].procattrs)) {
		...
		if(hlist[domain].trace_marker_fd < 0) {
			FARF(ERROR,
			     "Error: %s: failed to open '%s' for domain %d, "
			     "errno %d (%s)",
...
fastrpc_set_remote_uthread_params(int domain)
{
	...
	if((handle = get_remotectl1_handle(domain)) != INVALID_HANDLE) {
		...
		if(nErr) {
			FARF(ALWAYS,
			     "Warning 0x%x: %s: remotectl1 domains not "
			...
			// Set remotectlhandle to INVALID_HANDLE, so that all
			// subsequent calls are non-domain calls
...
bail:
	if(nErr != AEE_SUCCESS) {
		if(th_params) {
			FARF(ERROR,
			     "Error 0x%x: %s failed domain %d thread priority "
			     "%d stack size %d "
			     "(errno %s)",
			     nErr, __func__, domain,
		...
		FARF(ALWAYS,
		     "Successfully set remote user thread priority to %d and "
		     "stack size to "
		     "%d for domain %d",
		     th_params->thread_priority, th_params->stack_size,
		     domain);
	...
	if(domain_name) {
		if(!strncmp(domain_name, SUBSYSTEM_NAME[ADSP_DOMAIN_ID],
			...
			domain = CDSP_DOMAIN_ID;
		                   ...
		                   SUBSYSTEM_NAME[GDSP0_DOMAIN_ID],
		                   strlen(SUBSYSTEM_NAME[GDSP0_DOMAIN_ID]))) {
			domain = GDSP0_DOMAIN_ID;
		} else if(!strncmp(domain_name,
		                   SUBSYSTEM_NAME[GDSP1_DOMAIN_ID],
...
bail:
	FARF(RUNTIME_RPC_ERROR, "Warning 0x%x: %s failed for uri %s", nErr,
	     __func__, uri);
	return NULL;
}
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for local handle 0x%x, remote "
		     "handle 0x%x, "
		     "domain %d\n",
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for request ID %u, handle 0x%x, "
		     "domain %d\n",
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for request ID %u, domain %d\n",
...
wakelock_control_kernel_pm(int domain, int dev, uint32_t timeout)
{
	...
	if(nErr) {
		if(errno == EBADRQC || errno == ENOTTY) {
			VERIFY_WPRINTF("Warning: %s: kernel does not support "
			               "PM management (%s)",
			               __func__, strerror(errno));
		} else if(errno == EACCES || errno == EPERM) {
			VERIFY_WPRINTF("Warning: %s: application does not "
			               "have permission for PM "
			               "management (%s)",
			               __func__, strerror(errno));
		}
		FARF(ERROR,
		     "Error 0x%x: %s PM control failed for domain %d, dev %d "
		     "with timeout "
		     "%d (errno %s)",
...
fastrpc_timer_callback(void *ptr)
{
	...
	if(nErr) {
		FARF(ERROR,
		     "%s: Failed to create exception in the remote process on "
		     "domain %d "
		     "(errno %s)",
	...
	if(fastrpc_wake_lock_enable[domain]) {
		if(!IS_REVERSE_RPC_CALL(handle, sc)
		   || is_first_reverse_rpc_call(domain, handle, sc)) {
			...
			/* Since wake-lock is not released at the end of
			 * previous "remote_handle_invoke" for subsequent
			 * reverse RPC calls, it doesn't have to be taken again
			 * here. It will be released before "ioctl invoke" call
			 * to kernel */
			wake_lock = 1;
	}
	...
	for(i = 0; i < bufs; i++) {
		...
		if(pra[i].buf.nLen) {
			...
			if(attr & FASTRPC_ATTR_COHERENT) {
				...
				VERIFY_IPRINTF("coherent buffer idx: %d addr: "
				               "%p size: %d",
				               i, pra[i].buf.pv,
				               pra[i].buf.nLen);
			}
			if(attr & FASTRPC_ATTR_FORCE_NOFLUSH) {
				...
				VERIFY_IPRINTF("force no flush for buffer "
				               "idx: %d addr: %p size: %d",
				               i, pra[i].buf.pv,
				               pra[i].buf.nLen);
			}
			if(attr & FASTRPC_ATTR_FORCE_NOINVALIDATE) {
				...
				VERIFY_IPRINTF("force no invalidate buffer "
				               "idx: %d addr: %p size: %d",
				               i, pra[i].buf.pv,
				               pra[i].buf.nLen);
			}
			if(attr & FASTRPC_ATTR_KEEP_MAP) {
				...
				VERIFY_IPRINTF(
				    "invoke: mapping with attribute KEEP_MAP");
			}
		}
	}
	...
	for(i = bufs; i < total; i++) {
		...
		if(hlist[domain].dma_handle_reverse_rpc_map_capability
		   && (attr & FASTRPC_ATTR_NOMAP)) {
			// Register fd again, for reverse RPC call to retrive
			// FASTRPC_ATTR_NOMAP flag for fd
	   ...
	   && (!IS_STATIC_HANDLE(handle)) && !asyncjob.isasyncjob) {
		...
		for(i = 0; (i < bufs) && (i < M_CRCLIST); i++) {
			if(crc_local[i] != crc_remote[i]) {
				FARF(ERROR,
				     "CRC mismatch for buffer %d[%d], crc "
				     "local %x remote %x",
...
bail:
	if(asyncjob.isasyncjob) {
		if(!nErr) {
			FARF(RUNTIME_RPC_HIGH,
			     "adsprpc : %s Async job Queued, job 0x%" PRIx64
			     "",
	...
	if(wake_lock) {
		// Keep holding wake-lock for reverse RPC calls to keep CPU
		// awake for any further processing
		if(!IS_REVERSE_RPC_CALL(handle, sc)) {
			fastrpc_wake_unlock();
			wake_lock = 0;
		}
	...
	if(nErr != AEE_SUCCESS) {
		...
		// FARF(ERROR, "Error 0x%x: %s failed for handle 0x%x on domain
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		...
		/*
		 * handle_info or so name cannot be obtained from remote
		 * handles which are used for non-domain calls.
		 */
		if(0 == check_rpc_error(nErr)) {
			if(get_logger_state(domain)) {
				FARF(ERROR,
				     "Error 0x%x: %s failed for handle 0x%x, "
				     "method %d on domain %d "
				     "(sc 0x%x) (errno %s)\n",
...
remote_handle64_invoke(remote_handle64 local, uint32_t sc, remote_arg *pra)
{
	...
	if(IS_STATICPD_HANDLE(local)) {
		nErr = AEE_EINVHANDLE;
		FARF(ERROR,
		     "Error 0x%x: %s cannot be called for staticPD handle "
		     "0x%" PRIx64 "\n",
		     nErr, __func__, local);
		goto bail;
	}
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		...
		if(0 == check_rpc_error(nErr)) {
			if(get_logger_state(domain)) {
				FARF(ERROR,
				     "Error 0x%x: %s failed for module %s, "
				     "handle 0x%" PRIx64
				     ", method %d on domain %d (sc 0x%x) "
				     "(errno %s)\n",
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: %s failed for handle 0x%x, method "
			     "%d async type %d on "
                          ...
                          uint64_t *t_spawn, uint64_t *t_load)
{
	...
	if(total_dsp_lib_refcnt > MAX_LIB_INSTANCE_ALLOWED) {
		FARF(ERROR,
		     "Error: aborting due to %d instances of libxdsprpc. Only "
		     "%d allowed\n",
	...
	if(!strncmp(name, ITRANSPORT_PREFIX "createstaticpd",
		 ...
		 * "createstaticpd:oispd&_dom=adsp&_session=1" to create a
		 * session on both static PDs.
		 */
	...
	PROFILE_ALWAYS(
	    ...
	    if((handle = get_remotectl1_handle(domain)) != INVALID_HANDLE) {
		    ...
		    if(nErr) {
			    ...
			    // Set remotectlhandle to INVALID_HANDLE, so that
			    // all subsequent calls are non-domain calls
			    hlist[domain].remotectlhandle = INVALID_HANDLE;
			    VERIFY(AEE_SUCCESS
			           == (nErr = remotectl_open(
...
bail:
	if(dlerr != 0) {
		FARF(ERROR,
		     "Error 0x%x: %s: dynamic loading failed for %s on domain "
		     "%d (dlerror "
		     "%s) (errno %s)\n",
...
bail:
	if(nErr) {
		...
		FARF(ALWAYS,
		     "%s: Successfully opened handle 0x%x for %s on domain %d "
		     "(spawn time "
...
bail:
	if(nErr) {
		...
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: %s failed for %s (errno %s)\n", nErr,
...
bail:
	if(nErr != AEE_SUCCESS) {
		if(0 == check_rpc_error(nErr)) {
			FARF(ERROR,
			     "Error 0x%x: %s failed for handle 0x%x, domain "
			     "%d (dlerr %s) (errno "
			     "%s)\n",
		...
		FARF(RUNTIME_RPC_HIGH,
		     "%s: closed module with handle 0x%x (skel unload time "
		     "%" PRIu64 " us)",
		     __func__, h, t_close);
	}
	if(dlerrstr) {
...
bail:
	if(nErr != AEE_EINVHANDLE && IS_VALID_EFFECTIVE_DOMAIN_ID(domain)) {
		...
		if(nErr != AEE_SUCCESS) {
			...
			if(0 == check_rpc_error(nErr))
				FARF(ERROR,
				     "Error 0x%x: %s close module %s failed "
				     "for handle 0x%" PRIx64
...
manage_adaptive_qos(int domain, uint32_t enable)
{
	...
	if(hlist[domain].dev != -1) {
		/* If session is already open on DSP, then make rpc call
		 * directly to user PD
		 */
		if((handle = get_remotectl1_handle(domain))
		   != INVALID_HANDLE) {
			...
			if(nErr) {
				FARF(ALWAYS,
				     "Warning 0x%x: %s: remotectl1 domains "
				...
				// Set remotectlhandle to INVALID_HANDLE, so
				// that all subsequent calls are non-domain
				// calls
				hlist[domain].remotectlhandle = INVALID_HANDLE;
		...
		if(nErr) {
			FARF(ERROR,
			     "Error: %s: remotectl_set_param failed to reset "
			     "adaptive QoS on DSP "
			     "to %d on domain %d",
		...
		/* If session is not created already, then just set process
		 * attribute */
		hlist[domain].qos.adaptive_qos
		    = ((enable == RPC_ADAPTIVE_QOS) ? 1 : 0);
	}
...
bail:
	if(nErr) {
		FARF(ERROR,
		     "Error 0x%x (errno %d): %s failed for domain %d, handle "
		     "0x%" PRIx64 ", enable %u, timeout %u (%s)\n",
...
bail:
	...
	if(nErr) {
		FARF(ERROR,
		     "Error 0x%x: %s failed to get async response data for "
		     "domain %d errno "
		     "%s",
...
update_kernel_wakelock_status(int domain, int dev, uint32_t wl_enable)
{
	...
	if(nErr) {
		...
		FARF(ERROR,
		     "Error 0x%x: %s failed for domain %d, dev %d, wakelock "
		     "control %d "
		     "errno %s",
...
bail:
	if(nErr)
		...
		if(wl_enable)
			FARF(ALWAYS,
			     "%s: enabled wakelock control for domain %d",
			     __func__, domain);
		else
			FARF(ALWAYS,
			     "%s: disable wakelock control for domain %d",
			     __func__, domain);
	}
	return nErr;
}
                             ...
                             void *data, uint32_t len)
{
	...
	case DSPRPC_CONTROL_LATENCY: {
		...
		case RPC_DISABLE_QOS: {
			...
			VERIFY(AEE_SUCCESS
			       == (nErr = manage_poll_qos(domain, h,
			                                  RPC_DISABLE_QOS,
			                                  lp->latency)));
			/* Error ignored, currently meeting qos requirement is
			 * optional. Consider to error out in later targets */
		...
		case RPC_ADAPTIVE_QOS: {
			/* Disable PM QoS if enabled and then enable adaptive
			 * QoS */
			VERIFY(AEE_SUCCESS
			       == (nErr
			           = manage_pm_qos(domain, h, RPC_DISABLE_QOS,
		...
		case RPC_POLL_QOS: {
			...
			/*
			 * Poll QoS option also enables PM QoS to enable early
			 * response from DSP and stop the CPU cores from going
			 * into deep sleep low power modes.
			 */
			VERIFY(AEE_SUCCESS
			       == (nErr
		...
		default:
			nErr = AEE_EBADPARM;
			FARF(ERROR,
			     "Error: %s: Bad enable parameter %d passed for "
			     "QoS control",
			     __func__, lp->enable);
			goto bail;
		}
		FARF(ALWAYS,
		     "%s: requested QOS %d, latency %u for domain %d handle "
		     "0x%" PRIx64 "\n",
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for request ID %d (errno %s)",
...
set_unsigned_pd_attribute(int domain, int enable)
{
	...
	if(hlist[domain].dev != -1) {
		if(hlist[domain].unsigned_module == enable) {
			FARF(HIGH,
			     "%s: %s session already open on domain %d , "
			     "enable %d ",
			     __func__,
			...
			FARF(ERROR,
			     "Error 0x%x: %s: %s session already open on "
			     "domain %d , enable %d ",
			     nErr, __func__,
			     hlist[domain].unsigned_module ? "Unsigned"
			                                   : "Signed",
...
store_domain_thread_params(int domain, int thread_priority, int stack_size)
{
	...
	if(thread_priority != -1) {
		if((thread_priority < MIN_THREAD_PRIORITY)
		   || (thread_priority > MAX_THREAD_PRIORITY)) {
			nErr = AEE_EBADPARM;
			FARF(ERROR,
			     "%s: Thread priority %d is invalid! Should be "
			     "between %d and %d",
	...
	if(stack_size != -1) {
		if((stack_size < MIN_UTHREAD_STACK_SIZE)
		   || (stack_size > MAX_UTHREAD_STACK_SIZE)) {
			nErr = AEE_EBADPARM;
			FARF(ERROR,
			     "%s: Stack size %d is invalid! Should be between "
			     "%d and %d",
	...
	if(hlist[domain].dev != -1) {
		...
		FARF(ALWAYS,
		     "Dynamically set remote user thread priority to %d and "
		     "stack size to "
		     "%d for domain %d",
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for domain %d for thread priority "
		     "%d, stack "
		     "size %d",
...
set_pd_dump_attribute(int domain, int enable)
{
	...
	if(hlist[domain].dev != -1) {
		nErr = AEE_ERPC;
		FARF(ERROR,
		     "%s: Session already open on domain %d ! Request "
		     "unsigned offload "
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for domain %d for pd init mem "
		     "size :0x%x",
...
remote_session_control(uint32_t req, void *data, uint32_t datalen)
{
	...
	case FASTRPC_THREAD_PARAMS: {
		...
		if(!params) {
			nErr = AEE_EBADPARM;
			FARF(RUNTIME_RPC_LOW,
			     "%s: Thread params struct passed is %p", __func__,
			     params);
			goto bail;
		}
		...
		FARF(ALWAYS,
		     "%s DSP info request for domain %d, thread priority %d, "
		     "stack size %d",
	...
	case DSPRPC_CONTROL_UNSIGNED_MODULE: {
		...
		FARF(ALWAYS, "%s Unsigned PD enable %d request for domain %d",
	...
	case FASTRPC_RELATIVE_THREAD_PRIORITY: {
		...
		FARF(ALWAYS,
		     "%s DSP thread priority request for domain %d, priority "
		     "%d",
	...
	case FASTRPC_REMOTE_PROCESS_KILL: {
		...
		FARF(ALWAYS, "%s Remote process kill request for domain %d",
		     __func__, domain);
		break;
	}
	case FASTRPC_SESSION_CLOSE: {
		...
		if(!sclose) {
			nErr = AEE_EBADPARM;
			FARF(RUNTIME_RPC_LOW,
			     "Error: %s: session close data pointer passed is "
			     "NULL\n",
			     __func__);
			goto bail;
		}
		...
		FARF(ALWAYS,
		     "%s Fastrpc session close request for domain %d\n",
	...
	case FASTRPC_CONTROL_PD_DUMP: {
		...
		FARF(ALWAYS, "%s PD dump request to enable(%d) for domain %d",
	...
	case FASTRPC_REMOTE_PROCESS_EXCEPTION: {
		 ...
		 * AEE_EBADSTATE for the request thread.
		 */
		...
		FARF(ALWAYS,
		     "%s Remote process exception request for domain %d, "
		     "handle 0x%" PRIx64 "\n",
	...
	case FASTRPC_REGISTER_STATUS_NOTIFICATIONS: {
		...
		FARF(RUNTIME_RPC_HIGH,
		     "%s Register PD status notification request for domain "
		     "%d\n",
		     __func__, domain);
		break;
	}
	case FASTRPC_PD_INITMEM_SIZE: {
		...
		if(params->domain != -1) {
			if(!IS_VALID_EFFECTIVE_DOMAIN_ID(params->domain)) {
				nErr = AEE_EBADPARM;
				FARF(ERROR, "%s: Invalid domain ID %d passed",
				     __func__, params->domain);
				goto bail;
			}
			if(hlist[params->domain].unsigned_module) {
				nErr = AEE_EUNSUPPORTED;
				FARF(ERROR, "Configuring User PD init mem "
				            "length is not supported for "
				            "unsigned PDs");
				goto bail;
			}
			VERIFY(
			...
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
				if(hlist[ii].unsigned_module) {
					FARF(
					    ALWAYS,
					    "Warning: %s: Configuring User PD "
					    "init mem length for domain %d "
					    "is not supported for unsigned "
					    "PDs",
					    __func__, ii);
				} else {
					VERIFY(
		...
		FARF(ALWAYS,
		     "%s DSP userpd memlen request for domain %d, userpd "
		     "memlen 0x%x",
	...
	case FASTRPC_RESERVE_NEW_SESSION: {
		...
		// Initialize effective domain ID to 2nd session of domain,
		// first session is default usage and cannot be reserved
	...
	case FASTRPC_GET_URI: {
		...
		if(rpc_uri->uri_len <= ret_val) {
			nErr = AEE_EBADSIZE;
			FARF(ERROR,
			     "ERROR 0x%x: %s Session URI length %u is not "
			     "enough, need %u "
			     "characters",
		...
		if(ret_val < 0) {
			nErr = AEE_EBADSIZE;
			FARF(ERROR,
			     "ERROR 0x%x: %s Invalid Session URI length %u",
...
domain_deinit(int domain)
{
	...
	FARF(RUNTIME_RPC_HIGH, "%s for domain %d: dev %d", __func__, domain,
	     olddev);
	if(olddev != -1) {
		...
		FARF(RUNTIME_RPC_HIGH,
		     "%s: closed device %d on domain %d (kill time %" PRIu64
		     " us)",
...
open_device_node(int domain_id)
{
	...
	case MDSP_DOMAIN_ID:
		...
		if((dev < 0) && (errno == ENOENT)) {
			FARF(RUNTIME_RPC_HIGH,
			     "Device node %s open failed for domain %d (errno "
			     "%s),"
			...
			if((dev < 0) && (errno == ENOENT)) {
				FARF(RUNTIME_RPC_HIGH,
				     "Device node %s open failed for domain "
				     "%d (errno %s),"
				     "falling back to node %s \n",
		...
		if((dev < 0) && ((errno == ENOENT) || (errno == EACCES))) {
			...
			if((dev < 0)
			   && ((errno == ENOENT) || (errno == EACCES))) {
				// Open the default device node if actual
				// device node is not present
				FARF(RUNTIME_RPC_HIGH,
				     "Device node %s open failed for domain "
				     "%d (errno %s),"
				     "falling back to node %s \n",
...
open_shell(int domain_id, apps_std_FILE *fh, int unsigned_shell)
{
	...
	if(!nErr)
		FARF(RUNTIME_RPC_HIGH, "Successfully opened %s, domain %d",
		     absName, domain);
bail:
	if(domain_str) {
		free(domain_str);
	...
	if(nErr != AEE_SUCCESS) {
		if(domain == SDSP_DOMAIN_ID && fh != NULL) {
			...
			*fh = -1;
			...
			FARF(ERROR,
			     "Error 0x%x: %s failed for domain %d search "
			     "paths used are %s, %s, "
			     "%s (errno %s)\n",
...
fastrpc_enable_kernel_optimizations(int domain)
{
	...
	if(((dom != CDSP_DOMAIN_ID) && (dom != CDSP1_DOMAIN_ID)
	    && (dom != GDSP0_DOMAIN_ID) && (dom != GDSP1_DOMAIN_ID))
	   || (hlist[domain].dsppd != USERPD))
		goto bail;
	errno = 0;
	...
	if((nErr == -1 || nErr == (DSP_AEE_EOFFSET + AEE_ERPC))
	   && (errno == ENOTTY || errno == EINVAL || errno == EBADRQC)) {
		 ...
		 * TODO: kernel cleanup to return ENOTTY for all unsupported
		 * IOCTLs
		 */
		nErr = 0;
	}
bail:
	if(nErr) {
		FARF(RUNTIME_RPC_ERROR,
		     "Error 0x%x: %s failed for domain %d (%s)\n", nErr,
...
print_process_attrs(int domain)
{
	...
	if(fastrpc_config_is_logpacket_enabled())
		logpkt = true;
	FARF(ALWAYS,
	     "Info: Created user PD on domain %d, dbg_trace 0x%x, enabled "
	     "attr=> RPC "
	     ...
	     (dbgMode ? "Y" : "N"), (crc ? "Y" : "N"),
	     (unsignedMd ? "Y" : "N"), (signedMd ? "Y" : "N"),
	     (qos ? "Y" : "N"), (configPDdump ? "Y" : "N"),
...
remote_init(int domain)
{
	...
	if(hlist[domain].dev == -1) {
		...
		if(nErr == AEE_SUCCESS) {
			...
			// Skip setting session related info in multiple ioctl
			// calls, if FASTRPC_CONTROL_SESS_INFO is supported
			goto set_sess_info_supported;
		} else {
			// Fallback to previous approach, if
			// FASTRPC_CONTROL_SESS_INFO is not supported
			FARF(RUNTIME_RPC_HIGH,
			     "%s: FASTRPC_CONTROL_SESS_INFO not supported "
			     "with error %d",
			     __func__, nErr);
		}
		...
		if(pd_type == SENSORS_STATICPD || pd_type == GUEST_OS_SHARED) {
			...
			if(ioctl_control(dev, DSPRPC_SMMU_SUPPORT, &smmu)) {
				FARF(RUNTIME_RPC_HIGH,
				     "%s: DSPRPC_SMMU_SUPPORT not supported",
				     __func__);
			}
		}
	...
	set_sess_info_supported:
		...
		if(nErr == AEE_SUCCESS) {
			...
			FARF(ERROR,
			     "Error %d: %s: app does not have access to "
			     "fastrpc device of domain "
			     "%d (%s)",
			     nErr, __func__, domain, strerror(errno));
			goto bail;
			...
			FARF(ERROR,
			     "Error %d: %s: fastRPC device driver is disabled "
			     "(%s)",
			     nErr, __func__, strerror(errno));
			goto bail;
		} else if(nErr) {
			FARF(ERROR,
			     "Error 0x%x: %s: failed to setup fastrpc session "
			     "in kernel",
			     nErr, __func__);
			goto bail;
		}
		...
		if(pd_type == ROOT_PD || pd_type == GUEST_OS_SHARED
		   || pd_type == SECURE_STATICPD) {
			FARF(RUNTIME_RPC_HIGH,
			     "%s: attaching to guest OS/Secure PD (attach %d) "
			     "for domain %d",
			...
			// 3MB of remote heap for dynamic loading is available
			// only for Audio PD.
			...
			if(!(FASTRPC_MODE_UNSIGNED_MODULE
				...
				if(hlist[domain].pd_initmem_size
				   != DEFAULT_PD_INITMEM_SIZE)
					FARF(ERROR,
					     "Setting user PD initial memory "
					     "length is not supported "
					     "for unsigned PD, using default "
					     "size\n");
			}
			errno = 0;
			...
			if(ioErr) {
				...
				if(errno == ECONNREFUSED) {
					nErr = AEE_ECONNREFUSED;
					FARF(ERROR,
					     "Error 0x%x: %s: untrusted app "
					     "trying to offload to signed "
					     "remote process (errno %d, %s). "
					     "Try offloading to unsignedPD "
			...
			FARF(ERROR, "Error: %s called for unknown mode %d",
			     __func__, pd_type);
		}
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		...
		FARF(ERROR,
		     "Error 0x%x: %s failed for domain %d, errno %s, ioErr "
		     "%d\n",
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: adsp current process handle failed. domain "
		     "%d (errno "
		     "%s)\n",
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: get adsp msgd handle failed. domain %d "
		     "(errno %s)\n",
		     nErr, domain, strerror(errno));
	}
	return INVALID_HANDLE;
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: remotectl1 handle failed. domain %d (errno "
		     "%s)\n",
		     nErr, domain, strerror(errno));
	}
	return INVALID_HANDLE;
...
bail:
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: adsp_perf1 handle failed. domain %d (errno "
		     "%s)\n",
		     nErr, domain, strerror(errno));
	}
	return INVALID_HANDLE;
...
domain_init(int domain, int *dev)
{
	...
	if(dom == CDSP_DOMAIN_ID || dom == CDSP1_DOMAIN_ID
	   || dom == GDSP0_DOMAIN_ID || dom == GDSP1_DOMAIN_ID) {
		...
		if(panic_handle != INVALID_HANDLE) {
			int ret = -1;
			/* If error codes are available in debug config, send
			 * panic error codes to dsp to crash. */
			...
			if(err_codes_to_send) {
				...
				if(AEE_SUCCESS == ret) {
					FARF(ALWAYS,
					     "%s : panic error codes sent "
					     "successfully\n",
					     __func__);
				} else {
					FARF(ERROR,
					     "Error 0x%x: %s : panic error "
					     "codes send failed\n",
					     ret, __func__);
				}
			}
			...
			FARF(ALWAYS,
			     "%s : current process handle is not valid\n",
			     __func__);
		}
	}
	VERIFY(AEE_SUCCESS
	...
	if(fastrpc_notif_flag) {
		...
		if(ret == (int)(DSP_AEE_EOFFSET + AEE_EUNSUPPORTED)) {
			VERIFY_WPRINTF(
			    "Warning: %s: DSP does not support notifications",
			    __func__);
		}
		VERIFYC(
		    ret == AEE_SUCCESS
	...
	if((dom != SDSP_DOMAIN_ID) && hlist[domain].dsppd == ROOT_PD) {
		...
		if(handle != INVALID_HANDLE) {
			adspmsgd_init(handle,
			              0x10); // enable PD exception logging
		}
	}
#endif
...
bail:
	if(nErr != AEE_SUCCESS) {
		...
		if(hlist) {
			FARF(ERROR,
			     "Error 0x%x: %s (%d) failed for domain %d (errno "
			     "%s)\n",
	...
	if(hlist) {
		...
		*dev = -1;
		FARF(ERROR,
		     "Error 0x%x: Unable to get dev as hlist is NULL for "
		     "domain %d\n",
		     nErr, __func__, domain);
		return nErr;
	}
...
bail:
	...
	*/
	if(nErr) {
		...
		FARF(ALWAYS,
		     "%s done. default domain:%x and &fastrpc_trace:%p\n",
...
check_multilib_util(void)
{
	...
	if(ret != 0 && errno == ENOMEM) {
		nErr = ERRNO;
		FARF(ERROR,
		     "Error 0x%x: setenv failed for %s (domain %u), errno is "
		     "%s\n",
		     nErr, env_name, DEFAULT_DOMAIN_ID, strerror(ERRNO));
		/*
		 * If failed to set env variable then free the memory allocated
		 * to env variables and exit the application.
		 */
		deinit_fastrpc_dsp_lib_refcnt();
		exit(EXIT_FAILURE);
	} else {
	...
	FOR_EACH_DOMAIN_ID(ii)
	{
		...
		if(NULL != env_val) {
			...
			/* If the total reference count exceeds one then show
			 * warning, application will abort on first handle
			 * open. */
			if(total_dsp_lib_refcnt > MAX_LIB_INSTANCE_ALLOWED) {
				FARF(ERROR,
				     "Warning: %s %d instances of libxdsprpc "
				     "(already loaded %s). Only "
				     "%d allowed\n",
				     fastrpc_library[DEFAULT_DOMAIN_ID],
...
multidsplib_env_init(void)
{
	const char *local_fastrpc_lib_refcnt[NUM_DOMAINS]
		...
		"FASTRPC_CDSP1_REFCNT", "FASTRPC_GDSP0_REFCNT",
	...
	FOR_EACH_DOMAIN_ID(ii)
	{
		...
		if(env_name) {
			...
			FARF(ERROR,
			     "Error %d: %s: env variable allocation for %zu "
			     "bytes failed, domain "
			     "%d\n",
