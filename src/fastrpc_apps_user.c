...
static const char *fastrpc_library[NUM_DOMAINS]
	...
	"libcdsprpc.so", "libcdsprpc.so", "libcdsprpc.so" };
...
#define IS_KERNEL_PERF_ENABLED(var) ((var & FASTRPC_MODE_PERF_KERNEL) && perf_v2_kernel)
...
const char *ENV_DEBUG_VAR_NAME[]
    = { "FASTRPC_PROCESS_ATTRS",  "FASTRPC_DEBUG_TRACE",  "FASTRPC_DEBUG_TESTSIG",
	"FASTRPC_PERF_KERNEL",    "FASTRPC_PERF_ADSP",    "FASTRPC_PERF_FREQ",
	"FASTRPC_DEBUG_SYSTRACE", "FASTRPC_DEBUG_PDDUMP", "FASTRPC_PROCESS_ATTRS_PERSISTENT",
	"ro.debuggable" };
const char *ANDROIDP_DEBUG_VAR_NAME[] = { "vendor.fastrpc.process.attrs",
	                                  ...
	                                  "ro.build.type" };
const char *ANDROID_DEBUG_VAR_NAME[]
    = { "fastrpc.process.attrs",  "fastrpc.debug.trace",  "fastrpc.debug.testsig",
	"fastrpc.perf.kernel",    "fastrpc.perf.adsp",    "fastrpc.perf.freq",
	"fastrpc.debug.systrace", "fastrpc.debug.pddump", "persist.fastrpc.process.attrs",
	"ro.build.type" };

const char *SUBSYSTEM_NAME[]
    = { "adsp", "mdsp", "sdsp", "cdsp", "cdsp1", "gdsp0", "gdsp1", "reserved" };
...
static const size_t invoke_begin_trace_strlen = sizeof(INVOKE_BEGIN_TRACE_STR) - 1;
static const size_t invoke_end_trace_strlen = sizeof(INVOKE_END_TRACE_STR) - 1;

int NO_ENV_DEBUG_VAR_NAME_ARRAY_ELEMENTS = sizeof(ENV_DEBUG_VAR_NAME) / sizeof(char *);
int NO_ANDROIDP_DEBUG_VAR_NAME_ARRAY_ELEMENTS = sizeof(ANDROIDP_DEBUG_VAR_NAME) / sizeof(char *);
int NO_ANDROID_DEBUG_VAR_NAME_ARRAY_ELEMENTS = sizeof(ANDROID_DEBUG_VAR_NAME) / sizeof(char *);
...
int check_rpc_error(int err)
{
	if (check_error_code_change_present() == 1) {
		if (err > KERNEL_ERRNO_START && err <= HLOS_ERR_END) // driver or HLOS err
			...
			return 0;
		else if (err == AEE_ENOSUCH || err == AEE_EINTERRUPTED) // common DSP HLOS err
			return 0;
		else
			return -1;
	} else
...
int fastrpc_get_property_int(fastrpc_properties UserPropKey, int defValue)
{
	if (((int)UserPropKey > NO_ENV_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR, "%s: Index %d out-of-bound for ENV_DEBUG_VAR_NAME array of len %d",
	...
	if (((int)UserPropKey > NO_ANDROIDP_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR, "%s: Index %d out-of-bound for ANDROIDP_DEBUG_VAR_NAME array of len %d",
		     __func__, UserPropKey, NO_ANDROIDP_DEBUG_VAR_NAME_ARRAY_ELEMENTS);
		return defValue;
	}
	return (int)property_get_int32(ANDROIDP_DEBUG_VAR_NAME[UserPropKey], defValue);
#else // system library
	if (((int)UserPropKey > NO_ANDROID_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR, "%s: Index %d out-of-bound for ANDROID_DEBUG_VAR_NAME array of len %d",
...
int fastrpc_get_property_string(fastrpc_properties UserPropKey, char *value, char *defValue)
{
	int len = 0;
	if (((int)UserPropKey > NO_ENV_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR, "%s: Index %d out-of-bound for ENV_DEBUG_VAR_NAME array of len %d",
	...
	return property_get(ANDROIDP_DEBUG_VAR_NAME[UserPropKey], (int *)value, (int *)defValue);
#else // system library
	if (((int)UserPropKey > NO_ANDROID_DEBUG_VAR_NAME_ARRAY_ELEMENTS)) {
		FARF(ERROR, "%s: Index %d out-of-bound for ANDROID_DEBUG_VAR_NAME array of len %d",
		     __func__, UserPropKey, NO_ANDROID_DEBUG_VAR_NAME_ARRAY_ELEMENTS);
...
static inline int is_first_reverse_rpc_call(int domain, remote_handle handle, uint32_t sc)
{
	int ret = 0;
...
static int verify_local_handle(int domain, remote_handle64 local)
{
	...
	VERIFYC((hinfo->hlist >= &hlist[0]) && (hinfo->hlist < &hlist[NUM_DOMAINS_EXTEND]),
	        AEE_ERPC);
 ...
 * @return : Domain ID
 */
static int get_domain_from_domain_name(const char *domain_name, int domain_name_len)
{
...
inline int is_smmu_enabled(void) { return hlist[get_current_domain()].info & FASTRPC_INFO_SMMU; }
...
static void print_open_handles(int domain)
{
	...
	QLIST_FOR_ALL(&hlist[domain].ql, pn)
	{
		...
		if (hi->name)
			FARF(RUNTIME_RPC_HIGH, "%s, handle 0x%" PRIx64 "", hi->name, hi->remote);
	}
	pthread_mutex_unlock(&hlist[domain].mut);
}
...
bail:
	FARF(RUNTIME_RPC_ERROR, "Warning 0x%x: %s failed for uri %s", nErr, __func__, uri);
	return NULL;
}
...
int fastrpc_update_module_list(uint32_t req, int domain, remote_handle64 h, remote_handle64 *local,
                               const char *name)
{
	int nErr = AEE_SUCCESS;
	...
	case DOMAIN_LIST_DEQUEUE: {
		VERIFY(AEE_SUCCESS == (nErr = fastrpc_free_handle(domain, &hlist[domain].ql, h)));
		if (IS_CONST_HANDLE(h)) {
			pthread_mutex_lock(&hlist[domain].lmut);
			hlist[domain].constCount--;
			pthread_mutex_unlock(&hlist[domain].lmut);
	...
	case NON_DOMAIN_LIST_DEQUEUE: {
		VERIFY(AEE_SUCCESS == (nErr = fastrpc_free_handle(domain, &hlist[domain].nql, h)));
		pthread_mutex_lock(&hlist[domain].lmut);
		hlist[domain].nondomainsCount--;
		pthread_mutex_unlock(&hlist[domain].lmut);
		break;
	...
	case REVERSE_HANDLE_LIST_DEQUEUE: {
		VERIFY(AEE_SUCCESS == (nErr = fastrpc_free_handle(domain, &hlist[domain].rql, h)));
		pthread_mutex_lock(&hlist[domain].lmut);
		hlist[domain].reverseCount--;
		pthread_mutex_unlock(&hlist[domain].lmut);
		break;
...
bail:
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for request ID %u, handle 0x%x, domain %d\n",
...
static void fastrpc_timer_callback(void *ptr)
{
	...
	FARF(ALWAYS, "%s fastrpc time out of %d ms on thread %d on domain %d sc 0x%x handle 0x%x\n",
	     __func__, frpc_timer->timeout_millis, frpc_timer->tid, frpc_timer->domain,
	     frpc_timer->sc, frpc_timer->handle);
...
static void fastrpc_add_timer(fastrpc_timer *frpc_timer)
{
	...
	sigevent.sigev_notify_function = (void (*)(union sigval))fastrpc_timer_callback;
	...
	time_spec.it_value.tv_nsec = (frpc_timer->timeout_millis % 1000) * 1000 * 1000;
...
int remote_handle_invoke_domain(int domain, remote_handle handle, fastrpc_async_descriptor_t *desc,
                                uint32_t sc, remote_arg *pra)
{
	...
	if (IS_QTF_TRACING_ENABLED(hlist[domain].procattrs) && !IS_STATIC_HANDLE(handle)
	    && trace_marker_fd > 0) {
	...
	if (desc) {
		...
		VERIFYC(desc->type >= FASTRPC_ASYNC_NO_SYNC && desc->type < FASTRPC_ASYNC_TYPE_MAX,
		        AEE_EBADPARM);
	...
	for (i = 0; i < bufs; i++) {
		...
		if (pra[i].buf.nLen) {
			...
			if (nova) {
				...
				// pra[i].buf.pv = (void*)((uintptr_t)pra[i].buf.pv -
				// (uintptr_t)base);
			...
			if (attr & FASTRPC_ATTR_FORCE_NOFLUSH) {
				...
				VERIFY_IPRINTF(
				    "force no flush for buffer idx: %d addr: %p size: %d", i,
				    pra[i].buf.pv, pra[i].buf.nLen);
			}
			if (attr & FASTRPC_ATTR_FORCE_NOINVALIDATE) {
				...
				VERIFY_IPRINTF(
				    "force no invalidate buffer idx: %d addr: %p size: %d", i,
				    pra[i].buf.pv, pra[i].buf.nLen);
			}
	...
	if (IS_CRC_CHECK_ENABLED(hlist[domain].procattrs) && (!IS_STATIC_HANDLE(handle))
	    && !asyncjob.isasyncjob) {
	...
	if (IS_KERNEL_PERF_ENABLED(hlist[domain].procattrs) && (!IS_STATIC_HANDLE(handle))) {
		perf_kernel = (uint64_t *)calloc(PERF_KERNEL_KEY_MAX, sizeof(uint64_t));
		VERIFYC(perf_kernel != NULL, AEE_ENOMEMORY);
		req = INVOKE_PERF;
	}
	if (IS_DSP_PERF_ENABLED(hlist[domain].procattrs) && (!IS_STATIC_HANDLE(handle))) {
	...
	nErr = ioctl_invoke(dev, req, handle, sc, get_args(), pfds, pattrs, job, crc_remote,
	...
	if (IS_CRC_CHECK_ENABLED(hlist[domain].procattrs) && (!IS_STATIC_HANDLE(handle))
		...
		for (i = 0; (i < bufs) && (i < M_CRCLIST); i++) {
			if (crc_local[i] != crc_remote[i]) {
				FARF(ERROR,
				     "CRC mismatch for buffer %d[%d], crc local %x remote %x", i,
	...
	if (IS_KERNEL_PERF_ENABLED(hlist[domain].procattrs) && (!IS_STATIC_HANDLE(handle))
		...
		FARF(ALWAYS,
		     ...
		     handle, sc, perf_kernel[0], perf_kernel[1], perf_kernel[2], perf_kernel[3],
		     perf_kernel[4], perf_kernel[5], perf_kernel[6], perf_kernel[7],
		     perf_kernel[8]);
	}
	if (IS_DSP_PERF_ENABLED(hlist[domain].procattrs) && (!IS_STATIC_HANDLE(handle))
	    && !asyncjob.isasyncjob) {
		...
		FARF(ALWAYS,
		     ...
		     " us G:%" PRIu64 " us INVOKE:%" PRIu64 " us P:%" PRIu64 " us CACHE:%" PRIu64
		     " us UM:%" PRIu64 " us "
		     "UM_H:%" PRIu64 " us R:%" PRIu64 " us E_R:%" PRIu64 " us J_S_T:%" PRIu64
		     " us\n",
		     handle, sc, perf_dsp[0], perf_dsp[1], perf_dsp[2], perf_dsp[3], perf_dsp[4],
		     perf_dsp[5], perf_dsp[6], perf_dsp[7], perf_dsp[8], perf_dsp[9], perf_dsp[10],
		     perf_dsp[11]);
	}
...
int remote_handle_invoke(remote_handle handle, uint32_t sc, remote_arg *pra)
{
	...
	FARF(RUNTIME_RPC_HIGH, "Entering %s, handle %u sc %X remote_arg %p\n", __func__, handle, sc,
	     pra);
	FASTRPC_ATRACE_BEGIN_L("%s called with handle 0x%x , scalar 0x%x", __func__, (int)handle,
	                       sc);
	VERIFYC(handle != (remote_handle)-1, AEE_EINVHANDLE);

	domain = get_current_domain();
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS == (nErr = remote_handle_invoke_domain(domain, handle, NULL, sc, pra)));
bail:
		...
		if (0 == check_rpc_error(nErr)) {
			if (get_logger_state(domain)) {
				FARF(
				    ...
				    "(sc 0x%x) (errno %s)\n",
				    nErr, __func__, (int)handle, REMOTE_SCALARS_METHOD(sc), domain,
				    sc, strerror(errno));
			}
		}
	}
...
int remote_handle64_invoke(remote_handle64 local, uint32_t sc, remote_arg *pra)
{
	...
	FASTRPC_ATRACE_BEGIN_L("%s called with handle 0x%x , scalar 0x%x", __func__, (int)local,
	                       sc);
	...
	VERIFY(AEE_SUCCESS == (nErr = remote_handle_invoke_domain(domain, remote, NULL, sc, pra)));
bail:
	FASTRPC_PUT_REF(domain);
	if (nErr != AEE_SUCCESS) {
		if (is_process_exiting(domain)) {
		...
		if (0 == check_rpc_error(nErr)) {
			if (get_logger_state(domain)) {
				FARF(ERROR,
				     ...
				     nErr, __func__, h->name, local, REMOTE_SCALARS_METHOD(sc),
...
int remote_handle_invoke_async(remote_handle handle, fastrpc_async_descriptor_t *desc, uint32_t sc,
                               remote_arg *pra)
{
	int domain = -1, nErr = AEE_SUCCESS, ref = 0;
	...
	domain = get_current_domain();
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS == (nErr = remote_handle_invoke_domain(domain, handle, desc, sc, pra)));
bail:
	FASTRPC_PUT_REF(domain);
	if (nErr != AEE_SUCCESS) {
		if (0 == check_rpc_error(nErr)) {
...
int remote_handle64_invoke_async(remote_handle64 local, fastrpc_async_descriptor_t *desc,
                                 uint32_t sc, remote_arg *pra)
{
	remote_handle64 remote = 0;
	int nErr = AEE_SUCCESS, domain = -1, ref = 0;
	...
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS == (nErr = get_handle_remote(local, &remote)));
	VERIFY(AEE_SUCCESS == (nErr = remote_handle_invoke_domain(domain, remote, desc, sc, pra)));
bail:
	FASTRPC_PUT_REF(domain);
	if (nErr != AEE_SUCCESS) {
		if (0 == check_rpc_error(nErr)) {
...
int remote_handle_open_domain(int domain, const char *name, remote_handle *ph, uint64_t *t_spawn,
                              uint64_t *t_load)
{
	char dlerrstr[255];
	...
	FASTRPC_ATRACE_BEGIN_L("%s called with domain %d, name %s, handle 0x%x", __func__, domain,
	                       name, ph);
	/* If the total reference count exceeds one then exit the application. */
	if (total_dsp_lib_refcnt > MAX_LIB_INSTANCE_ALLOWED) {
		FARF(ERROR, "Error: aborting due to %d instances of libxdsprpc. Only %d allowed\n",
	...
	if (!strncmp(name, ITRANSPORT_PREFIX "createstaticpd",
		...
		VERIFYC(NULL != (pdname_uri = (char *)malloc((name_len + 1) * sizeof(char))),
		        AEE_ENOMEMORY);
	...
	PROFILE_ALWAYS(t_spawn, VERIFY(AEE_SUCCESS == (nErr = domain_init(domain, &dev)));
	               VERIFYM(-1 != dev, AEE_ERPC, "open dev failed\n"););
	PROFILE_ALWAYS(
	    t_load,
	    if ((handle = get_remotectl1_handle(domain)) != INVALID_HANDLE) {
		    nErr = remotectl1_open1(handle, name, (int *)ph, dlerrstr, sizeof(dlerrstr),
		                            &dlerr);
		    if (nErr) {
			    FARF(ALWAYS,
		    ...
		    VERIFY(AEE_SUCCESS
		           == (nErr = remotectl_open(name, (int *)ph, dlerrstr, sizeof(dlerrstr),
		                                     &dlerr)));
...
int remote_handle_open(const char *name, remote_handle *ph)
{
	...
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_open_domain(domain, name, ph, &t_spawn, &t_load)));
...
int remote_handle64_open(const char *name, remote_handle64 *ph)
{
	...
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_open_domain(domain, name, &h, &t_spawn, &t_load)));
	/* Returning local handle to "geteventd" call causes bad fd error when daemon
	   ...
	   "geteventd" call*/
	if (!strncmp(name, ITRANSPORT_PREFIX "geteventfd", strlen(ITRANSPORT_PREFIX "geteventfd"))
...
static int manage_pm_qos(int domain, remote_handle64 h, uint32_t enable, uint32_t latency)
{
	...
	if (hlist[domain].dev != -1) {
		...
		if ((handle = get_remotectl1_handle(domain)) != INVALID_HANDLE) {
			...
			if (nErr) {
				...
				// Set remotectlhandle to INVALID_HANDLE, so that all subsequent
				// calls are non-domain calls
				hlist[domain].remotectlhandle = INVALID_HANDLE;
...
static int manage_poll_qos(int domain, remote_handle64 h, uint32_t enable, uint32_t latency)
{
	...
	if (h == INVALID_HANDLE) {
		VERIFY(AEE_SUCCESS == (nErr = adsp_current_process_poll_mode(enable, latency)));
	} else {
	...
	FARF(ALWAYS,
	     "%s: poll mode updated to %u for domain %d, handle 0x%" PRIx64 " for timeout %u\n",
...
static inline int enable_process_state_notif_on_dsp(int domain)
{
	...
	if ((notif_handle = get_adsp_current_process1_handle(domain)) != INVALID_HANDLE) {
		VERIFY(AEE_SUCCESS
 ...
 *
 */
int get_remote_async_response(int domain, fastrpc_async_jobid *jobid, int *result)
{
	int nErr = AEE_SUCCESS, dev = -1;
	...
	nErr = ioctl_invoke2_response(dev, &job, &handle, &sc, &res, perf_kernel, perf_dsp);
	if (IS_KERNEL_PERF_ENABLED(hlist[domain].procattrs) && (!IS_STATIC_HANDLE(handle))) {
		VERIFYC(perf_kernel != NULL, AEE_ENOMEMORY);
		FARF(ALWAYS,
		     "RPCPERF-K  H:0x%x SC:0x%x C:%" PRIu64 " F:%" PRIu64 " ns M:%" PRIu64
...
/* fastrpc_set_qos_latency:
   ...
   guaranteed.
*/
static int fastrpc_set_qos_latency(int domain, remote_handle64 h, uint32_t latency)
{
	int nErr = 0;
...
static int update_kernel_wakelock_status(int domain, int dev, uint32_t wl_enable)
{
	int nErr = AEE_SUCCESS;
	...
	if (nErr) {
		if (errno == EBADRQC || errno == ENOTTY || errno == ENXIO || errno == EINVAL
		...
		FARF(ALWAYS, "%s: updated kernel wakelock control status to %d for domain %d",
...
static int wakelock_control(int domain, remote_handle64 h, uint32_t wl_enable)
{
	...
	if (IS_SESSION_OPEN_ALREADY(domain))
		VERIFY(AEE_SUCCESS
		       == (nErr
		           = update_kernel_wakelock_status(domain, hlist[domain].dev, wl_enable)));
	fastrpc_wake_lock_enable[domain] = wl_enable;
bail:
	if (nErr)
		FARF(ERROR, "Error 0x%x: %s failed for domain %d, handle 0x%x, enable %d", nErr,
		     __func__, domain, h, wl_enable);
	else {
		if (wl_enable)
...
static int fastrpc_dsp_process_clean(int domain)
{
	...
	VERIFYM(IS_SESSION_OPEN_ALREADY(domain), AEE_ERPC, "Session not open for domain %d",
	        domain);
	dev = hlist[domain].dev;
	nErr = ioctl_control(dev, DSPRPC_REMOTE_PROCESS_KILL, NULL);
bail:
	if (nErr)
		FARF(ERROR, "Error 0x%x: %s failed for domain %d (errno: %s) ", nErr, __func__,
		     domain, strerror(errno));
	return nErr;
}

int remote_handle_control_domain(int domain, remote_handle64 h, uint32_t req, void *data,
                                 uint32_t len)
{
	int nErr = AEE_SUCCESS;
	...
	case DSPRPC_CONTROL_WAKELOCK: {
		struct remote_rpc_control_wakelock *wp = (struct remote_rpc_control_wakelock *)data;
...
bail:
	if (nErr != AEE_SUCCESS)
		FARF(ERROR, "Error 0x%x: %s failed for request ID %d on domain %d (errno %s)", nErr,
		     __func__, req, domain, strerror(errno));
	return nErr;
}
...
int remote_handle_control(uint32_t req, void *data, uint32_t len)
{
	...
	FARF(RUNTIME_HIGH, "Entering %s, req %d, data %p, size %d\n", __func__, req, data, len);

	domain = get_current_domain();
	FASTRPC_GET_REF(domain);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_handle_control_domain(domain, INVALID_HANDLE, req, data, len)));
bail:
	FASTRPC_PUT_REF(domain);
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for request ID %d (errno %s)", nErr, __func__,
		     req, strerror(errno));
		if (0 == check_rpc_error(nErr)) {
...
static int close_domain_session(int domain)
{
	...
	FARF(ALWAYS, "%s: user requested to close fastrpc session on domain %d, dev %d\n", __func__,
	...
	if (hlist[domain].dev != -1) {
		if (hlist[domain].unsigned_module == enable) {
			...
			FARF(ERROR,
			     ...
			     nErr, __func__, hlist[domain].unsigned_module ? "Unsigned" : "Signed",
			     domain, enable);
		}
		goto bail;
	}
...
static int store_domain_thread_params(int domain, int thread_priority, int stack_size)
{
	int nErr = AEE_SUCCESS;
	...
	if (stack_size != -1) {
		if ((stack_size < MIN_UTHREAD_STACK_SIZE)
			...
			FARF(ERROR, "%s: Stack size %d is invalid! Should be between %d and %d",
			     __func__, stack_size, MIN_UTHREAD_STACK_SIZE, MAX_UTHREAD_STACK_SIZE);
			goto bail;
		} else
...
bail:
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed to enable %d for domain %d", nErr, __func__,
		     enable, domain);
	}
	return nErr;
}
...
static int store_domain_pd_initmem_size(int domain, uint32_t pd_initmem_size)
{
	...
	if ((pd_initmem_size < MIN_PD_INITMEM_SIZE) || (pd_initmem_size > MAX_PD_INITMEM_SIZE)) {
		nErr = AEE_EBADPARM;
		goto bail;
	} else {
...
bail:
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for domain %d for pd init mem size :0x%x", nErr,
...
int remote_session_control(uint32_t req, void *data, uint32_t datalen)
{
	...
	case FASTRPC_THREAD_PARAMS: {
		...
		if (params->domain != -1) {
			...
			{
				VERIFY(AEE_SUCCESS
				       == (nErr = store_domain_thread_params(ii, params->prio,
				                                             params->stack_size)));
			}
		}
		FARF(ALWAYS, "%s DSP info request for domain %d, thread priority %d, stack size %d",
	...
	case DSPRPC_CONTROL_UNSIGNED_MODULE: {
		...
		VERIFYC(datalen == sizeof(struct remote_rpc_control_unsigned_module), AEE_EBADPARM);
		...
		FARF(ALWAYS, "%s Unsigned PD enable %d request for domain %d", __func__, um->enable,
		     um->domain);
		break;
	}
	case FASTRPC_RELATIVE_THREAD_PRIORITY: {
		...
		if (params->domain != -1) {
			...
			VERIFY(AEE_SUCCESS
			       == (nErr = store_domain_thread_params(params->domain,
			                                             thread_priority, -1)));
		} else {
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
				VERIFY(AEE_SUCCESS
				       == (nErr
				           = store_domain_thread_params(ii, thread_priority, -1)));
			}
		}
		FARF(ALWAYS, "%s DSP thread priority request for domain %d, priority %d", __func__,
		     req, params->domain, thread_priority);
		break;
	}
	case FASTRPC_REMOTE_PROCESS_KILL: {
		...
		VERIFYC(datalen == sizeof(struct remote_rpc_process_clean_params), AEE_EBADPARM);
		VERIFYC(dp, AEE_EBADPARM);
		domain = dp->domain;
		VERIFY(AEE_SUCCESS == (nErr = fastrpc_dsp_process_clean(domain)));
		FARF(ALWAYS, "%s Remote process kill request for domain %d", __func__, domain);
		break;
	}
	case FASTRPC_SESSION_CLOSE: {
		struct remote_rpc_session_close *sclose = (struct remote_rpc_session_close *)data;
		if (!sclose) {
			nErr = AEE_EBADPARM;
			FARF(RUNTIME_RPC_LOW,
	...
	case FASTRPC_REGISTER_STATUS_NOTIFICATIONS: {
		...
		struct remote_rpc_notif_register *notif = (struct remote_rpc_notif_register *)data;
	...
	case FASTRPC_PD_INITMEM_SIZE: {
		...
		if (params->domain != -1) {
			...
			if (hlist[params->domain].unsigned_module) {
				nErr = AEE_EUNSUPPORTED;
				FARF(ERROR,
				     "Configuring User PD init mem length is not supported for "
				     "unsigned PDs");
				goto bail;
			}
			VERIFY(AEE_SUCCESS
			       == (nErr = store_domain_pd_initmem_size(params->domain,
			...
			FOR_EACH_EFFECTIVE_DOMAIN_ID(ii)
			{
				if (hlist[ii].unsigned_module) {
					FARF(ALWAYS,
					     "Warning: %s: Configuring User PD init mem length for "
					     "domain %d "
		...
		FARF(ALWAYS, "%s DSP userpd memlen request for domain %d, userpd memlen 0x%x",
		     __func__, params->domain, params->pd_initmem_size);
		break;
	}
	case FASTRPC_RESERVE_NEW_SESSION: {
		remote_rpc_reserve_new_session_t *sess = (remote_rpc_reserve_new_session_t *)data;
		int ii = 0, jj = 0;
	...
	case FASTRPC_GET_EFFECTIVE_DOMAIN_ID: {
		...
		VERIFYC(effec_domain_id && effec_domain_id->domain_name
		        ...
		        AEE_EBADPARM);
		domain
		    = get_domain_from_name(effec_domain_id->domain_name, DOMAIN_NAME_STAND_ALONE);
		VERIFYC(IS_VALID_DOMAIN_ID(domain), AEE_EBADPARM);
	...
	case FASTRPC_GET_URI: {
		...
		VERIFYC(rpc_uri && rpc_uri->domain_name && rpc_uri->domain_name_len > 0
		        ...
		        AEE_EBADPARM);
		domain = get_domain_from_name(rpc_uri->domain_name, DOMAIN_NAME_STAND_ALONE);
		VERIFYC(IS_VALID_DOMAIN_ID(domain), AEE_EBADPARM);
		VERIFYC(rpc_uri->module_uri != NULL && rpc_uri->module_uri_len > 0, AEE_EBADPARM);
		VERIFYC(rpc_uri->uri != NULL && rpc_uri->uri_len > rpc_uri->module_uri_len,
		        AEE_EBADPARM);
		ret_val
		    = snprintf(0, 0, "%s%s%s%s%d", rpc_uri->module_uri, FASTRPC_DOMAIN_URI,
		               SUBSYSTEM_NAME[domain], FASTRPC_SESSION_URI, rpc_uri->session_id);
		...
		ret_val = snprintf(rpc_uri->uri, rpc_uri->uri_len, "%s%s%s%s%d",
		                   rpc_uri->module_uri, FASTRPC_DOMAIN_URI, SUBSYSTEM_NAME[domain],
		                   FASTRPC_SESSION_URI, rpc_uri->session_id);
		if (ret_val < 0) {
			nErr = AEE_EBADSIZE;
			FARF(ERROR, "ERROR 0x%x: %s Invalid Session URI length %u", nErr, __func__,
			     rpc_uri->uri_len);
		}
		break;
	}
	...
	default:
		nErr = AEE_EUNSUPPORTED;
		FARF(ERROR, "ERROR 0x%x: %s Unsupported request ID %d", nErr, __func__, req);
		break;
	}
bail:
	FASTRPC_PUT_REF(domain);
...
int get_domain_from_name(const char *uri, uint32_t type)
{
	...
	if (uri && type == DOMAIN_NAME_STAND_ALONE) {
		if (!strncmp(uri, ADSP_DOMAIN_NAME, strlen(ADSP_DOMAIN_NAME))) {
			domain = ADSP_DOMAIN_ID;
		} else if (!strncmp(uri, MDSP_DOMAIN_NAME, strlen(MDSP_DOMAIN_NAME))) {
			domain = MDSP_DOMAIN_ID;
		} else if (!strncmp(uri, SDSP_DOMAIN_NAME, strlen(SDSP_DOMAIN_NAME))) {
			domain = SDSP_DOMAIN_ID;
		} else if (!strncmp(uri, CDSP1_DOMAIN_NAME, strlen(CDSP1_DOMAIN_NAME))) {
			domain = CDSP1_DOMAIN_ID;
		} else if (!strncmp(uri, CDSP_DOMAIN_NAME, strlen(CDSP_DOMAIN_NAME))) {
			domain = CDSP_DOMAIN_ID;
		} else if (!strncmp(uri, GDSP0_DOMAIN_NAME, strlen(GDSP0_DOMAIN_NAME))) {
			domain = GDSP0_DOMAIN_ID;
		} else if (!strncmp(uri, GDSP1_DOMAIN_NAME, strlen(GDSP1_DOMAIN_NAME))) {
			domain = GDSP1_DOMAIN_ID;
		} else {
...
static void domain_deinit(int domain)
{
	...
	if (olddev != -1) {
		...
		FASTRPC_ATRACE_BEGIN_L("%s called for handle 0x%x, domain %d, dev %d", __func__,
		...
		FARF(RUNTIME_RPC_HIGH,
		     "%s: closed device %d on domain %d (kill time %" PRIu64 " us)", __func__,
		     olddev, domain, t_kill);
...
static int get_process_attrs(int domain)
{
	...
	attrs |= (hlist[domain].pd_dump | fastrpc_config_is_pddump_enabled())
	             ...
	             : 0;
	attrs |= fastrpc_get_property_int(FASTRPC_DEBUG_PDDUMP, 0) ? FASTRPC_MODE_DEBUG_PDDUMP : 0;
	...
	attrs |= (fastrpc_config_is_perfdsp_enabled()
	          | fastrpc_get_property_int(FASTRPC_PERF_ADSP, 0))
	             ...
	             : 0;
	attrs |= fastrpc_config_is_log_iregion_enabled() ? FASTRPC_MODE_ENABLE_IREGION_LOG : 0;
	attrs |= fastrpc_config_is_qtf_tracing_enabled() ? FASTRPC_MODE_ENABLE_QTF_TRACING : 0;
	attrs |= (fastrpc_config_get_caller_level() << 13) & FASTRPC_MODE_CALLER_LEVEL_MASK;
	attrs |= fastrpc_config_is_uaf_enabled() ? FASTRPC_MODE_ENABLE_UAF : 0;
	attrs |= fastrpc_config_is_debug_logging_enabled() ? FASTRPC_MODE_ENABLE_DEBUG_LOGGING : 0;
#ifdef SYSTEM_RPC_LIBRARY
	attrs |= FASTRPC_MODE_SYSTEM_PROCESS;
#endif
	attrs |= fastrpc_config_is_sysmon_reserved_bit_enabled() ? FASTRPC_MODE_SYSMON_RESERVED_BIT
	                                                         : 0;
	attrs |= fastrpc_config_is_logpacket_enabled() ? FASTRPC_MODE_LOG_PACKET : 0;
	attrs |= (fastrpc_config_get_leak_detect() << 19) & FASTRPC_MODE_ENABLE_LEAK_DETECT;
	attrs |= (fastrpc_config_get_caller_stack_num() << 21) & FASTRPC_MODE_CALLER_STACK_NUM;
	return attrs;
}
...
static int open_shell(int domain_id, apps_std_FILE *fh, int unsigned_shell)
{
	...
	VERIFYC(NULL != (shell_absName = (char *)malloc(sizeof(char) * shell_absNameLen)),
	        AEE_ENOMEMORY);
	...
	if (nErr) {
		...
		VERIFYC(NULL != (absName = (char *)realloc(absName, sizeof(char) * absNameLen)),
		        AEE_ENOMEMORY);
		strlcpy(absName, DSP_MOUNT_LOCATION, absNameLen);
	...
	if (nErr) {
		...
		if (nErr) {
			...
			VERIFYC(
			    NULL != (absName = (char *)realloc(absName, sizeof(char) * absNameLen)),
			    AEE_ENOMEMORY);
			strlcpy(absName, VENDOR_DSP_LOCATION, absNameLen);
			strlcat(absName, SUBSYSTEM_NAME[domain], absNameLen);
			strlcat(absName, "/", absNameLen);
...
static int fastrpc_enable_kernel_optimizations(int domain)
{
	...
	if (((dom != CDSP_DOMAIN_ID) && (dom != CDSP1_DOMAIN_ID) && (dom != GDSP0_DOMAIN_ID)
	     && (dom != GDSP1_DOMAIN_ID))
	    || (hlist[domain].dsppd != USERPD))
		goto bail;
	errno = 0;
...
{
	bool dbgMode = false, crc = false, signedMd = false, unsignedMd = false, qos = false,
	     configPDdump = false;
	bool debugPDdump = false, KernelPerf = false, DSPperf = false, iregion = false, qtf = false,
	     uaf = false;
	...
	FARF(ALWAYS,
	     ...
	     domain, fastrpc_trace, fastrpc_config_get_rpctimeout(), (dbgMode ? "Y" : "N"),
	     (crc ? "Y" : "N"), (unsignedMd ? "Y" : "N"), (signedMd ? "Y" : "N"), (qos ? "Y" : "N"),
	     (configPDdump ? "Y" : "N"), (debugPDdump ? "Y" : "N"), (KernelPerf ? "Y" : "N"),
	     (DSPperf ? "Y" : "N"), (iregion ? "Y" : "N"), (qtf ? "Y" : "N"), (uaf ? "Y" : "N"),
...
static int remote_init(int domain)
{
	...
	if (hlist[domain].dev == -1) {
		...
		if (nErr == AEE_SUCCESS) {
			...
			// supported
			FARF(RUNTIME_RPC_HIGH,
			     "%s: FASTRPC_CONTROL_SESS_INFO not supported with error %d", __func__,
			     nErr);
		}
	...
	set_sess_info_supported:
		...
		FARF(RUNTIME_RPC_HIGH, "%s: device %d opened with info 0x%x (attach %d)", __func__,
		     dev, hlist[domain].info, pd_type);
		...
		if (pd_type == ROOT_PD || pd_type == GUEST_OS_SHARED
			...
			ioErr = ioctl_init(dev, flags, 0, (unsigned char *)file, filelen, -1, 0, 0,
			                   0, 0);
			if (file) {
				free(file);
				file = NULL;
			...
			ioErr = ioctl_init(dev, flags, 0, (unsigned char *)file, filelen, -1, 0,
			                   memlen, 0, 0);
			if (ioErr) {
			...
			if (fh != -1) {
				...
				VERIFYC(file, AEE_ENORPCMEMORY);
				VERIFY(AEE_SUCCESS
				       == (nErr = apps_std_fread(fh, (unsigned char *)file, len,
				                                 &readlen, &eof)));
			...
			if (!(FASTRPC_MODE_UNSIGNED_MODULE & hlist[domain].procattrs)) {
				...
				if (hlist[domain].pd_initmem_size != DEFAULT_PD_INITMEM_SIZE)
					FARF(ERROR, "Setting user PD initial memory length is not "
					            "supported "
			...
			if (hlist[domain].procattrs) {
				if (siglen && fsig != -1) {
					VERIFY(AEE_SUCCESS
					       == (nErr = apps_std_fread(
						       fsig, (unsigned char *)(file + len), siglen,
						       &readlen, &eof)));
			...
			ioErr
			    = ioctl_init(dev, flags, hlist[domain].procattrs, (unsigned char *)file,
			...
			if (ioErr) {
				...
				if (errno == ECONNREFUSED) {
					nErr = AEE_ECONNREFUSED;
					FARF(ERROR,
					     "Error 0x%x: %s: untrusted app trying to offload to "
					     "signed "
					     "remote process (errno %d, %s). Try offloading to "
					     "unsignedPD "
...
bail:
	...
	if (nErr != AEE_SUCCESS) {
		...
		FARF(ERROR, "Error 0x%x: %s failed for domain %d, errno %s, ioErr %d\n", nErr,
		     __func__, domain, strerror(errno), ioErr);
	}
...
bail:
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: get adsp msgd handle failed. domain %d (errno %s)\n", nErr,
		     domain, strerror(errno));
	}
	return INVALID_HANDLE;
}
...
static int domain_init(int domain, int *dev)
{
	...
	if (fastrpc_wake_lock_enable[domain]) {
		VERIFY(AEE_SUCCESS
		       == (nErr = update_kernel_wakelock_status(domain, hlist[domain].dev,
		...
		if (panic_handle != INVALID_HANDLE) {
			...
			if (err_codes_to_send) {
				...
				if (AEE_SUCCESS == ret) {
					...
					FARF(ERROR,
					     "Error 0x%x: %s : panic error codes send failed\n",
					     ret, __func__);
				}
			}
		} else {
	...
	if (fastrpc_notif_flag) {
		...
		if (ret == (int)(DSP_AEE_EOFFSET + AEE_EUNSUPPORTED)) {
			VERIFY_WPRINTF("Warning: %s: DSP does not support notifications", __func__);
		}
		VERIFYC(ret == AEE_SUCCESS || ret == (int)(DSP_AEE_EOFFSET + AEE_EUNSUPPORTED),
		        ret);
	}
	...
	VERIFY(AEE_SUCCESS
	       == (nErr
	           = listener_android_domain_init(domain, hlist[domain].th_params.update_requested,
...
bail:
	...
	if (hlist) {
		FARF(RUNTIME_RPC_LOW, "Done %s with dev %d, err %d", __func__, hlist[domain].dev,
		     nErr);
		*dev = hlist[domain].dev;
		return nErr;
	} else {
		*dev = -1;
		FARF(ERROR, "Error 0x%x: Unable to get dev as hlist is NULL for domain %d\n", nErr,
		     __func__, domain);
		return nErr;
	}
}
	...
	FOR_EACH_EFFECTIVE_DOMAIN_ID(domain)
	{
		if (hlist[domain].dev != -1) {
			if ((handle = get_adsp_current_process1_handle(domain)) != INVALID_HANDLE) {
				...
				if (nErr) {
					FARF(RUNTIME_RPC_HIGH, "%s: nErr:0x%x, dom:%d, h:0x%llx",
					     __func__, nErr, domain, handle);
				}
			} else if (domain == DEFAULT_DOMAIN_ID) {
				nErr = adsp_current_process_thread_exit();
				if (nErr) {
					FARF(RUNTIME_RPC_HIGH, "%s: nErr:0x%x, dom:%d", __func__,
					     nErr, domain);
				}
			}
		}
...
bail:
	...
	*/
	if (nErr) {
		FARF(ERROR, "Error 0x%x: %s failed. default domain:%x and &fastrpc_trace:%p \n",
...
static void check_multilib_util(void)
{
	...
	if (ret != 0 && errno == ENOMEM) {
		...
		FARF(ERROR, "Error 0x%x: setenv failed for %s (domain %u), errno is %s\n", nErr,
		     env_name, DEFAULT_DOMAIN_ID, strerror(ERRNO));
		/*
	...
	FOR_EACH_DOMAIN_ID(ii)
	{
		...
		if (NULL != env_val) {
			...
			if (total_dsp_lib_refcnt > MAX_LIB_INSTANCE_ALLOWED) {
				FARF(ERROR,
				     "Warning: %s %d instances of libxdsprpc (already loaded %s). "
				     "Only "
				     "%d allowed\n",
...
static void multidsplib_env_init(void)
{
	const char *local_fastrpc_lib_refcnt[NUM_DOMAINS]
		...
		"FASTRPC_CDSP_REFCNT", "FASTRPC_CDSP1_REFCNT", "FASTRPC_GDSP0_REFCNT",
		"FASTRPC_GDSP1_REFCNT" };
	char buf[64] = { 0 };
