	...
	do {
		...
		if(strncmp(param, CONFIG_ERR_CODES, strlen(CONFIG_ERR_CODES))
		   == 0) {
			...
			FARF(ALWAYS,
			     "%s: panic error codes applicable only to cdsp "
			     "domain\n",
			     __func__);
			do {
			...
			if(param != NULL && atoi(param)) {
				...
				FARF(ALWAYS,
				     "fastrpc config enabling PD dump\n");
			}
			...
			if(param != NULL) {
				...
				FARF(
				    ALWAYS,
				    "fastrpc config set rpc timeout with %d\n",
			...
			if(param != NULL && atoi(param)) {
				...
				FARF(ALWAYS, "fastrpc config enabling "
				             "profiling on kernel\n");
			}
			...
			if(param != NULL) {
				...
				strlcpy(frpc_config.farf_log_filename, param,
				        logFileNameLen);
				FARF(ALWAYS,
				     "fastrpc config enabling farf logs "
				     "collection into file %s",
				     frpc_config.farf_log_filename);
			}
		} else if(strncmp(
			...
			if(param != NULL) {
				...
				FARF(ALWAYS,
				     "fastrpc config enabling userspace farf "
				     "logs collection into file "
				     "%s",
				     frpc_config.farf_log_filename_userspace);
			}
			...
			if(param != NULL) {
				...
				FARF(ALWAYS, "fastrpc config enabling iregion "
				             "logging\n");
			}
			...
			if(param != NULL && atoi(param)) {
				...
				FARF(ALWAYS,
				     "fastrpc config enabling QTF tracing\n");
			}
			...
			if(param != NULL && atoi(param)) {
				...
				FARF(ALWAYS,
				     "fastrpc config setting heap caller "
				     "level with %d\n",
			...
			if(param != NULL && atoi(param)) {
				...
				FARF(ALWAYS,
				     "fastrpc config enabling uaf on heap\n");
			}
			...
			if(param != NULL) {
				...
				FARF(
				    ALWAYS,
				    "fastrpc config enabling debug logging\n");
			}
			...
			if(param != NULL) {
				...
				FARF(ALWAYS, "fastrpc config enabling sysmon "
				             "logging \n");
			}
			...
			if(param != NULL) {
				...
				FARF(ALWAYS,
				     "fastrpc config enabling Log packets\n");
			}
			...
			if(param != NULL && atoi(param)) {
				...
				FARF(ALWAYS,
				     "fastrpc config enabling leak detect "
				     "with %d\n",
			...
			if(param != NULL && atoi(param)) {
				...
				FARF(ALWAYS,
				     "fastrpc config setting call stack num "
				     "with %d\n",
	...
	for(i = 0; i < (int)numPaths; ++i) {
		...
		if(0
		   == fastrpc_read_config_file_from_path(paths[i].data,
			...
			file_found = 1;
			FARF(ALWAYS,
			     "Read fastrpc config file %s found at %s\n",
