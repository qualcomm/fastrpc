...
int fastrpc_read_config_file_from_path(const char *base, const char *file)
{
	...
	do {
		...
		if (strncmp(param, CONFIG_ERR_CODES, strlen(CONFIG_ERR_CODES)) == 0) {
			...
			do {
				...
				if (param != NULL) {
					...
					if (err_code >= MIN_DSP_ERR_RNG
						...
						FARF(ALWAYS,
						     "%s : panic error code read from debugcnfig : "
						     "0x%x is not in "
		...
		} else if (strncmp(param, CONFIG_PDDUMP, strlen(CONFIG_PDDUMP)) == 0) {
		...
		} else if (strncmp(param, CONFIG_RPCTIMEOUT, strlen(CONFIG_RPCTIMEOUT)) == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL) {
		...
		} else if (strncmp(param, CONFIG_PERF_KERNEL, strlen(CONFIG_PERF_KERNEL)) == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL && atoi(param)) {
				frpc_config.perfkernel = true;
				FARF(ALWAYS, "fastrpc config enabling profiling on kernel\n");
			}
		} else if (strncmp(param, CONFIG_PERF_DSP, strlen(CONFIG_PERF_DSP)) == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL && atoi(param)) {
			...
			if (param != NULL) {
				...
				strlcpy(frpc_config.farf_log_filename_userspace, param,
				        logFileNameLen);
				FARF(ALWAYS,
				     "fastrpc config enabling userspace farf logs collection into "
				     "file "
				     "%s",
				     frpc_config.farf_log_filename_userspace);
			}
		} else if (strncmp(param, CONFIG_LOG_IREGION, strlen(CONFIG_LOG_IREGION)) == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL) {
				frpc_config.log_iregion = true;
				FARF(ALWAYS, "fastrpc config enabling iregion logging\n");
			}
		} else if (strncmp(param, CONFIG_QTF_TRACING, strlen(CONFIG_QTF_TRACING)) == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL && atoi(param)) {
				frpc_config.qtf_tracing = true;
				FARF(ALWAYS, "fastrpc config enabling QTF tracing\n");
			}
		} else if (strncmp(param, CONFIG_CALLER_LEVEL, strlen(CONFIG_CALLER_LEVEL)) == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL && atoi(param)) {
		...
		} else if (strncmp(param, CONFIG_ENABLE_UAF, strlen(CONFIG_ENABLE_UAF)) == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL && atoi(param)) {
				frpc_config.uaf_enabled = true;
				FARF(ALWAYS, "fastrpc config enabling uaf on heap\n");
			}
		} else if (strncmp(param, CONFIG_DEBUG_LOGGING, strlen(CONFIG_DEBUG_LOGGING))
		           == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL) {
		...
		} else if (strncmp(param, CONFIG_LOGPACKET, strlen(CONFIG_LOGPACKET)) == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL) {
				frpc_config.logPackets = true;
				FARF(ALWAYS, "fastrpc config enabling Log packets\n");
			}
		} else if (strncmp(param, CONFIG_LEAK_DETECT, strlen(CONFIG_LEAK_DETECT)) == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL && atoi(param)) {
		...
		} else if (strncmp(param, CONFIG_CALL_STACK_NUM, strlen(CONFIG_CALL_STACK_NUM))
		           == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL && atoi(param)) {
		...
		} else if (strncmp(param, CONFIG_SETDMABUFNAME, strlen(CONFIG_SETDMABUFNAME))
		           == 0) {
			param = strtok_r(NULL, delim, &saveptr);
			if (param != NULL && atoi(param))
...
bool fastrpc_config_is_perfkernel_enabled(void) { return frpc_config.perfkernel; }
...
char *fastrpc_config_get_runtime_farf_file(void) { return frpc_config.farf_log_filename; }
...
bool fastrpc_config_is_log_iregion_enabled(void) { return frpc_config.log_iregion; }
// Function to get if debug logging feature is enabled.
bool fastrpc_config_is_debug_logging_enabled(void) { return frpc_config.debug_logging; }

// Function to get if debug logging feature is enabled for sysmon reserved bit.
bool fastrpc_config_is_sysmon_reserved_bit_enabled(void) { return frpc_config.sysmonreservedbit; }

// Function to get if QTF tracing is enabled.
bool fastrpc_config_is_qtf_tracing_enabled(void) { return frpc_config.qtf_tracing; }
...
bool fastrpc_config_is_logpacket_enabled(void) { return frpc_config.logPackets; }
...
int fastrpc_config_get_caller_stack_num(void) { return frpc_config.num_call_stack; }

bool fastrpc_config_is_setdmabufname_enabled(void) { return frpc_config.setdmabufname; }
...
int fastrpc_config_init()
{
	...
	apps_std_get_search_paths_with_env(ADSP_LIBRARY_PATH, ";", NULL, 0, &numPaths, &maxPathLen);
	maxPathLen += +1;

	// Allocate memory for the PATH's
	VERIFYC(NULL != (paths = calloc(1, sizeof(_cstring1_t) * numPaths)), AEE_ENOMEMORY);
	...
	VERIFYC(NULL != (data_paths = calloc(1, sizeof(char) * maxPathLen * numPaths)),
	        AEE_ENOMEMORY);

	// Get the paths
	VERIFY(AEE_SUCCESS
	       == (nErr = apps_std_get_search_paths_with_env(ADSP_LIBRARY_PATH, ";", paths,
	                                                     numPaths, &len, &maxPathLen)));
	maxPathLen += 1;
	for (i = 0; i < (int)numPaths; ++i) {
		strlcat(data_paths, paths[i].data, sizeof(char) * maxPathLen * numPaths);
