	...
	if(AEE_SUCCESS
	   != (nErr = mod_table_close(handle, errStr, errStrLen, dlErr))) {
		if(!is_process_exiting(domain)) {
			FARF(ERROR,
			     "Error 0x%x: %s: mod_table_close failed for "
			     "handle:0x%x (dlErr %s)",
...
listener(listener_config *me)
{
	...
	invoke:
		...
		FARF(RUNTIME_RPC_HIGH,
		     "%s responding 0x%x for ctx 0x%x, handle 0x%x, sc 0x%x",
		...
		if(nErr) {
			if(nErr == AEE_EINTERRUPTED) {
				 ...
				 * migration is complete. Also reset the
				 * context, as previous context is invalid
				 * after CPZ migration
				 */
				ctx = 0;
				result = -1;
				...
				/* UserPD in irrecoverable bad state. Exit
				 * listener */
				goto bail;
			}
			/* For any other error, retry once and exit if error
			 * seen again */
			if(me->adsp_listener1_handle != INVALID_HANDLE) {
				nErr = __QAIC_HEADER(adsp_listener1_next2)(
			...
			if(nErr) {
				FARF(RUNTIME_HIGH,
				     "Error 0x%x: %s response with result "
				     "0x%x for ctx 0x%x, handle "
				     "0x%x, sc 0x%x failed\n",
		...
		if(__builtin_smul_overflow(inBufsLenReq, 2, &bufs_len)) {
			FARF(ERROR,
			     "Error: %s: overflow occurred while multiplying "
			     "input buffer size: "
		...
		if(__builtin_smul_overflow(outBufsLen, 2, &bufs_len)) {
			FARF(ERROR,
			     "%s: Overflow occured while multiplying output "
			     "buffer size: %d * 2 "
			     "= %d",
		   ...
		   && outBufsCapacity > cache_size) {
			...
			if(NULL
			   == (buf = rpcmem_realloc(heapid, flags, outBufs,
				...
				result = AEE_ENORPCMEMORY;
				FARF(RUNTIME_RPC_HIGH,
				     "listener rpcmem_realloc shrink failed");
				goto invoke;
			}
			outBufs = buf;
			outBufsCapacity = size;
		...
		if(result && is_process_exiting(domain))
			result = AEE_EBADSTATE; // override result as process
			                        // is exiting
	} while(1);
bail:
	...
	if(nErr != AEE_SUCCESS) {
		if(!is_process_exiting(domain)) {
			FARF(ERROR,
			     "Error 0x%x: %s response with result 0x%x for "
			     "ctx 0x%x, handle 0x%x, "
			     "sc 0x%x failed : listener thread exited (errno "
			     "%s)",
			     nErr, __func__, result, ctx, handle, sc,
			     strerror(errno));
		}
	...
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x : Writing to listener event_fd %d "
		               "failed (errno %s)",
	   ...
	   != INVALID_HANDLE) {
		...
		if((nErr == DSP_AEE_EOFFSET + AEE_ERPC)
		   || nErr == DSP_AEE_EOFFSET + AEE_ENOSUCHMOD) {
			FARF(ERROR,
			     "Error 0x%x: %s domains support not available in "
			     "listener",
			     nErr, __func__);
...
listener_android_domain_init(int domain, int update_requested, sem_t *r_sem)
{
	...
	if(me->update_requested) {
		/*
		 * Semaphore initialized to 0. If main thread reaches wait
		 * first, then it will wait for listener to increment semaphore
		 * to 1. If listener posted semaphore first, then this wait
		 * will decrement semaphore to 0 and proceed.
		 */
		sem_wait(me->r_sem);
		VERIFY(AEE_SUCCESS == (nErr = me->params_updated));
	}
...
bail:
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error %x: listener android getevent file "
		               "descriptor failed "
		               "for domain %d\n",
		               nErr, domain);
	}
