...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_remotectl_open)(const char *name, uint32_t *handle,
                                                        ...
                                                        int *dlErr) __QAIC_IMPL_ATTRIBUTE
{
	...
	VERIFY(AEE_SUCCESS == (nErr = mod_table_open(name, handle, dlStr, dlerrorLen, dlErr)));
	VERIFY(AEE_SUCCESS
	       == (nErr = fastrpc_update_module_list(REVERSE_HANDLE_LIST_PREPEND, domain,
	                                             (remote_handle)*handle, &local, NULL)));
bail:
	return nErr;
}

__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_remotectl_close)(uint32_t handle, char *errStr,
                                                         int errStrLen,
                                                         int *dlErr) __QAIC_IMPL_ATTRIBUTE
{
	int domain = get_current_domain();
	...
	if (AEE_SUCCESS != (nErr = mod_table_close(handle, errStr, errStrLen, dlErr))) {
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = fastrpc_update_module_list(REVERSE_HANDLE_LIST_DEQUEUE, domain,
...
static __inline void *rpcmem_realloc(int heapid, uint32_t flags, void *buf, int oldsize,
                                     size_t size)
{
...
static void listener(listener_config *me)
{
	...
	if (eheap || eflags || emin) {
		FARF(RUNTIME_RPC_HIGH, "listener using ion heap: %d flags: %x cache: %lld\n",
	...
	invoke:
		...
		FARF(RUNTIME_RPC_HIGH, "%s responding 0x%x for ctx 0x%x, handle 0x%x, sc 0x%x",
		...
		if (me->adsp_listener1_handle != INVALID_HANDLE) {
			...
			nErr = __QAIC_HEADER(adsp_listener_next2)(ctx, result, outBufs, outBufsLen,
			                                          &ctx, &handle, &sc, inBufs,
			                                          inBufsLen, &inBufsLenReq);
		}
		if (nErr) {
			if (nErr == AEE_EINTERRUPTED) {
				/* UserPD in CPZ migration. Keep retrying until migration is
				 * complete. Also reset the context, as previous context is invalid
				 * after CPZ migration
				 */
				ctx = 0;
				result = -1;
		...
		if (inBufsLenReq > inBufsLen) {
			...
			if (AEE_SUCCESS != result) {
				FARF(RUNTIME_RPC_HIGH,
				     "adsp_listener_invoke_get_in_bufs2 failed  %x", result);
				goto invoke;
			}
			if (req > inBufsLen) {
				result = AEE_EBADPARM;
				FARF(RUNTIME_RPC_HIGH,
				     "adsp_listener_invoke_get_in_bufs2 failed, size is invalid "
				     "req %d "
		...
		if (ALIGNB(bufs_len) < outBufsCapacity && outBufsCapacity > cache_size) {
			...
			if (NULL
			    == (buf
			        = rpcmem_realloc(heapid, flags, outBufs, outBufsCapacity, size))) {
				result = AEE_ENORPCMEMORY;
...
bail:
	...
	if (nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x : Writing to listener event_fd %d failed (errno %s)",
...
int listener_android_init(void)
{
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = mod_table_register_const_handle(0, "apps_remotectl",
	                                                  apps_remotectl_skel_invoke)));
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = mod_table_register_static("adspmsgd_apps", adspmsgd_apps_skel_invoke)));
bail:
	if (nErr != AEE_SUCCESS) {
...
int listener_android_domain_init(int domain, int update_requested, sem_t *r_sem)
{
	...
	me->domain = domain;
	VERIFY(AEE_SUCCESS
	       == (nErr = pthread_create(&me->thread, 0, listener_start_thread, (void *)me)));
...
int close_reverse_handle(remote_handle64 h, char *dlerr, int dlerrorLen, int *dlErr)
{
