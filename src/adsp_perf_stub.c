...
static const Type types[2] = { { 0x8, { { (const uintptr_t)0, (const uintptr_t)1 } }, 2, 0x8 },
...
static const Parameter *const parameterArrays[5]
    = { (&(parameters[2])), (&(parameters[3])), (&(parameters[3])), (&(parameters[1])),
	(&(parameters[0])) };
static const Method methods[3] = { { REMOTE_SCALARS_MAKEX(0, 0, 0x1, 0x0, 0x0, 0x0), 0x4, 0x0, 1, 1,
...
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(adsp_perf_slim)
    = { 3, &(methodArrays[0]), 0, 0, &(methodStringsArrays[0]), methodStrings, strings };
...
__QAIC_STUB_EXPORT remote_handle _adsp_perf_handle(void)
{
	...
	if (adsp_pls_add_lookup) {
		if (0
		    == adsp_pls_add_lookup((uint32_t)_adsp_perf_handle, 0, sizeof(*ph),
		                           _adsp_perf_pls_ctor, "adsp_perf", _adsp_perf_pls_dtor,
		                           (void **)&ph)) {
			return *ph;
		}
		...
		if (0
		    == HAP_pls_add_lookup((uint32_t)_adsp_perf_handle, 0, sizeof(*ph),
		                          _adsp_perf_pls_ctor, "adsp_perf", _adsp_perf_pls_dtor,
		                          (void **)&ph)) {
			return *ph;
		}
...
uint32_t _adsp_perf_atomic_CompareAndExchange(uint32_t *volatile puDest, uint32_t uExchange,
                                              uint32_t uCompare);
...
static __inline int _stub_method_1(remote_handle _handle, uint32_t _mid, char *_rout0[1],
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_perf_get_usecs)(int64_t *dst,
...
static __inline int _stub_method_2(remote_handle _handle, uint32_t _mid, char *_rout0[1],
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_perf_get_keys)(char *keys, int keysLen, int *maxLen,
