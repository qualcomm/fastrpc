	...
	if(me->openMods) {
		...
		HASH_ITER(hh, me->openMods, dm, ftmp)
		{
			...
			FARF(RUNTIME_RPC_HIGH,
			     "%s: closed reverse module %s with handle 0x%x",
...
bail:
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF(
		    "Error %x: failed to register const handle in modtable\n",
		    nErr);
		if(dm) {
			free(dm);
			dm = NULL;
	...
	if(!sbuf_end(&buf) && sbuf_char(&buf, '&')) {
		...
		while(!sbuf_end(&buf)) {
			...
			if(sbuf_char(&buf, '=')) {
				...
				// value is until the next query that starts
				// with '&'
				VERIFY(sbuf_many1(&buf, notand));
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF(
		    "Error %x: parseuri failed for uri %s, urilen %d\n", nErr,
		    uri, urilen);
	}
	return nErr;
}
                         ...
                         const char *uri)
{
	...
	if(keyfound) {
		*handle = dmOld->key;
		dmOld->refs++;
		FARF(ALWAYS,
		     "%s: reverse module %s already found with handle 0x%x "
		     "(idx %u) refs %d",
	...
	if((dlErr = dm->dlhandle == 0 ? AEE_EINVHANDLE : 0)
	   && errno == ENOENT) {
		...
		for(ii = str_len - 1; ii >= 0; ii--) {
			...
			// Find index of last character before the extension
			// ".so" starts
			if(tmp[ii] == '.') {
				tmp[ii] = '\0';
				break;
...
bail:
	...
	if(nErr == AEE_SUCCESS) {
		FARF(ALWAYS,
		     "%s: dynamic reverse module %s opened with handle 0x%x "
		     "(idx %u)",
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		...
		FARF(RUNTIME_RPC_HIGH,
		     "%s: reverse module %s opened with handle 0x%x (idx %u)",
		     __func__, uri, *handle, handle_idx);
	}
	if(nErr && dm) {
		if(dm->h64) {
                    ...
                    int *pdlErr)
{
	...
	if(0 != open_mod_table_open_static_override(me, uri, handle)) {
		...
		if(dlErr != 0) {
			FARF(RUNTIME_RPC_HIGH,
			     "dynammic open failed, trying static");
                     ...
                     char *errStr, int errStrLen, int *pdlErr)
{
	...
	if(dm) {
		...
		if(dm->refs == 0) {
			...
			FARF(RUNTIME_RPC_HIGH,
			     "%s: pending ref: dm->refs %d, for uri: %s",
