...
static void open_mod_table_dtor_imp(void *data)
{
	...
	if (me->openMods) {
		...
		HASH_ITER(hh, me->openMods, dm, ftmp)
		{
			...
			FARF(RUNTIME_RPC_HIGH, "%s: closed reverse module %s with handle 0x%x",
...
static int open_mod_table_open_from_static(struct open_mod_table *me, struct static_mod **tbl,
                                           const char *uri, remote_handle *handle);

static int open_mod_table_open_static_override(struct open_mod_table *me, const char *uri,
...
static int open_mod_table_open_static(struct open_mod_table *me, const char *uri,
                                      remote_handle *handle)
{
                          ...
                          int (*handle_invoke)(remote_handle64, uint32_t sc, remote_arg *pra))
{
	...
	VERIFYC(NULL != (sm = ((struct static_mod *)calloc(1, sizeof(struct static_mod) + len))),
	        AEE_ENOMEMORY);
...
static int static_mod_table_register_static_override(struct static_mod_table *me, const char *uri,
                                                     int (*pfn)(uint32_t sc, remote_arg *pra))
{
	return static_mod_add(me, &me->staticModOverrides, uri, pfn, 0);
}
static int static_mod_table_register_static_override1(struct static_mod_table *me, const char *uri,
                                                      int (*pfn)(remote_handle64, uint32_t sc,
                                                                 remote_arg *pra))
{
	return static_mod_add(me, &me->staticModOverrides, uri, 0, pfn);
}
static int static_mod_table_register_static(struct static_mod_table *me, const char *uri,
                                            int (*pfn)(uint32_t sc, remote_arg *pra))
{
	return static_mod_add(me, &me->staticMods, uri, pfn, 0);
}
static int static_mod_table_register_static1(struct static_mod_table *me, const char *uri,
                                             int (*pfn)(remote_handle64, uint32_t sc,
                                                        remote_arg *pra))
{
                                                                       ...
                                                                       remote_arg *pra))
{
	...
	VERIFYC(NULL != (dm = ((struct const_mod *)calloc(1, sizeof(struct open_mod) + len))),
	        AEE_ENOMEMORY);
	dm->key = local;
	dm->invoke = invoke;
...
static int open_mod_handle_open(struct open_mod *mod, const char *name, remote_handle64 *ph)
{
	int nErr = AEE_SUCCESS;
	remote_arg args[3] = { 0 };
...
static inline int next_available_rev_handle(uint32_t *handle_idx)
{
	...
	if (nErr)
		FARF(ERROR, "Error 0x%x: %s: max number of reverse RPC handles (%u) open already",
		     nErr, __func__, MAX_REV_HANDLES);
	return nErr;
}

uint32_t is_reverse_handle_opened(struct open_mod_table *me, remote_handle *handle, const char *uri)
{
	int ii = 0;
	uint32_t keyfound = 0;
...
static int open_mod_table_open_dynamic(struct open_mod_table *me, const char *uri,
                                       ...
                                       int *pdlErr)
{
	...
	VERIFYC((REV_HANDLE_SIZE >= sizeof(struct open_mod) + len + 1), AEE_ENOMEMORY);
	...
	FARF(RUNTIME_RPC_HIGH, "vals %d %d %d", dm->vals.filelen, dm->vals.symlen, dm->vals.verlen);
	if (dm->vals.filelen) {
	...
	if (dm->vals.verlen && 0 == strncmp(dm->vals.ver, "1.0", dm->vals.verlen)) {
...
bail:
	...
	if (nErr == AEE_SUCCESS) {
		FARF(ALWAYS, "%s: dynamic reverse module %s opened with handle 0x%x (idx %u)",
		...
		VERIFY_EPRINTF("Error 0x%x: %s failed for %s, dlerr 0x%x", nErr, __func__, uri,
		               dlErr);
	}

	FARF(RUNTIME_RPC_HIGH, "done open_mod_table_open_dynamic for %s rv %x handle: %p %x", uri,
	     nErr, *handle, dlErr);
	if (tmp) {
		free(tmp);
		tmp = NULL;
...
static int open_mod_table_open(struct open_mod_table *me, const char *uri, remote_handle *handle,
                               char *dlerr, int dlerrorLen, int *pdlErr)
{
	int nErr = AEE_SUCCESS, dlErr = 0;
	if (pdlErr) {
		*pdlErr = 0;
	...
	if (0 != open_mod_table_open_static_override(me, uri, handle)) {
		VERIFY(AEE_SUCCESS
		       == (nErr = open_mod_table_open_dynamic(me, uri, handle, dlerr, dlerrorLen,
		                                              &dlErr)));
		if (dlErr != 0) {
...
bail:
	FARF(RUNTIME_RPC_HIGH, "done open for %s rv %d handle: %p", uri, nErr, *handle);
	if (nErr != AEE_SUCCESS) {
...
static void open_mod_close(struct open_mod_table *me, struct open_mod *dm)
{
	...
	if (dm->refs == 0) {
		...
		FARF(RUNTIME_RPC_HIGH, "%s : module %s has pending invokes ref count %d", __func__,
		     dm->uri, dm->refs);
		dm = 0;
	}
	RW_MUTEX_UNLOCK_WRITE(me->mut);
	if (dm) {
		...
		FARF(ALWAYS, "%s: closed reverse module %s with handle 0x%x", __func__, dm->uri,
		     (uint32_t)dm->key);
		dm->key = 0;
		dm = NULL;
	}
}
static int open_mod_table_close(struct open_mod_table *me, remote_handle64 handle, char *errStr,
                                int errStrLen, int *pdlErr)
{
	...
	if (del) {
		...
		FARF(ALWAYS, "%s: closed reverse module %s with handle 0x%x", __func__, del->uri,
		     (uint32_t)handle);
		del = NULL;
	}
bail:
...
static struct open_mod *open_mod_table_get_open(struct open_mod_table *me, remote_handle handle)
{
	...
	if (0 != om) {
		om->refs++;
		FARF(RUNTIME_RPC_HIGH, "%s: module %s, increament refs %d", __func__, om->uri,
		     om->refs);
	}
	RW_MUTEX_UNLOCK_WRITE(me->mut);
	return om;
...
static int open_mod_table_handle_invoke(struct open_mod_table *me, remote_handle handle,
                                        uint32_t sc, remote_arg *pra)
{
	...
	FASTRPC_ATRACE_BEGIN_L("%s called with handle 0x%x , scalar 0x%x", __func__, handle, sc);
	if (cm) {
		invoke = cm->invoke;
...
bail:
	...
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for handle:0x%x, sc:0x%x", nErr, __func__,
		     handle, sc);
	}
	FASTRPC_ATRACE_END();
	return nErr;
...
int mod_table_open(const char *uri, remote_handle *handle, char *dlerr, int dlerrorLen, int *pdlErr)
{
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = HAP_pls_add_lookup((uintptr_t)open_mod_table_ctor_imp, 0, sizeof(*pomt),
	                                     open_mod_table_ctor_imp, (void *)&static_mod_table_obj,
	                                     open_mod_table_dtor_imp, (void **)&pomt)));
	VERIFY(AEE_SUCCESS
	       == (nErr = open_mod_table_open(pomt, uri, handle, dlerr, dlerrorLen, pdlErr)));
bail:
...
int mod_table_close(remote_handle handle, char *errStr, int errStrLen, int *pdlErr)
{
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = HAP_pls_lookup((uintptr_t)open_mod_table_ctor_imp, 0, (void **)&pomt)));
	VERIFY(AEE_SUCCESS
	       == (nErr = open_mod_table_close(pomt, handle, errStr, errStrLen, pdlErr)));
bail:
	if (nErr != AEE_SUCCESS) {
...
{
	if (0 == static_mod_table_ctor(&static_mod_table_obj)) {
		return static_mod_table_register_const_handle(&static_mod_table_obj, remote, 0, uri,
		                                              pfn, 0);
	}
	return AEE_EUNKNOWN;
}
int mod_table_register_const_handle1(remote_handle remote, remote_handle64 local, const char *uri,
                                     int (*pfn)(remote_handle64, uint32_t sc, remote_arg *pra))
{
	if (0 == static_mod_table_ctor(&static_mod_table_obj)) {
...
static int mod_table_ctor(void) { return static_mod_table_ctor(&static_mod_table_obj); }
