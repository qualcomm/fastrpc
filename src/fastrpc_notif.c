...
int fastrpc_notif_domain_init(int domain)
{
	...
	me->thread = 0;
	VERIFY(AEE_SUCCESS
	       == (nErr = pthread_create(&me->thread, 0, notif_fastrpc_thread, (void *)me)));
...
int fastrpc_notif_register(int domain, struct remote_rpc_notif_register *notif)
{
	...
	VERIFYC(NULL != (lnotif = calloc(1, sizeof(struct fastrpc_notif))), AEE_ENOMEMORY);
...
int get_remote_notif_response(int domain)
{
	...
	FARF(ALWAYS, "%s: received status notification %u for domain %d, session %d", __func__,
	     status, dom, session);
	fastrpc_notify_status(dom, session, status);
bail:
	if (nErr && (errno != EBADF) && (nErr != AEE_EEXPIRED)) {
		FARF(ERROR, "Error 0x%x: %s failed to get notification response data errno %s",
