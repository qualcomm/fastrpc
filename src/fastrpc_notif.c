...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF(
		    "Error 0x%x: %s FastRPC notification worker thread exited "
...
fastrpc_notif_domain_deinit(int domain)
{
	...
	if(me->thread) {
		FARF(ALWAYS,
		     "%s: Waiting for FastRPC notification worker thread to "
		     "join",
		     __func__);
		me->deinit_started = 1;
...
bail:
	if(nErr && (errno != EBADF) && (nErr != AEE_EEXPIRED)) {
		FARF(ERROR,
		     "Error 0x%x: %s failed to get notification response data "
		     "errno %s",
