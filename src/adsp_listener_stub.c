...
static const Parameter *const parameterArrays[23]
	...
	(&(parameters[7])),  (&(parameters[0])), (&(parameters[1])), (&(parameters[8])),
	(&(parameters[3])),  (&(parameters[4])), (&(parameters[5])), (&(parameters[9])),
	(&(parameters[10])), (&(parameters[0])), (&(parameters[1])), (&(parameters[9])),
	(&(parameters[10])), (&(parameters[0])), (&(parameters[6])) };
...
static const Method *const methodArrays[6]
    = { &(methods[0]), &(methods[1]), &(methods[2]), &(methods[2]), &(methods[3]), &(methods[4]) };
...
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(adsp_listener_slim)
    = { 6, &(methodArrays[0]), 0, 0, &(methodStringsArrays[0]), methodStrings, strings };
...
__QAIC_STUB_EXPORT remote_handle _adsp_listener_handle(void)
{
	...
	if (adsp_pls_add_lookup) {
		if (0
		    == adsp_pls_add_lookup((uint32_t)_adsp_listener_handle, 0, sizeof(*ph),
		...
		if (0
		    == HAP_pls_add_lookup((uint32_t)_adsp_listener_handle, 0, sizeof(*ph),
		                          _adsp_listener_pls_ctor, "adsp_listener",
		                          _adsp_listener_pls_dtor, (void **)&ph)) {
			return *ph;
		}
...
uint32_t _adsp_listener_atomic_CompareAndExchange(uint32_t *volatile puDest, uint32_t uExchange,
                                                  uint32_t uCompare);
...
uint32_t _adsp_listener_atomic_CompareAndExchange(uint32_t *volatile puDest, uint32_t uExchange,
                                                  uint32_t uCompare)
{
	return (uint32_t)InterlockedCompareExchange((volatile LONG *)puDest, (LONG)uExchange,
	                                            (LONG)uCompare);
}
#elif __GNUC__
uint32_t _adsp_listener_atomic_CompareAndExchange(uint32_t *volatile puDest, uint32_t uExchange,
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_listener_next_invoke)(
    adsp_listener_invoke_ctx prevCtx, int prevResult, const adsp_listener_buffer *outBufs,
    int outBufsLen, adsp_listener_invoke_ctx *ctx, adsp_listener_remote_handle *handle,
    uint32_t *sc, adsp_listener_buffer *inBuffers, int inBuffersLen, int *inBufLenReq,
    int inBufLenReqLen, int *routBufLenReq, int routBufLenReqLen) __QAIC_STUB_ATTRIBUTE
{
	uint32_t _mid = 0;
	remote_handle _handle = _adsp_listener_handle();
	if (_handle != (remote_handle)-1) {
		return _stub_method(_handle, _mid, (uint32_t *)&prevCtx, (uint32_t *)&prevResult,
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_listener_next2)(
    adsp_listener_invoke_ctx prevCtx, int prevResult, const uint8_t *prevbufs, int prevbufsLen,
    adsp_listener_invoke_ctx *ctx, adsp_listener_remote_handle *handle, uint32_t *sc, uint8_t *bufs,
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adsp_listener_get_in_bufs2)(
    ...
    int *bufsLenReq) __QAIC_STUB_ATTRIBUTE
{
	...
	if (_handle != (remote_handle)-1) {
		return _stub_method_4(_handle, _mid, (uint32_t *)&ctx, (uint32_t *)&offset,
		                      (char **)&bufs, (uint32_t *)&bufsLen, (uint32_t *)bufsLenReq);
	} else {
		return AEE_EINVHANDLE;
	}
}
