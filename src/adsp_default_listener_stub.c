...
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(adsp_default_listener_slim)
    = { 1, &(methodArrays[0]), 0, 0, &(methodStringsArrays[0]), methodStrings, strings };
...
__QAIC_STUB_EXPORT remote_handle _adsp_default_listener_handle(void)
{
	...
	if (adsp_pls_add_lookup) {
		if (0
		    == adsp_pls_add_lookup((uint32_t)_adsp_default_listener_handle, 0, sizeof(*ph),
		...
		if (0
		    == HAP_pls_add_lookup((uint32_t)_adsp_default_listener_handle, 0, sizeof(*ph),
		                          _adsp_default_listener_pls_ctor, "adsp_default_listener",
		                          _adsp_default_listener_pls_dtor, (void **)&ph)) {
			return *ph;
		}
...
uint32_t _adsp_default_listener_atomic_CompareAndExchange(uint32_t *volatile puDest,
