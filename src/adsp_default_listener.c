...
fastrpc_wait_for_device(int domain)
{
	...
	if(watch_fd < 0) {
		close(inotify_fd);
		VERIFY_EPRINTF(
		    "Error: inotify_add_watch failed, invalid fd errno = %s\n",
		    strerror(errno));
		return AEE_EINVALIDFD;
	}
...
adsp_default_listener_start(int argc, char *argv[])
{
	...
	if(argc > 2) {
		 ...
		 * Example: ./adsprpcd "createstaticpd:audiopd&dom=adsp" (or)
		 * ./adsprpcd audiopd Get domain name from arguments and use
		 * domains API.
		 */
		...
		// If no arguments passed, default/rootpd daemon of remote
		// subsystem
