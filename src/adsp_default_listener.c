...
static domain_t supported_domains[]
    = { { ADSP_DOMAIN_ID, ADSP_DOMAIN },   { MDSP_DOMAIN_ID, MDSP_DOMAIN },
	{ SDSP_DOMAIN_ID, SDSP_DOMAIN },   { CDSP_DOMAIN_ID, CDSP_DOMAIN },
	{ CDSP1_DOMAIN_ID, CDSP1_DOMAIN }, { GDSP0_DOMAIN_ID, GDSP0_DOMAIN },
...
int adsp_default_listener_start(int argc, char *argv[])
{
	...
	if (argc > 2) {
		...
		VERIFYC(AEE_SUCCESS == (nErr = fastrpc_wait_for_device(domain_id)),
		        AEE_ECONNREFUSED);
		...
		if (!strncmp(argv[1], ROOTPD_NAME, strlen(argv[1]))) {
			...
			strlcat(name, argv[1],
			        strlen(ITRANSPORT_PREFIX CREATE_STATICPD) + strlen(argv[1]) + 1);
		}
	...
	VERIFYC(AEE_SUCCESS == (nErr = fastrpc_wait_for_device(DEFAULT_DOMAIN_ID)),
	        AEE_ECONNREFUSED);
...
start_listener:
	...
	if (domain_id != INVALID_DOMAIN_ID) {
		...
		nErr = adsp_default_listener1_open(adsp_default_listener1_URI_domain, &listener_fd);
...
start_poll:
	...
	if (domain_id != INVALID_DOMAIN_ID) {
		...
		VERIFYC(NULL != (eventfd_domain = (char *)malloc((eventfdlen) * sizeof(char))),
		        AEE_ENOMEMORY);
		nErr = snprintf(eventfd_domain, eventfdlen, "%s%s", ITRANSPORT_PREFIX "geteventfd",
		                dsp_domain->uri);
		if (nErr < 0) {
			VERIFY_EPRINTF("ERROR: %s: %d returned from snprintf\n", __func__, nErr);
			nErr = AEE_EFAILED;
		...
		VERIFY(0
		       == (nErr = remote_handle_open(ITRANSPORT_PREFIX "geteventfd",
