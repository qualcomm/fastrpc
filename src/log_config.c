...
static int parseLogConfig(int dom, unsigned int mask, char *filenames)
{
	...
	VERIFYC(NULL != (tempFiles = malloc(sizeof(char) * (strlen(filenames) + 1))),
	        AEE_ENOMEMORY);
	...
	if (handle != INVALID_HANDLE) {
		if (AEE_SUCCESS
		    != (nErr = adsp_current_process1_set_logging_params2(handle, mask, filesToLog,
		                                                         filesToLogLen))) {
			VERIFY(AEE_SUCCESS
		...
		if (AEE_SUCCESS
		    != (nErr = adsp_current_process_set_logging_params2(mask, filesToLog,
		                                                        filesToLogLen))) {
			VERIFY(AEE_SUCCESS
			       == (nErr = adsp_current_process_set_logging_params(mask, filesToLog,
			                                                          filesToLogLen)));
		}
	}
...
static int readLogConfigFromPath(int dom, const char *base, const char *file)
{
	...
	VERIFYM(len <= MAX_FARF_FILE_SIZE, AEE_ERPC, "len greater than %d for path %s (%s)\n", nErr,
	...
	if (farf_logmask == LLONG_MAX || farf_logmask == (uint64_t)LLONG_MIN || farf_logmask == 0) {
	...
	case 1:
		...
		if (handle != INVALID_HANDLE) {
			if (AEE_SUCCESS
			    != (nErr = adsp_current_process1_set_logging_params2(handle, mask, NULL,
			                                                         0))) {
				VERIFY(AEE_SUCCESS
				       == (nErr = adsp_current_process1_set_logging_params(
					       handle, mask, NULL, 0)));
	...
	for (i = 0; i < (int)log_config_watcher[dom].numPaths; ++i) {
		if (log_config_watcher[dom].wd[i] == event->wd) {
			if (strcmp(log_config_watcher[dom].fileToWatch, event->name) == 0) {
				return readLogConfigFromPath(dom,
				                             log_config_watcher[dom].paths[i].data,
				                             log_config_watcher[dom].fileToWatch);
			} else if (strcmp(log_config_watcher[dom].asidFileToWatch, event->name)
			           == 0) {
				return readLogConfigFromPath(
				    dom, log_config_watcher[dom].paths[i].data,
				    log_config_watcher[dom].asidFileToWatch);
			} else if (strcmp(log_config_watcher[dom].pidFileToWatch, event->name)
			           == 0) {
				return readLogConfigFromPath(
				    dom, log_config_watcher[dom].paths[i].data,
...
static int resetLogConfigFromEvent(int dom, struct inotify_event *event)
{
	...
	for (i = 0; i < (int)log_config_watcher[dom].numPaths; ++i) {
		if (log_config_watcher[dom].wd[i] == event->wd) {
			if ((strcmp(log_config_watcher[dom].fileToWatch, event->name) == 0)
			    || (strcmp(log_config_watcher[dom].asidFileToWatch, event->name) == 0)
			    || (strcmp(log_config_watcher[dom].pidFileToWatch, event->name) == 0)) {
				...
				handle = get_adsp_current_process1_handle(dom);
				if (handle != INVALID_HANDLE) {
					return adsp_current_process1_set_logging_params(handle, 0,
					                                                NULL, 0);
				} else {
...
static void *file_watcher_thread(void *arg)
{
	...
	for (i = 0; i < (int)log_config_watcher[dom].numPaths; ++i) {
		if (0
		    == readLogConfigFromPath(dom, log_config_watcher[dom].paths[i].data,
		                             log_config_watcher[dom].fileToWatch)) {
			file_found = 1;
				...
				if (event->len) {
					...
					if (log_config_watcher[dom].asidToWatch == -1) {
						...
						len = strlen(fileExtension)
						      + strlen(__TOSTR__(INT_MAX));
						VERIFYC(
						    NULL
							!= (log_config_watcher[dom].asidFileToWatch
						            = malloc(sizeof(char) * len)),
						    AEE_ENOMEMORY);
						snprintf(log_config_watcher[dom].asidFileToWatch,
						         len, "%d%s",
						         log_config_watcher[dom].asidToWatch,
						         fileExtension);
						VERIFY_IPRINTF(
						    "%s: Watching ASID file %s\n",
						    log_config_watcher[dom].fileToWatch,
						    log_config_watcher[dom].asidFileToWatch);
					}
					...
					if ((event->mask & IN_CREATE)
						...
						if (0 != readLogConfigFromEvent(dom, event)) {
							VERIFY_EPRINTF(
							    "Error : %s: Error reading config file "
							    "%s",
							    log_config_watcher[dom].fileToWatch,
							    log_config_watcher[dom].paths[i].data);
						}
	...
	if (log_config_watcher[dom].file_watcher_init_flag) {
		...
		if (0 <= log_config_watcher[dom].event_fd) {
			for (i = 0; i < RETRY_WRITE; i++) {
				...
				sz = write(log_config_watcher[dom].event_fd, &stop,
				           sizeof(uint64_t));
				if ((sz < (ssize_t)sizeof(uint64_t))
				    || (sz == -1 && errno == EAGAIN)) {
					VERIFY_WPRINTF(
					    "Warning: Written %zd bytes on event_fd %d for domain "
					    "%d (errno = %s): Retrying ...\n",
		...
		if (sz != sizeof(uint64_t) && 0 <= log_config_watcher[dom].event_fd) {
			VERIFY_EPRINTF("Error: Written %zd bytes on event_fd %d for domain %d: "
			               ...
			               sz, log_config_watcher[dom].event_fd, dom, strerror(errno));
...
int initFileWatcher(int dom)
{
	...
	VERIFYC(NULL != (log_config_watcher[dom].fileToWatch = malloc(sizeof(char) * len)),
	        AEE_ENOMEMORY);
	snprintf(log_config_watcher[dom].fileToWatch, len, "%s%s", name, fileExtension);

	len = strlen(fileExtension) + strlen(__TOSTR__(INT_MAX));
	VERIFYC(NULL != (log_config_watcher[dom].pidFileToWatch = malloc(sizeof(char) * len)),
	        AEE_ENOMEMORY);
	...
	VERIFY_IPRINTF("Opened file watcher fd %d eventfd %d for domain %d\n",
	               log_config_watcher[dom].fd, log_config_watcher[dom].event_fd, dom);
	...
	VERIFYC(NULL
	            != (log_config_watcher[dom].paths
	                = malloc(sizeof(_cstring1_t) * log_config_watcher[dom].numPaths)),
	        AEE_ENOMEMORY);
	VERIFYC(NULL
	            != (log_config_watcher[dom].wd
