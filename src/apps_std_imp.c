                     ...
                     struct apps_std_dir_info **pDirinfo)
{
	...
	if(match) {
		*pDirinfo = dirinfo;
		...
		nErr = ESTALE;
		VERIFY_EPRINTF("Error 0x%x: %s: stale directory handle 0x%llx "
		               "passed by DSP\n",
...
bail:
	if(nErr) {
		FREEIF(sinfo);
		VERIFY_EPRINTF(
		    "Error 0x%x: apps_std_FILE_alloc failed, errno %s \n",
		    nErr, strerror(nErr));
	}
...
apps_std_FILE_get(apps_std_FILE fd, struct apps_std_info **info)
{
	...
	pthread_mutex_unlock(&apps_std_mt);
	if(nErr) {
		VERIFY_EPRINTF("Error 0x%x: apps_std_FILE_get failed for fd "
		               "0x%x, errno %s \n",
...
bail:
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x: freopen for %s mode %s sin %x "
		               "failed. errno: %s\n",
		...
		if(out <= bufLen) {
			...
			if(0 == out
			   && (0 != (err = ferror(sinfo->u.stream)))) {
				nErr = ERRNO;
				VERIFY_EPRINTF("Error 0x%x: fread returning "
				               "%d bytes in %" PRIu64
				               " us, requested was %d "
				               "bytes, errno is %x\n",
		...
		*bEOF = false;
		if(out <= bufLen) {
			...
			if(0 == out
			   && (0 != (err = ferror(sinfo->u.stream)))) {
				nErr = ERRNO;
				VERIFY_EPRINTF("Error 0x%x: fwrite returning "
				               "%d bytes, requested was "
				               "%d bytes, errno is %x\n",
				               nErr, out, bufLen, err);
				return nErr;
			}
...
bail:
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF(
		    "Error 0x%x: clearerr failed for %x, errno is %s\n", nErr,
		    sin, strerror(nErr));
	}
	FASTRPC_ATRACE_END();
	return nErr;
                           ...
                           uint64_t *len) __QAIC_IMPL_ATTRIBUTE
{
	...
	if(sinfo->type == APPS_STD_STREAM_FILE) {
		...
		if(fd == -1) {
			nErr = ERRNO;
			VERIFY_EPRINTF(
			    "Error 0x%x: flen failed for %x, errno is %s\n",
...
__QAIC_IMPL(apps_std_unsetenv)(const char *name) __QAIC_IMPL_ATTRIBUTE
{
	...
	if(0 != unsetenv(name)) {
		nErr = ERRNO;
		VERIFY_EPRINTF(
		    "Error 0x%x: unsetenv failed for %s, errno is %s\n", nErr,
		    name, strerror(ERRNO));
		return nErr;
	}
	return AEE_SUCCESS;
    ...
    const char *mode, apps_std_FILE *psout) __QAIC_IMPL_ATTRIBUTE
{
	...
	while(dirList) {
		...
		// fallback: If not found in domain path /vendor/dsp/adsp try
		// in /vendor/dsp
		...
		if(AEE_SUCCESS == nErr) {
			...
			if(name != NULL
			   ...
			   && (strncmp(name, TESTSIG_FILE_NAME,
			       ...
			       != 0))
				FARF(ALWAYS, "Successfully opened file %s",
				     name);
			goto bail;
		}
		FREEIF(absName);
		...
		if(ERRNO != ENOENT
		   || (name != NULL
		       ...
		       && strncmp(name, TESTSIG_FILE_NAME,
		              ...
		              != 0))
			VERIFY_WPRINTF(
			    " Warning: %s failed with 0x%x for %s (%s)",
                                        ...
                                        int *len) __QAIC_IMPL_ATTRIBUTE
{
  ...
  if (tempName[0] == '\0') {
	...
	while(dirList) {
		...
		/* Do not Update nErr if error is no such file, as it may not
		 * be genuine error until we find in all path's.
		 */
		...
		if(ERRNO != ENOENT
		   || (name != NULL
		       ...
		       && strncmp(name, TESTSIG_FILE_NAME,
		              ...
		              != 0)) {
			if(errabsName) {
				VERIFY_WPRINTF(" Warning: %s failed with 0x%x "
				               "for path %s name %s (%s)",
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF(
		    "Error 0x%x: apps_std_get_search_paths_with_env failed\n",
		    nErr);
	}
		...
		*bEOF = false;
		if(!out) {
			...
			if(0 != (err = ferror(sinfo->u.stream))) {
				nErr = ERRNO;
				VERIFY_EPRINTF("Error 0x%x: fgets failed for "
				               "%x, errno is %s\n",
				               nErr, sin, strerror(ERRNO));
				goto bail;
			}
...
bail:
	if(nErr != AEE_SUCCESS || err) {
		FARF(RUNTIME_RPC_HIGH,
		     "Warniing 0x%x: fileExists failed for path %s, errno is "
		     "%s\n",
...
__QAIC_IMPL(apps_std_fsync)(apps_std_FILE sin) __QAIC_IMPL_ATTRIBUTE
{
	...
	if(sinfo->type == APPS_STD_STREAM_FILE) {
		...
		if(nErr != AEE_SUCCESS) {
			nErr = ERRNO;
			VERIFY_EPRINTF(
			    "Error 0x%x: apps_std fsync failed,errno is %s\n",
			    nErr, strerror(ERRNO));
		}
	} else {
		nErr = EBADF;
...
__QAIC_IMPL(apps_std_fremove)(const char *name) __QAIC_IMPL_ATTRIBUTE
{
	...
	if(nErr != AEE_SUCCESS) {
		nErr = ERRNO;
		VERIFY_EPRINTF(
		    "Error 0x%x: failed to remove file %s,errno is %s\n", nErr,
		    name, strerror(ERRNO));
	}
	FASTRPC_ATRACE_END();
	return nErr;
...
decrypt_int(char *fbuf, int size)
{
	...
	VERIFYM(NULL != (l_deinit = dlsym(handle, "license_manager_deinit")),
	        AEE_ERPC,
	        "Error: %s failed symbol license_manager_deinit not found err "
	        "0x%x "
	        "errno is %s",
	        __func__, nErr, strerror(ERRNO));
	VERIFYM(NULL != (l_decrypt = dlsym(handle, "license_manager_decrypt")),
	        AEE_ERPC,
	        "Error: %s failed symbol license_manager_decrypt not found "
	        "err 0x%x "
	        "errno is %s",
	        __func__, nErr, strerror(ERRNO));
...
bail:
	if(nErr) {
		VERIFY_EPRINTF(
		    "Error 0x%x: dlopen for licmgr failed. errno: %s\n", nErr,
		    dlerror());
	}
	if(handle) {
		dlclose(handle);
    ...
    __QAIC_IMPL_ATTRIBUTE
{
	...
	if(sinfo->type == APPS_STD_STREAM_FILE) {
		...
		VERIFYM(-1 != (fd = fileno(sinfo->u.stream)), AEE_EFILE,
		        "Error: %s failed file len is not proper err 0x%x "
		        "errno is %s",
		        __func__, nErr, strerror(ERRNO));
		...
		VERIFYC(0
		        ...
		        AEE_ENORPCMEMORY);
		VERIFYM(
		    0 == fseek(sinfo->u.stream, 0, SEEK_SET), AEE_EFILE,
		    "Error: %s failed as fseek failed err 0x%x errno is %s",
		    __func__, nErr, strerror(ERRNO));
		VERIFYM(
		    sz == (int)fread(fbuf, 1, sz, sinfo->u.stream), AEE_EFILE,
		    "Error: %s failed as fread failed err 0x%x errno is %s",
		    __func__, nErr, strerror(ERRNO));
...
bail:
	if(nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x: %s: failed to stat %s, file open "
		               "returned 0x%x (%s)\n",
