...
int apps_std_get_dirinfo(const apps_std_DIR *dir, struct apps_std_dir_info **pDirinfo)
{
	...
	if (match) {
		*pDirinfo = dirinfo;
		...
		nErr = ESTALE;
		VERIFY_EPRINTF("Error 0x%x: %s: stale directory handle 0x%llx passed by DSP\n",
...
static int apps_std_FILE_get(apps_std_FILE fd, struct apps_std_info **info)
{
	...
	pthread_mutex_unlock(&apps_std_mt);
	if (nErr) {
		VERIFY_EPRINTF("Error 0x%x: apps_std_FILE_get failed for fd 0x%x, errno %s \n",
...
static void apps_std_FILE_set_buffer_stream(struct apps_std_info *sinfo, char *fbuf, int flen,
                                            int pos)
{
	pthread_mutex_lock(&apps_std_mt);
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fopen)(const char *name, const char *mode,
...
bail:
	...
	FARF(RUNTIME_RPC_LOW, "Exiting %s name %s mode %s err %d", __func__, name, mode, nErr);
	FASTRPC_ATRACE_END();
	return nErr;
}

__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fopen_fd)(const char *name, const char *mode, int *fd,
                                                      int *len) __QAIC_IMPL_ATTRIBUTE
{
	...
	*len = statbuf.st_size;
	PROFILE_ALWAYS(&mmap_time,
	               nErr = fastrpc_mmap(domain, *fd, source, 0, *len, FASTRPC_MAP_FD));
...
bail:
	...
	FARF(RUNTIME_RPC_CRITICAL,
	     "%s: done for %s with fopen:%" PRIu64 "us, read:%" PRIu64 "us, rpc_alloc:%" PRIu64
	     "us, mmap:%" PRIu64 "us, fd 0x%x error_code 0x%x",
	     __func__, name, fopen_time, read_time, rpc_alloc_time, mmap_time, *fd, nErr);
	return nErr;
}
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_freopen)(apps_std_FILE sin, const char *name,
                                                     const char *mode,
                                                     apps_std_FILE *psout) __QAIC_IMPL_ATTRIBUTE
{
	int nErr = AEE_SUCCESS;
	...
	if (name) {
		FASTRPC_ATRACE_BEGIN_L("%s for %s (fd 0x%x) in %s mode", __func__, name, sin, mode);
	}
	FARF(RUNTIME_RPC_LOW, "Entering %s", __func__);
	errno = 0;
	VERIFYC(name != NULL, AEE_EBADPARM);
...
bail:
	if (nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x: freopen for %s mode %s sin %x failed. errno: %s\n",
		               nErr, name, mode, sin, strerror(nErr));
	}
	FARF(RUNTIME_RPC_LOW, "Exiting %s name %s mode %s sin %x err %d", __func__, name, mode, sin,
	     nErr);
	FASTRPC_ATRACE_END();
	return nErr;
}
...
bail:
	...
	FARF(RUNTIME_RPC_LOW, "Exiting %s sin %x err %d", __func__, sin, nErr);
	FASTRPC_ATRACE_END_L("%s fd 0x%x in %" PRIu64 " us error_code 0x%x ", __func__, sin, tdiff,
	                     nErr);
	return nErr;
}

__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fclose_fd)(int fd) __QAIC_IMPL_ATTRIBUTE
{
	int nErr = AEE_SUCCESS;
	int domain = get_current_domain();
	uint64_t tdiff = 0;
...
bail:
	...
	FASTRPC_ATRACE_END_L("%s fd 0x%x in %" PRIu64 " us error_code 0x%x", __func__, fd, tdiff,
	                     nErr);
	return nErr;
}
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fread)(apps_std_FILE sin, unsigned char *buf,
                                                   ...
                                                   int *bEOF) __QAIC_IMPL_ATTRIBUTE
{
	...
	FASTRPC_ATRACE_BEGIN_L("%s requested for %d bytes with fd 0x%x", __func__, bufLen, sin);
	FARF(RUNTIME_RPC_LOW, "Entering %s", __func__);
	errno = 0;
	VERIFY(0 == (nErr = apps_std_FILE_get(sin, &sinfo)));
	if (sinfo->type == APPS_STD_STREAM_FILE) {
		...
		*bytesRead = out;
		...
		unsigned int read = STD_MIN(bufLen, sinfo->u.binfo.flen - sinfo->u.binfo.pos);
...
bail:
	FARF(RUNTIME_RPC_LOW,
	     "Exiting %s returning %d bytes, requested was %d bytes for %x, err 0x%x", __func__,
	     out, bufLen, sin, nErr);
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fwrite)(apps_std_FILE sin, const unsigned char *buf,
                                                    int bufLen, int *bytesRead,
                                                    int *bEOF) __QAIC_IMPL_ATTRIBUTE
{
	int out = 0, nErr = AEE_SUCCESS;
		...
		*bEOF = false;
		if (out <= bufLen) {
			...
			if (0 == out && (0 != (err = ferror(sinfo->u.stream)))) {
				nErr = ERRNO;
				VERIFY_EPRINTF(
				    "Error 0x%x: fwrite returning %d bytes, requested was "
				    "%d bytes, errno is %x\n",
				    nErr, out, bufLen, err);
				return nErr;
			}
...
bail:
	FARF(RUNTIME_RPC_LOW,
	     "Exiting %s returning %d bytes, requested was %d bytes for %x, err %d", __func__, out,
	     bufLen, sin, nErr);
	FASTRPC_ATRACE_END();
	return nErr;
}

__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fgetpos)(apps_std_FILE sin, unsigned char *pos,
                                                     int posLen,
                                                     int *posLenReq) __QAIC_IMPL_ATTRIBUTE
{
	...
	FASTRPC_ATRACE_BEGIN_L("%s for file with fd 0x%x with posLen %d", __func__, sin, posLen);
	FARF(RUNTIME_RPC_LOW, "Entering %s", __func__);
	errno = 0;
	VERIFY(0 == (nErr = apps_std_FILE_get(sin, &sinfo)));
	if (sinfo->type == APPS_STD_STREAM_FILE) {
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fsetpos)(apps_std_FILE sin, const unsigned char *pos,
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_ftell)(apps_std_FILE sin,
                                                   int *pos) __QAIC_IMPL_ATTRIBUTE
{
	int nErr = AEE_SUCCESS;
	struct apps_std_info *sinfo = 0;
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fseek)(apps_std_FILE sin, int offset,
                                                   apps_std_SEEK whence) __QAIC_IMPL_ATTRIBUTE
{
	...
	FASTRPC_ATRACE_BEGIN_L("%s for file with fd 0x%x for op %d on offset %d", __func__, sin,
	                       whence, offset);
	...
	VERIFY(0 == (nErr = apps_std_FILE_get(sin, &sinfo)));
	if (sinfo->type == APPS_STD_STREAM_FILE) {
		PROFILE(&tdiff, VERIFYC(0 == fseek(sinfo->u.stream, offset, whence), ERRNO););
	} else {
		switch (op) {
...
bail:
	...
	FARF(RUNTIME_RPC_LOW, "Exiting %s for %x offset %d, err %d", __func__, sin, offset, nErr);
	FASTRPC_ATRACE_END_L("%s done for fd 0x%x, op %d on offset %d, time %" PRIu64 " us, err %d",
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_feof)(apps_std_FILE sin,
                                                  int *bEOF) __QAIC_IMPL_ATTRIBUTE
{
	int nErr = AEE_SUCCESS;
	struct apps_std_info *sinfo = 0;
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_ferror)(apps_std_FILE sin,
                                                    int *err) __QAIC_IMPL_ATTRIBUTE
{
	int nErr = AEE_SUCCESS;
	struct apps_std_info *sinfo = 0;
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_flen)(apps_std_FILE sin,
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_getenv)(const char *name, char *val, int valLen,
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_setenv)(const char *name, const char *val,
...
static int get_dirlist_from_env(const char *envvarname, char **ppDirList)
{
	...
	*envList = '\0';
	if (0 == apps_std_getenv(envvarname, envList, envLenGuess, &envListLen)) {
		if (strncmp(envvarname, ADSP_LIBRARY_PATH, strlen(ADSP_LIBRARY_PATH)) == 0
		    || strncmp(envvarname, DSP_LIBRARY_PATH, strlen(DSP_LIBRARY_PATH)) == 0) {
			...
			if (envLenGuess < envListPrependLen) {
				...
				envList = envListBuf;
				VERIFY(0
				       == (nErr = apps_std_getenv(envvarname, envList,
		...
		} else if (strncmp(envvarname, ADSP_AVS_PATH, strlen(ADSP_AVS_PATH)) == 0) {
	...
	} else if (strncmp(envvarname, ADSP_LIBRARY_PATH, strlen(ADSP_LIBRARY_PATH)) == 0
	           || strncmp(envvarname, DSP_LIBRARY_PATH, strlen(DSP_LIBRARY_PATH)) == 0) {
		envListLen = listLen = 1 + strlcpy(envListBuf, DSP_SEARCH_PATH, envLenGuess);
	} else if (strncmp(envvarname, ADSP_AVS_PATH, strlen(ADSP_AVS_PATH)) == 0) {
		envListLen = listLen = 1 + strlcpy(envListBuf, ADSP_AVS_CFG_PATH, envLenGuess);
	}
...
bail:
	...
	FARF(RUNTIME_RPC_LOW, "Exiting %s for %s, err %d", __func__, envvarname, nErr);
	return nErr;
}

__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fopen_with_env)(
    const char *envvarname, const char *delim, const char *name, const char *mode,
    apps_std_FILE *psout) __QAIC_IMPL_ATTRIBUTE
{
	...
	while (dirList) {
		...
		absNameLen = strlen(dirName) + strlen(name) + 2 + strlen("adsp") + 1;
		...
		if (AEE_SUCCESS == nErr) {
			// Success
			if (name != NULL
			    && (strncmp(name, OEM_CONFIG_FILE_NAME, strlen(OEM_CONFIG_FILE_NAME))
			        != 0)
	...
	if (nErr != AEE_SUCCESS) {
		if (ERRNO != ENOENT
		    || (name != NULL
		        ...
		        && strncmp(name, RPC_VERSION_FILE_NAME, strlen(RPC_VERSION_FILE_NAME)) != 0
		        && strncmp(name, TESTSIG_FILE_NAME, strlen(TESTSIG_FILE_NAME)) != 0))
			VERIFY_WPRINTF(" Warning: %s failed with 0x%x for %s (%s)", __func__, nErr,
			               name, strerror(ERRNO));
	}
	FARF(RUNTIME_RPC_LOW, "Exiting %s for %s envvarname %s mode %s delim %s, err %d", __func__,
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fopen_with_env_fd)(const char *envvarname,
...
bail:
	if (nErr != AEE_SUCCESS) {
		if (ERRNO != ENOENT
			...
			if (errabsName) {
				VERIFY_WPRINTF(
				    " Warning: %s failed with 0x%x for path %s name %s (%s)",
				    __func__, nErr, errabsName, name, strerror(ERRNO));
			} else {
				VERIFY_WPRINTF(" Warning: %s failed with 0x%x for %s (%s)",
...
__QAIC_HEADER_EXPORT int __QAIC_IMPL(apps_std_get_search_paths_with_env)(
    const char *envvarname, const char *delim, _cstring1_t *paths, int pathsLen, uint32_t *numPaths,
    uint16_t *maxPathLen) __QAIC_IMPL_ATTRIBUTE
{
	...
	*maxPathLen = 0;
	...
	while (path != NULL) {
		...
		VERIFYC(pathDomain = (char *)malloc(sizeof(char) * (pathDomainLen)), AEE_ENOMEMORY);
...
bail:
	...
	FARF(RUNTIME_RPC_LOW, "Exiting %s for envvarname %s delim %s, err %d", __func__, envvarname,
	     delim, nErr);
	return nErr;
}

__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fgets)(apps_std_FILE sin, unsigned char *buf,
                                                   int bufLen, int *bEOF) __QAIC_IMPL_ATTRIBUTE
{
	int nErr = AEE_SUCCESS;
	struct apps_std_info *sinfo = 0;

	FASTRPC_ATRACE_BEGIN_L("%s for file with fd 0x%x for buflen %d", __func__, sin, bufLen);
	VERIFY(0 == (nErr = apps_std_FILE_get(sin, &sinfo)));
	if (sinfo->type == APPS_STD_STREAM_FILE) {
...
__QAIC_HEADER_EXPORT int __QAIC_HEADER(apps_std_fileExists)(const char *path,
...
static int decrypt_int(char *fbuf, int size)
{
	...
	VERIFYC(NULL != (handle = dlopen("liblmclient.so", RTLD_NOW)), AEE_EINVHANDLE);
	...
	VERIFYM(NULL != (l_deinit = dlsym(handle, "license_manager_deinit")), AEE_ERPC,
	        "Error: %s failed symbol license_manager_deinit not found err 0x%x "
	        "errno is %s",
	        __func__, nErr, strerror(ERRNO));
	VERIFYM(NULL != (l_decrypt = dlsym(handle, "license_manager_decrypt")), AEE_ERPC,
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_fdopen_decrypt)(apps_std_FILE sin, apps_std_FILE *psout)
    __QAIC_IMPL_ATTRIBUTE
{
	...
	if (sinfo->type == APPS_STD_STREAM_FILE) {
		...
		VERIFYM(-1 != (fd = fileno(sinfo->u.stream)), AEE_EFILE,
		        "Error: %s failed file len is not proper err 0x%x errno is %s", __func__,
		        nErr, strerror(ERRNO));
		VERIFYM(0 == fstat(fd, &st_buf), AEE_EFILE,
		        "Error: %s failed file len is not proper err 0x%x errno is %s", __func__,
		        nErr, strerror(ERRNO));
		sz = (int)st_buf.st_size;
		VERIFYC(0 != (fbuf = rpcmem_alloc_internal(ION_HEAP_ID_QSEECOM, 1, (size_t)sz)),
		        AEE_ENORPCMEMORY);
		VERIFYM(0 == fseek(sinfo->u.stream, 0, SEEK_SET), AEE_EFILE,
		        "Error: %s failed as fseek failed err 0x%x errno is %s", __func__, nErr,
		        strerror(ERRNO));
		VERIFYM(sz == (int)fread(fbuf, 1, sz, sinfo->u.stream), AEE_EFILE,
		        "Error: %s failed as fread failed err 0x%x errno is %s", __func__, nErr,
		        strerror(ERRNO));
...
__QAIC_IMPL_EXPORT int __QAIC_HEADER(apps_std_opendir)(const char *name,
                                                       apps_std_DIR *dir) __QAIC_IMPL_ATTRIBUTE
{
	...
	if (odir != NULL) {
		...
		dirinfo = (struct apps_std_dir_info *)calloc(1, sizeof(struct apps_std_dir_info));
...
__QAIC_IMPL_EXPORT int __QAIC_HEADER(apps_std_readdir)(const apps_std_DIR *dir,
...
__QAIC_IMPL_EXPORT int __QAIC_HEADER(apps_std_mkdir)(const char *name,
                                                     int mode) __QAIC_IMPL_ATTRIBUTE
{
	int nErr = AEE_SUCCESS;
	uint64_t tdiff = 0;
	...
	FASTRPC_ATRACE_END_L("%s done for %s mode %d in %" PRIu64 " us error_code 0x%x", __func__,
	                     name, mode, tdiff, nErr);
	return nErr;
}
...
__QAIC_IMPL_EXPORT int __QAIC_HEADER(apps_std_stat)(const char *name,
                                                    apps_std_STAT *ist) __QAIC_IMPL_ATTRIBUTE
{
	...
	FASTRPC_ATRACE_BEGIN_L("%s for file %s", __func__, name);
	errno = 0;
	VERIFYM(0 == (nOpenErr = apps_std_fopen_with_env(ADSP_LIBRARY_PATH, ";", name, "r", &ps)),
	        AEE_EFILE, "Error: %s failed as fopen failed err 0x%x", __func__, nErr);
...
bail:
	if (nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x: %s: failed to stat %s, file open returned 0x%x (%s)\n",
...
__QAIC_HEADER_EXPORT int __QAIC_HEADER(apps_std_ftrunc)(apps_std_FILE sin,
                                                        int offset) __QAIC_HEADER_ATTRIBUTE
{
	int nErr = 0, fd = -1;
	struct apps_std_info *sinfo = 0;

	FASTRPC_ATRACE_BEGIN_L("%s for file with fd 0x%x for length %d", __func__, sin, offset);
	VERIFY(0 == (nErr = apps_std_FILE_get(sin, &sinfo)));
	errno = 0;
	VERIFYC(-1 != (fd = fileno(sinfo->u.stream)), ERRNO);
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_std_frename)(const char *oldname,
                                                     const char *newname) __QAIC_IMPL_ATTRIBUTE
{
	...
	if (NULL == oldname || NULL == newname)
		return EINVAL;
	FASTRPC_ATRACE_BEGIN_L("%s for file with oldname %s to new name %s", __func__, oldname,
	                       newname);
