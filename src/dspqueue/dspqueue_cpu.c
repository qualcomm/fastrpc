...
init_domain_queues_locked(int domain)
{
	...
	if((nErr
	   ...
	   != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s: obtaining session URI for %s on domain "
		     "%d, session "
		     "%u failed\n",
		     nErr, __func__, dspqueue_rpc_URI, domain,
		     dspqueue_skel.session_id);
		/* In case of failure due to SSR, return corresponding error to
		 * client */
	...
	if((nErr = dspqueue_rpc_open(dspqueue_skel.uri, &dq->dsp_handle))
	   != 0) {
		FARF(ERROR,
		     "dspqueue_rpc_open failed with %x on domain %d - packet "
		     "queue support "
	...
	if(!queues->notif_registered[domain]) {
		// Register for process exit notifications. Only do this once
		// for the lifetime of the process to avoid multiple
		// registrations and leaks.
		...
		if(nErr == AEE_EUNSUPPORTED) {
			FARF(ERROR,
			     "Warning 0x%x: %s: DSP doesn't support status "
			     "notification",
			     nErr, __func__);
			nErr = 0;
		} else if(!nErr) {
	...
	if((nErr != 0) || (cap == 0)) {
		...
		if((nErr != 0) || (cap == 0)) {
			FARF(HIGH, "dspqueue: No driver signaling support in "
			           "CPU driver");
		} else {
			FARF(HIGH,
			     "dspqueue: Optimized driver signaling supported");
			dq->have_dspsignal = 1;
		}
	}
	...
	*queue = NULL;
	...
	if((nErr == AEE_EUNSUPPORTED)
	   || (nErr == (int)(DSP_AEE_EOFFSET + AEE_EUNSUPPORTED))) {
		// OK, the DSP does pay attention to the version. It should
		// also support wait counts and optionally driver signaling.
		// Create the queue.
		FARF(HIGH,
		     "Initial queue create failed with %0x%x as expected",
		     nErr);
		...
		// Note that we expect the DSP will support both wait counts
		// and driver signaling or neither. However we can operate with
		// wait counts only in case we have an updated DSP image but an
		// older CPU kernel driver without driver signaling support.
	} else if(nErr != AEE_SUCCESS) {
		FARF(ERROR, "dspqueue_rpc_create_queue failed: 0x%x", nErr);
		goto bail;
	} else {
		FARF(HIGH,
		     "First-cut queue create succeeded unexpectedly? 0x%x",
		     nErr);
		// No new features available, including driver signaling
		if(q->have_driver_signaling) {
			unsigned i;
			FARF(HIGH, "Driver signaling not supported on DSP, "
			           "fall back to FastRPC "
			           "signaling");
			for(i = 0; i < signals; i++) {
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed (domain %d, flags 0x%x, sizes %u, "
		     "%u errno %s)",
...
dspqueue_close(dspqueue_t queue)
{
	...
	if(!dq->dsp_error) {
		...
		if(imported) {
			FARF(ERROR,
			     "Attempting to close queue 0x%p still open on "
			     "the DSP",
			     queue);
			nErr = AEE_EBADPARM;
			goto bail;
	...
	if(q->error_callback_thread) {
		...
		if(nErr == EDEADLK || nErr == EINVAL) {
			FARF(ERROR,
			     "Error %d: %s: Error callback thread join failed "
			     "for thread : %d",
	...
	if(q->packet_callback) {
		...
		if(nErr && nErr != ESRCH) {
			FARF(ERROR,
			     "Error: %s: packet callback thread for queue %p "
			     "joined with error %d",
			     __func__, q, nErr);
			goto bail;
		}
...
dspqueue_export(dspqueue_t queue, uint64_t *queue_id)
{
	...
	if(q->mdq.is_mdq) {
		FARF(ALWAYS,
		     "Warning: %s not supported for multi-domain queue, "
		     "already exported during create",
		     __func__);
	...
	if(q->have_driver_signaling) {
		...
		if(timeout) {
			...
			to_now = timespec_diff_us(
			    timeout, &now); // microseconds until expiry
			if(to_now == 0) {
				return AEE_EEXPIRED;
			}
		} else {
                           ...
                           uint32_t timeout_us, bool block)
{
	...
	for(unsigned int ii = 0; ii < mdq->num_domain_ids; ii++) {
		if(block) {
			/*
			 * Multi-domain blocking write operation will serially
			 * block on each individual queue.
			 */
			VERIFY(AEE_SUCCESS
			       == (nErr = dspqueue_write(
...
bail:
	...
	if(nErr) {
		 ...
		 * and it cannot be "erased". Client is expected to close
		 * queue.
		 */
		nErr = AEE_EBADSTATE;
		FARF(ERROR,
		     "Error 0x%x: %s (block %d): failed for queue %p, flags "
		     "0x%x, num bufs %u, msg len %u, timeout %u",
                       ...
                       uint32_t message_length, const uint8_t *message)
{
	...
	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_write_noblock' on individual
		// queues
	...
	if((qsize - w) < alen) {
		// Don't wrap the packet around queue end, but rather move it
		// to the beginning, replicating the header
		wrap = 1;
	...
	for(i = 0; i < num_buffers; i++) {
		...
		// Find buffer in internal FastRPC structures and handle
		// refcounts
		...
		if(nErr == AEE_ENOSUCHMAP) {
			FARF(ERROR,
			     "Buffer FD %d in queue message not mapped to "
			     "domain %d",
			     b->fd, q->domain);
			goto bail;
		}
		...
		// Ensure buffer offset and size are within the buffer as
		// mapped. Use mapped size if not specified by the client
		if(b->size != 0) {
			uint64_t bend
...
bail:
	...
	if(nErr != AEE_SUCCESS) {
		FARF(ERROR,
		     "Error 0x%x: %s failed for queue %p (flags 0x%x, "
		     "num_buffers %u, "
		     "message_length %u)",
                          ...
                          uint32_t timeout_us, bool read, bool block)
{
	...
	if(block && mdq->num_domain_ids > 1) {
		/*
		 * Blocked read cannot be supported on multi-domain queues
		 * because the read operation is done in a serial manner on the
		 * individual queues of each domain and the first queue with a
		 * valid packet is returned to client. Say if the first
		 * domain's queue does not have any response packets to consume
		 * but the second domain's queue does, then the read call will
		 * just block indefinitely on the first queue without ever
		 * checking the subsequence queues.
		 */
		nErr = AEE_EUNSUPPORTED;
		FARF(ALWAYS,
		     "Error 0x%x: %s: not supported for multi-domain queue %p",
		     nErr, __func__, q);
		return nErr;
	}
...
bail:
	...
	if(nErr && nErr != AEE_EWOULDBLOCK) {
		FARF(ALWAYS,
		     "Error 0x%x: %s (read %d, block %d): failed for queue "
		     "%p, max bufs %u, max msg len %u",
                      ...
                      uint32_t *message_length)
{
	...
	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_peek_noblock' on individual
		// queues
              ...
              uint32_t *message_length, uint32_t timeout_us)
{
	...
	if(q->mdq.is_mdq) {
		// Recursively call 'dspqueue_read_noblock' on individual
		// queues
		return dspqueue_multidomain_read(q, flags, 0, num_buffers,
		                                 NULL, 0, message_length, NULL,
                      ...
                      uint8_t *message)
{
	...
	if(f & DSPQUEUE_PACKET_FLAG_WAKEUP) {
		...
		if(qleft < 8) {
			...
			if(qleft < 8) {
				...
				FARF(LOW, "Didn't get packet after %uus",
				     (unsigned)(t2 - t1));
				pthread_mutex_unlock(&q->mutex);
				return AEE_EWOULDBLOCK;
			}
	...
	if((f & DSPQUEUE_PACKET_FLAG_BUFFERS) && (buffers != NULL)) {
		if(max_buffers < num_b) {
			FARF(ERROR, "Too many buffer references in packet to "
	...
	if((f & DSPQUEUE_PACKET_FLAG_MESSAGE) && (message != NULL)) {
		if(max_message_length < msg_l) {
			FARF(ERROR, "Message in packet too large to fit in "
			            "output buffer");
			nErr = AEE_EBADPARM;
			goto bail;
		}
	...
	if((buffers != NULL) && (num_b > 0)) {
		for(i = 0; i < num_b; i++) {
			...
			if(nErr == AEE_ENOSUCHMAP) {
				FARF(ERROR,
				     "Buffer FD %d in queue message not "
				     "mapped to domain %d",
				     b->fd, q->domain);
				goto bail;
			}
			...
			// Check and use offset and size from the packet if
			// specified
			if(b->size != 0) {
	...
	for(i = 0; i <= dq->max_queue; i++) {
		if((dq->queues[i] != UNUSED_QUEUE)
		   && (dq->queues[i] != INVALID_QUEUE)) {
			...
			if(q->have_driver_signaling) {
				int s;
				FARF(HIGH, "%s: Cancel all signal waits",
				     __func__);
...
dspqueue_receive_signal_thread(void *arg)
{
	...
	while(1) {
		...
		// Got a signal - at least one queue has more packets or space.
		// Find out which one and signal it.
		FARF(LOW, "Got signal");
...
dspqueue_packet_callback_thread(void *arg)
{
	...
	while(1) {
		...
		// Call the callback if we have any packets we haven't seen
		// yet.
		cache_invalidate_word(&write_state->packet_count);
		...
		// Mark we're waiting and call again if we just got more
		// packets
		if(q->have_wait_counts) {
			atomic_fetch_add(wait_count, 1);
			cache_flush_word(wait_count);
		...
		if(nErr == AEE_EINTERRUPTED || nErr == AEE_EBADSTATE) {
			...
			pthread_mutex_unlock(&q->packet_mutex);
			FARF(ERROR,
			     "Error: %s: wait_signal failed with 0x%x (queue "
			     "%p)",
			     __func__, nErr, q);
                        ...
                        remote_rpc_status_flags_t status)
{
	...
	if(session && domain < NUM_DOMAINS) {
		// Did not receive effective domain ID for extended session.
		// Compute it.
