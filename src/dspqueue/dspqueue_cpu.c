...
struct dspqueue_domain_queues {
	int domain;
	unsigned num_queues;
	pthread_mutex_t queue_list_mutex; // Hold this to manipulate queues[] or max_queue
	unsigned max_queue;
...
#define CACHE_ALIGN_SIZE(x) ((x + (QUEUE_CACHE_ALIGN - 1)) & (~(QUEUE_CACHE_ALIGN - 1)))
...
static AEEResult init_domain_queues_locked(int domain)
{
	...
	/* Get URI of session */
	dspqueue_skel.domain_name_len = (dom == CDSP1_DOMAIN_ID) ? strlen(CDSP1_DOMAIN_NAME) + 1
	...
	if (dom == CDSP_DOMAIN_ID) {
		strlcpy(dspqueue_skel.domain_name, CDSP_DOMAIN_NAME, dspqueue_skel.domain_name_len);
	} else if (dom == ADSP_DOMAIN_ID) {
		strlcpy(dspqueue_skel.domain_name, ADSP_DOMAIN_NAME, dspqueue_skel.domain_name_len);
	...
	VERIFYC(
	    (dspqueue_skel.module_uri = (char *)calloc(dspqueue_skel.module_uri_len, sizeof(char)))
		!= NULL,
	    AEE_ENOMEMORY);
	strlcpy(dspqueue_skel.module_uri, dspqueue_rpc_URI, dspqueue_skel.module_uri_len);
	...
	VERIFYC((dspqueue_skel.uri = (char *)calloc(dspqueue_skel.uri_len, sizeof(char))) != NULL,
	        AEE_ENOMEMORY);
	...
	if (!queues->notif_registered[domain]) {
		...
		nErr = remote_session_control(FASTRPC_REGISTER_STATUS_NOTIFICATIONS, (void *)&reg,
		                              sizeof(reg));
	...
	VERIFYC((dq->state = rpcmem_alloc_internal(RPCMEM_HEAP_ID_SYSTEM,
	...
	VERIFY((nErr = fastrpc_mmap(domain, dq->state_fd, dq->state, 0,
	                            sizeof(struct dspqueue_process_queue_state), FASTRPC_MAP_FD))
	       == 0);
	state_mapped = 1;
	VERIFY((nErr = dspqueue_rpc_init_process_state(dq->dsp_handle, dq->state_fd)) == 0);
...
bail:
	...
	free_skel_uri(&dspqueue_skel);
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed (domain %d) errno %s", nErr, __func__, domain,
		     strerror(errno));
	}
	return nErr;
}
...
static AEEResult destroy_domain_queues_locked(int domain)
{
	...
	if (dq->dsp_error) {
		...
		VERIFY((nErr = fastrpc_munmap(dq->domain, dq->state_fd, dq->state,
		                              sizeof(struct dspqueue_process_queue_state)))
		       == 0);
	}
                          ...
                          dspqueue_t *queue)
{
	...
	*queue = NULL;
	...
	VERIFY((nErr = fastrpc_mmap(domain, q->user_queue_fd, q->user_queue, 0, q->user_queue_size,
	                            FASTRPC_MAP_FD))
	       == 0);
	queue_mapped = 1;
	...
	nErr = dspqueue_rpc_create_queue(dq->dsp_handle, q->id, q->user_queue_fd, queue_count,
	                                 &q->dsp_id);
	if ((nErr == AEE_EUNSUPPORTED) || (nErr == (int)(DSP_AEE_EOFFSET + AEE_EUNSUPPORTED))) {
		...
		VERIFY((nErr = dspqueue_rpc_create_queue(dq->dsp_handle, q->id, q->user_queue_fd,
		...
		if (q->have_driver_signaling) {
			...
			FARF(HIGH, "Driver signaling not supported on DSP, fall back to FastRPC "
			           "signaling");
			for (i = 0; i < signals; i++) {
				VERIFY((nErr = dspsignal_destroy(domain, QUEUE_SIGNAL(id, i)))
				       == 0);
			}
			signals = 0;
		}
	...
	*queue = q;
	...
	dq->queues[id] = q;
	pthread_mutex_unlock(&dq->queue_list_mutex);
	FARF(ALWAYS, "%s: created Queue %u, %p, DSP 0x%08x for domain %d", __func__, q->id, q,
	     (unsigned)q->dsp_id, q->domain);
...
bail:
	if (q) {
		if (callbackthread) {
			if (q->have_driver_signaling) {
				dspsignal_cancel_wait(
				    domain, QUEUE_SIGNAL(id, DSPQUEUE_SIGNAL_RESP_PACKET));
			} else {
		...
		if (queue_mapped) {
			fastrpc_munmap(domain, q->user_queue_fd, q->user_queue, q->user_queue_size);
		}
		if (q->user_queue) {
	...
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed (domain %d, flags 0x%x, sizes %u, %u errno %s)",
...
static int dspqueue_multidomain_close(struct dspqueue *q, bool queue_mut)
{
	...
	FARF(ALWAYS, "%s: closed queue %p (ctx 0x%" PRIx64 "), num domains %u", __func__, q,
	     mdq->ctx, mdq->num_domain_ids);
	free(q);
bail:
	if (nErr) {
...
AEEResult dspqueue_close(dspqueue_t queue)
{
	...
	if (!dq->dsp_error) {
		VERIFY((nErr = dspqueue_rpc_is_imported(dq->dsp_handle, q->dsp_id, &imported))
		       == 0);
		if (imported) {
			FARF(ERROR, "Attempting to close queue 0x%p still open on the DSP", queue);
			nErr = AEE_EBADPARM;
			goto bail;
		}
	}
	...
	if (dq->dsp_error) {
		...
		dspqueue_rpc_destroy_queue(dq->dsp_handle, q->dsp_id);
		fastrpc_munmap(dq->domain, q->user_queue_fd, q->user_queue, q->user_queue_size);
	} else {
	...
	pthread_mutex_unlock(&queues->mutex);
	FARF(ALWAYS, "%s: closed Queue %u, %p, DSP 0x%08x for domain %d", __func__, q->id, q,
	     (unsigned)q->dsp_id, q->domain);
	free(q);

bail:
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed errno %s", nErr, __func__, strerror(errno));
	}
	return nErr;
}
...
AEEResult dspqueue_export(dspqueue_t queue, uint64_t *queue_id)
{
	...
	if (q->mdq.is_mdq) {
		FARF(ALWAYS,
		     "Warning: %s not supported for multi-domain queue, already exported during "
		     "create",
		     __func__);
...
static int dspqueue_multidomain_create(dspqueue_create_req *create)
{
	...
	VERIFY(AEE_SUCCESS
	       == (nErr
	           = fastrpc_context_get_domains(create->ctx, &effec_domain_ids, &num_domain_ids)));
	...
	VERIFYC(AEE_SUCCESS == (nErr = pthread_once(&queues_once, init_process_queues_once)),
	        AEE_ENOTINITIALIZED);
	...
	VERIFYC(NULL != (mdq->queues = calloc(num_domain_ids, sizeof(*(mdq->queues)))),
	        AEE_ENOMEMORY);
	...
	for (unsigned int ii = 0; ii < num_domain_ids; ii++) {
		VERIFY(AEE_SUCCESS
		       == (nErr = dspqueue_create(effec_domain_ids[ii], create->flags,
		...
		VERIFY(AEE_SUCCESS == (nErr = dspqueue_export(mdq->queues[ii], &mdq->dsp_ids[ii])));
	}
	...
	FARF(ALWAYS,
	     "%s: created queue %p for ctx 0x%" PRIx64 ", sizes: req %u, rsp %u, num domains %u",
	     __func__, q, create->ctx, create->req_queue_size, create->resp_queue_size,
	     num_domain_ids);
bail:
	if (nErr) {
		FARF(ERROR,
		     ...
		     nErr, __func__, create->ctx, create->req_queue_size, create->resp_queue_size,
		     create->num_ids);
...
static void get_queue_state_write(void *memory, struct dspqueue_packet_queue_header *pq,
                                  uint32_t *space_left, uint32_t *read_pos, uint32_t *write_pos)
{

	struct dspqueue_packet_queue_state *read_state
	    = (struct dspqueue_packet_queue_state *)(((uintptr_t)memory) + pq->read_state_offset);
	struct dspqueue_packet_queue_state *write_state
	    = (struct dspqueue_packet_queue_state *)(((uintptr_t)memory) + pq->write_state_offset);
...
static void get_queue_state_read(void *memory, struct dspqueue_packet_queue_header *pq,
                                 uint32_t *data_left, uint32_t *read_pos, uint32_t *write_pos)
{
...
static inline uint32_t write_64(volatile uint8_t *packet_queue, uint32_t write_pos,
...
static inline uint32_t write_data(volatile uint8_t *packet_queue, uint32_t write_pos,
...
static AEEResult send_signal(struct dspqueue *q, uint32_t signal_no)
{
	...
	if (q->have_driver_signaling) {
		...
		VERIFY((nErr = dspsignal_signal(q->domain, QUEUE_SIGNAL(q->id, signal_no))) == 0);
	} else {
		pthread_mutex_lock(&dq->send_signal_mutex);
...
bail:
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for queue %p signal %u", nErr, __func__, q,
		     (unsigned)signal_no);
	}
	return nErr;
}
                                    ...
                                    struct timespec *timeout)
{
	...
	if (q->have_driver_signaling) {
		...
		VERIFY((nErr = dspsignal_wait(q->domain, QUEUE_SIGNAL(q->id, signal_no), to_now))
		       == 0);
...
static int dspqueue_multidomain_write(struct dspqueue *q, uint32_t flags, uint32_t num_buffers,
                                      struct dspqueue_buffer *buffers, uint32_t message_length,
                                      const uint8_t *message, uint32_t timeout_us, bool block)
{
	int nErr = AEE_SUCCESS;
	struct dspqueue_multidomain *mdq = NULL;
	bool locked = false;
	...
	for (unsigned int ii = 0; ii < mdq->num_domain_ids; ii++) {
		if (block) {
			...
			VERIFY(
			    AEE_SUCCESS
			    == (nErr = dspqueue_write(mdq->queues[ii], flags, num_buffers, buffers,
...
bail:
	...
	if (nErr) {
		...
		nErr = AEE_EBADSTATE;
		FARF(ERROR,
		     "Error 0x%x: %s (block %d): failed for queue %p, flags 0x%x, num bufs %u, msg "
		     "len %u, timeout %u",
		     nErr, __func__, block, q, flags, num_buffers, message_length, timeout_us);
	}
	return nErr;
}

AEEResult dspqueue_write_noblock(dspqueue_t queue, uint32_t flags, uint32_t num_buffers,
                                 struct dspqueue_buffer *buffers, uint32_t message_length,
                                 const uint8_t *message)
{
	...
	if (q->mdq.is_mdq) {
		...
		return dspqueue_multidomain_write(q, flags, num_buffers, buffers, message_length,
		                                  message, 0, false);
	}
	...
	for (i = 0; i < num_buffers; i++) {
		...
		if (nErr == AEE_ENOSUCHMAP) {
			FARF(ERROR, "Buffer FD %d in queue message not mapped to domain %d", b->fd,
			     q->domain);
			goto bail;
		}
		VERIFY(nErr == 0);
	...
	phdr
	    = (((uint64_t)(len & 0xffffffff)) | (((uint64_t)(flags & 0xffff)) << 32)
	       | (((uint64_t)(num_buffers & 0xff)) << 48) | (((uint64_t)(q->seq_no & 0xff)) << 56));
	...
	if (num_buffers > 0) {
		w = write_data(qp, w, qsize, buffers, num_buffers * sizeof(struct dspqueue_buffer));
	}
	...
	FARF(LOW, "Queue %u req_packet_count %u", (unsigned)q->id, (unsigned)q->req_packet_count);
                         ...
                         const uint8_t *message, uint32_t timeout_us)
{
	...
	nErr = dspqueue_write_noblock(queue, flags, num_buffers, buffers, message_length, message);
	...
	if (q->have_wait_counts) {
		...
		waiting = 1;
		nErr = dspqueue_write_noblock(queue, flags, num_buffers, buffers, message_length,
		                              message);
		if (nErr != AEE_EWOULDBLOCK) {
			goto bail;
		}
	...
	while (1) {
		...
		VERIFY((nErr = wait_signal_locked(q, DSPQUEUE_SIGNAL_REQ_SPACE, timeout_ts)) == 0);
...
AEEResult dspqueue_write_early_wakeup_noblock(dspqueue_t queue, uint32_t wakeup_delay,
                                              uint32_t packet_flags)
{
	...
	if (q->mdq.is_mdq) {
		// Recursively call 'dspqueue_write_noblock' on individual queues
		return dspqueue_multidomain_write(q, flags, 0, NULL, msg_len, msg, 0, false);
	}
	return dspqueue_write_noblock(queue, flags, 0, NULL, msg_len, msg);
}

static AEEResult peek_locked(volatile const uint8_t *qp, uint32_t r, uint32_t *flags,
                             uint32_t *num_buffers, uint32_t *message_length, uint64_t *raw_header)
{
 ...
 */
static int dspqueue_multidomain_read(struct dspqueue *q, uint32_t *flags, uint32_t max_buffers,
                                     ...
                                     uint8_t *message, uint32_t timeout_us, bool read, bool block)
{
	...
	if (block && mdq->num_domain_ids > 1) {
		 ...
		 */
		nErr = AEE_EUNSUPPORTED;
		FARF(ALWAYS, "Error 0x%x: %s: not supported for multi-domain queue %p", nErr,
		     __func__, q);
		return nErr;
	}
	...
	for (unsigned int ii = 0; ii < mdq->num_domain_ids; ii++) {
		...
		if (read) {
			if (block) {
				...
				nErr = dspqueue_read_noblock(cq, flags, max_buffers, num_buffers,
				                             buffers, max_message_length,
				                             message_length, message);
			}
		} else {
			if (block) {
				nErr = dspqueue_peek(cq, flags, num_buffers, message_length,
				                     timeout_us);
			} else {
				nErr
				    = dspqueue_peek_noblock(cq, flags, num_buffers, message_length);
			}
		}
		if (!nErr) {
...
bail:
	...
	if (nErr && nErr != AEE_EWOULDBLOCK) {
		FARF(ALWAYS,
		     "Error 0x%x: %s (read %d, block %d): failed for queue %p, max bufs %u, max "
		     "msg len %u",
                                ...
                                uint32_t *message_length)
{
	...
	if (q->mdq.is_mdq) {
		...
		return dspqueue_multidomain_read(q, flags, 0, num_buffers, NULL, 0, message_length,
		                                 NULL, 0, false, false);
	}
	...
	VERIFY((nErr = peek_locked(qp, r, flags, num_buffers, message_length, NULL)) == 0);
                        ...
                        uint32_t *message_length, uint32_t timeout_us)
{
	...
	while (1) {
		...
		VERIFY((nErr = wait_signal_locked(q, DSPQUEUE_SIGNAL_RESP_PACKET, timeout_ts))
		       == 0);
...
static inline uint32_t read_data(volatile const uint8_t *packet_queue, uint32_t read_pos,
...
AEEResult dspqueue_read_noblock(dspqueue_t queue, uint32_t *flags, uint32_t max_buffers,
                                uint32_t *num_buffers, struct dspqueue_buffer *buffers,
                                uint32_t max_message_length, uint32_t *message_length,
                                uint8_t *message)
{
	...
	if (q->mdq.is_mdq) {
		// Recursively call 'dspqueue_read_noblock' on individual queues
		return dspqueue_multidomain_read(q, flags, max_buffers, num_buffers, buffers,
	...
	if ((f & DSPQUEUE_PACKET_FLAG_BUFFERS) && (buffers != NULL)) {
		if (max_buffers < num_b) {
			FARF(ERROR, "Too many buffer references in packet to fit in output buffer");
			nErr = AEE_EBADPARM;
			goto bail;
		}
	}
	...
	q->resp_space_count++;
	dq->state->resp_space_count[q->id] = q->resp_space_count;
	FARF(LOW, "Queue %u resp_space_count %u", (unsigned)q->id, (unsigned)q->resp_space_count);
	cache_flush_word(&dq->state->resp_space_count[q->id]);
	if (q->have_wait_counts) {
		// Only signal if the other end is potentially waiting
		cache_invalidate_word(&write_state->wait_count);
...
bail:
	...
	if (nErr != AEE_SUCCESS) {
		FARF(ERROR, "Error 0x%x: %s failed for queue %p errno %s", nErr, __func__, queue,
		     strerror(errno));
	}
	return nErr;
}
...
static int dspqueue_multidomain_get_stat(struct dspqueue *q, enum dspqueue_stat stat,
                                         uint64_t *value)
{
	int nErr = AEE_SUCCESS;
	struct dspqueue_multidomain *mdq = NULL;
	 ...
	 */
	VERIFY(AEE_SUCCESS == (nErr = dspqueue_get_stat(mdq->queues[0], stat, value)));
bail:
	if (locked)
		pthread_mutex_unlock(&q->mutex);

	if (nErr) {
		FARF(ALWAYS, "Error 0x%x: %s: failed for queue %p, stat %d", nErr, __func__, q,
		     stat);
	}
	return nErr;
}
...
static void *error_callback_thread(void *arg)
{
	...
	FARF(ALWAYS, "%s starting for queue %p with id %u", __func__, a->queue, a->queue->id);
		...
		if ((dq->queues[i] != UNUSED_QUEUE) && (dq->queues[i] != INVALID_QUEUE)) {
			...
			if (q->error_callback != NULL) {
				...
				if (a != NULL) {
					...
					if (err != 0) {
						FARF(ERROR,
						     "Error callback thread creation failed: %d",
						     err);
						free(a);
					}
				} else {
...
bail:
	FARF(ERROR, "dspqueue_send_signal_thread failed with %d errno %s", nErr, strerror(errno));
...
static void *dspqueue_receive_signal_thread(void *arg)
{
	...
	while (1) {
		...
		cache_invalidate(dq->state->req_space_count, sizeof(dq->state->req_space_count));
...
static void *dspqueue_packet_callback_thread(void *arg)
{
	...
	while (1) {
		...
		if (nErr == AEE_EINTERRUPTED || nErr == AEE_EBADSTATE) {
			...
			pthread_mutex_unlock(&q->packet_mutex);
			FARF(ERROR, "Error: %s: wait_signal failed with 0x%x (queue %p)", __func__,
			     nErr, q);
                                   ...
                                   remote_rpc_status_flags_t status)
{
	...
	FARF(ALWAYS, "%s for domain %d, session %d, status %u", __func__, domain, session, status);
