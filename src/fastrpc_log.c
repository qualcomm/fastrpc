...
void HAP_debug_runtime(int level, const char *file, int line, const char *format, ...)
{
	...
	if (((1 << level) & (fastrpc_logmask))
		...
		snprintf(log, MAX_FARF_LEN, "%d:%d:%s:%s:%d: %s", getpid(), gettid(), __progname,
		         file, line, buf);
	}
...
{
#ifdef __ANDROID__
	__android_log_print(hap_2_android_log_level(level), __progname, "%s:%d: %s", filename, line,
	                    msg);
#elif defined(USE_SYSLOG)
	...
	if (newlines)
		printf("ADSPRPC: %d %d %c %s: %s:%d: %s", getpid(), gettid(),
		       android_log_level_to_char(level), __progname, short_filename, line, msg);
	else
