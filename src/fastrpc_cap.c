...
const char *RPROC_SUBSYSTEM_NAME[]
    = { "adsp", "mss", "spss", "cdsp", "cdsp1", "gdsp0", "gdsp1", "reserved" };
...
static inline uint32_t fastrpc_check_if_dsp_present_pil(uint32_t domain)
{
	...
	const char *SUBSYSTEM_DEV_NAME[] = { "/dev/subsys_adsp",  "",
		                             "/dev/subsys_slpi",  "/dev/subsys_cdsp",
		                             "/dev/subsys_cdsp1", "/dev/subsys_gdsp0",
		                             "/dev/subsys_gdsp1", "reserved",
		                             "reserved",          "reserved",
		                             "reserved",          "reserved",
		                             "reserved",          "reserved",
		                             "reserved",          "reserved" };
...
int fastrpc_get_cap(uint32_t domain, uint32_t attributeID, uint32_t *capability)
{
	...
	VERIFYC(attributeID < FASTRPC_MAX_ATTRIBUTES,
	        AEE_EBADPARM); // Check if the attribute ID is less than max attributes accepted by
	                       // userspace
...
bail:
	...
	if (nErr) {
		FARF(ERROR, "Warning 0x%x: %s failed to get attribute %u for domain %u (errno %s)",
