...
static const Type types[2] = { { 0x1, { { (const uintptr_t)0, (const uintptr_t)1 } }, 2, 0x1 },
...
static const Parameter *const parameterArrays[11]
    = { (&(parameters[0])), (&(parameters[1])), (&(parameters[2])), (&(parameters[1])),
	(&(parameters[3])), (&(parameters[2])), (&(parameters[1])), (&(parameters[3])),
	(&(parameters[5])), (&(parameters[4])), (&(parameters[4])) };
static const Method methods[4] = { { REMOTE_SCALARS_MAKEX(0, 0, 0x2, 0x2, 0x0, 0x0), 0x8, 0x8, 6, 4,
...
static const char strings[84]
    = "set_param\0grow_"
      "heap\0phyAddr\0dlerror\0params\0handle\0reqID\0nSize\0close\0nErr\0name\0open\0";
...
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(remotectl_slim)
    = { 4, &(methodArrays[0]), 0, 0, &(methodStringsArrays[0]), methodStrings, strings };
...
__QAIC_STUB_EXPORT remote_handle _remotectl_handle(void)
{
	...
	if (adsp_pls_add_lookup) {
		if (0
		    == adsp_pls_add_lookup((uint32_t)_remotectl_handle, 0, sizeof(*ph),
		                           _remotectl_pls_ctor, "remotectl", _remotectl_pls_dtor,
		                           (void **)&ph)) {
			return *ph;
		}
		...
		if (0
		    == HAP_pls_add_lookup((uint32_t)_remotectl_handle, 0, sizeof(*ph),
		                          _remotectl_pls_ctor, "remotectl", _remotectl_pls_dtor,
		                          (void **)&ph)) {
			return *ph;
		}
...
uint32_t _remotectl_atomic_CompareAndExchange(uint32_t *volatile puDest, uint32_t uExchange,
                                              uint32_t uCompare);
...
extern "C" {
#endif
static __inline int _stub_method(remote_handle _handle, uint32_t _mid, char *_in0[1],
...
__QAIC_STUB_EXPORT int __QAIC_STUB(remotectl_open)(const char *name, int *handle, char *dlerror,
                                                   int dlerrorLen, int *nErr) __QAIC_STUB_ATTRIBUTE
{
	...
	if (_handle != (remote_handle)-1) {
		return _stub_method(_handle, _mid, (char **)&name, (uint32_t *)handle,
		                    (char **)&dlerror, (uint32_t *)&dlerrorLen, (uint32_t *)nErr);
	} else {
...
__QAIC_STUB_EXPORT int __QAIC_STUB(remotectl_close)(int handle, char *dlerror, int dlerrorLen,
...
__QAIC_STUB_EXPORT int __QAIC_STUB(remotectl_grow_heap)(uint32_t phyAddr,
...
__QAIC_STUB_EXPORT int __QAIC_STUB(remotectl_set_param)(int reqID, const uint32_t *params,
