...
#define FASTRPC_ALLOC_HLOS_FD 0x10000 /* Flag to allocate HLOS FD to be shared with DSP */
...
void apps_mem_deinit(int domain)
{
	...
	GET_HASH_NODE(apps_mem_info, domain, me);
	if (!me) {
		FARF(ALWAYS, "Warning: %s: unable to find hash-node for domain %d", __func__,
		     domain);
		return;
	}
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_mem_request_map64)(int heapid, uint32_t lflags,
                                                           uint32_t rflags, uint64_t vin,
                                                           int64_t len, uint64_t *vapps,
                                                           uint64_t *vadsp) __QAIC_IMPL_ATTRIBUTE
{
	...
	VERIFY(AEE_SUCCESS == (nErr = get_unsigned_pd_attribute(domain, &unsigned_module)));
	...
	*vadsp = 0;
	if (rflags == ADSP_MMAP_HEAP_ADDR || rflags == ADSP_MMAP_REMOTE_HEAP_ADDR) {
		VERIFY(AEE_SUCCESS
		       == (nErr = remote_mmap64_internal(-1, rflags, 0, len, (uint64_t *)vadsp)));
		*vapps = 0;
		minfo->vapps = 0;
		 ...
		 */
		VERIFY(AEE_SUCCESS
		       == (nErr = fastrpc_mmap(domain, fd, buf, 0, len, FASTRPC_MAP_FD_DELAYED)));
		pbuf = (uint64_t)buf;
		*vapps = pbuf;
		minfo->vapps = *vapps;
		...
		*vadsp = (uint64_t)fd;
		 ...
		 * kernel.
		 */
		if (((rflags != ADSP_MMAP_ADD_PAGES) && (rflags != ADSP_MMAP_ADD_PAGES_LLC))
		    || (((rflags == ADSP_MMAP_ADD_PAGES) || (rflags == ADSP_MMAP_ADD_PAGES_LLC))
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_mem_request_map)(int heapid, uint32_t lflags,
                                                         uint32_t rflags, uint32_t vin, int32_t len,
                                                         uint32_t *vapps,
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_mem_request_unmap64)(uint64_t vadsp,
                                                             int64_t len) __QAIC_IMPL_ATTRIBUTE
{
	...
	FASTRPC_ATRACE_BEGIN_L("%s called with vadsp 0x%llx, len 0x%llx", __func__, vadsp, len);
	GET_HASH_NODE(apps_mem_info, domain, me);
	VERIFYC(me, AEE_ERESOURCENOTFOUND);
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_mem_request_unmap)(uint32_t vadsp,
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_mem_share_map)(int fd, int size, uint64_t *vapps,
                                                       uint64_t *vadsp) __QAIC_IMPL_ATTRIBUTE
{
	struct mem_info *minfo = 0;
	int nErr = AEE_SUCCESS;
	...
	*vadsp = 0;
	VERIFYC(MAP_FAILED
	            != (buf = (void *)mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0)),
	        AEE_ERPC);
	VERIFY(AEE_SUCCESS
	       == (nErr = remote_mmap64_internal(fd, 0, (uint64_t)buf, size, (uint64_t *)vadsp)));
	pbuf = (uint64_t)buf;
	*vapps = pbuf;
	minfo->vapps = *vapps;
	minfo->vadsp = *vadsp;
...
bail:
	if (nErr) {
		...
		VERIFY_EPRINTF("Error 0x%x: apps_mem_share_map failed for fd 0x%x of size %d\n",
		               nErr, fd, size);
	}
	return nErr;
}

__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_mem_share_unmap)(uint64_t vadsp,
                                                         int size) __QAIC_IMPL_ATTRIBUTE
{
	...
	if (nErr != AEE_SUCCESS) {
		VERIFY_EPRINTF("Error 0x%x: apps_mem_share_unmap failed size %d (vadsp 0x%llx)\n",
		               nErr, size, vadsp);
	}
	return nErr;
}

__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_mem_dma_handle_map)(int fd, int offset,
                                                            int size) __QAIC_IMPL_ATTRIBUTE
{
	int nErr = AEE_SUCCESS;
	...
	VERIFY(AEE_SUCCESS == (nErr = fastrpc_mmap(domain, fd, 0, offset, size, flags)));
bail:
	if (nErr) {
...
__QAIC_IMPL_EXPORT int __QAIC_IMPL(apps_mem_dma_handle_unmap)(int fd,
                                                              int size) __QAIC_IMPL_ATTRIBUTE
{
	int nErr = AEE_SUCCESS;
	int domain = get_current_domain();
