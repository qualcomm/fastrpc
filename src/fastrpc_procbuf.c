...
int proc_sharedbuf_init(int dev, int domain)
{
	...
	errno = 0;
	VERIFYC(NULL
	            != (proc_sharedbuf = rpcmem_alloc_internal(0, RPCMEM_HEAP_DEFAULT,
	...
	if (ioErr) {
		if (errno == ENOTTY) {
			...
			FARF(ERROR,
			     "Error 0x%x: %s: sharedbuff capability not supported by kernel (errno "
			     "%d, %s).",
...
static int get_non_preload_lib_names(char **lib_names, size_t *buffer_size, int domain)
{
	...
	VERIFYC(NULL != (data_paths = calloc(1, sizeof(char) * dsp_search_path_len)),
	        AEE_ENOMEMORY);
	VERIFYC(AEE_SUCCESS
	            == apps_std_getenv(DSP_LIBRARY_PATH, data_paths, dsp_search_path_len,
	                               &env_list_len),
	        AEE_EGETENV);
			...
			if (entry->d_type == DT_REG) {
				...
				if (strstr(file, FILE_EXT) != NULL) {
					if (concat_len + strlen(file) > MAX_NON_PRELOAD_LIBS_LEN) {
						FARF(ALWAYS,
						     "ERROR: Failed to pack library names in "
						     "custom DSP_LIBRARY_PATH as required buffer "
						     "size exceeds Max limit (%d).",
					...
					concat_len
					    = strlcat(*lib_names, ";", MAX_NON_PRELOAD_LIBS_LEN);
				}
			}
		}
		if (dir != NULL) {
...
static int pack_proc_shared_buf_params(int domain, uint32_t param_id, void *param_addr,
                                       uint32_t param_size)
{
	...
	if (buf_last_addr
	    < (uint32_t *)((char *)buf_write_addr + align_param_size + sizeof(param_id))) {
...
void fastrpc_process_pack_params(int dev, int domain)
{
	...
	if (nErr) {
		FARF(ERROR, "Error 0x%x: %s: Failed to pack process id in shared buffer", nErr,
		     __func__);
	}
	nErr = pack_proc_shared_buf_params(domain, THREAD_PARAM_ID, &hlist[domain].th_params,
	                                   sizeof(hlist[domain].th_params));
	if (nErr) {
		FARF(ERROR, "Error 0x%x: %s: Failed to pack thread parameters in shared buffer",
		     nErr, __func__);
	}
	nErr = pack_proc_shared_buf_params(domain, PROC_ATTR_ID, &hlist[domain].procattrs,
	                                   sizeof(hlist[domain].procattrs));
	if (nErr) {
		FARF(ERROR,
		     "Error 0x%x: %s: Failed to pack process config parameters in shared buffer",
		     nErr, __func__);
	}
	err_codes_to_send = fastrpc_config_get_errcodes();
	if (err_codes_to_send) {
		nErr = pack_proc_shared_buf_params(domain, PANIC_ERR_CODES_ID, err_codes_to_send,
	...
	nErr = pack_proc_shared_buf_params(domain, HLOS_PROC_EFFEC_DOM_ID, &domain, sizeof(domain));
	if (nErr) {
		FARF(ERROR,
		     "Error 0x%x: %s: Failed to pack effective domain id %d in shared buffer", nErr,
		     __func__, domain);
	}
	...
	nErr = pack_proc_shared_buf_params(domain, HLOS_PROC_SESS_ID, &sess_id, sizeof(sess_id));
	if (nErr) {
		FARF(ERROR, "Error 0x%x: %s: Failed to pack session id %d in shared buffer", nErr,
		     __func__, sess_id);
	}
	if (lib_names) {
		free(lib_names);
