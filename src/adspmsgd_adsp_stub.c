...
static const Parameter *const parameterArrays[5]
    = { (&(parameters[0])), (&(parameters[1])), (&(parameters[1])), (&(parameters[1])),
	(&(parameters[2])) };
static const Method methods[2]
    = { { REMOTE_SCALARS_MAKEX(0, 0, 0x1, 0x1, 0x0, 0x0), 0x10, 0x4, 5, 5, (&(parameterArrays[0])),
	  0x4, 0x4 },
...
__QAIC_STUB_EXPORT remote_handle _adspmsgd_adsp_handle(void)
{
	...
	if (adsp_pls_add_lookup) {
		if (0
		    == adsp_pls_add_lookup((uint32_t)_adspmsgd_adsp_handle, 0, sizeof(*ph),
		...
		if (0
		    == HAP_pls_add_lookup((uint32_t)_adspmsgd_adsp_handle, 0, sizeof(*ph),
		                          _adspmsgd_adsp_pls_ctor, "adspmsgd_adsp",
		                          _adspmsgd_adsp_pls_dtor, (void **)&ph)) {
			return *ph;
		}
...
uint32_t _adspmsgd_adsp_atomic_CompareAndExchange(uint32_t *volatile puDest, uint32_t uExchange,
                                                  uint32_t uCompare);
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adspmsgd_adsp_init)(int heapid, uint32_t ion_flags,
                                                       ...
                                                       int *buff_addr) __QAIC_STUB_ATTRIBUTE
{
	...
	if (_handle != (remote_handle)-1) {
		return _stub_method(_handle, _mid, (uint32_t *)&heapid, (uint32_t *)&ion_flags,
		                    (uint32_t *)&filter, (uint32_t *)&buf_size,
		                    (uint32_t *)buff_addr);
	} else {
