...
int adspmsgd_init(remote_handle64 handle, int filter)
{
	...
	msgd_handle->bufferSize = DEFAULT_MEMORY_SIZE - sizeof(*(msgd_handle->currentIndex));
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = pthread_create(&(msgd_handle->msgreader_thread), NULL, adspmsgd_reader,
	                                 (void *)handle)));
...
bail:
	if ((nErr != AEE_SUCCESS) && (nErr != (int)(AEE_EUNSUPPORTED + DSP_AEE_EOFFSET))) {
		VERIFY_EPRINTF("Error 0x%x: %s failed for handle 0x%lx filter %d with errno %s\n",
