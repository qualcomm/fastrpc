...
#define GET_HASH_FROM_JOBID(jobid) ((jobid & FASTRPC_ASYNC_HASH_MASK) >> FASTRPC_ASYNC_JOB_POS)
...
static int get_remote_async_response(int domain, fastrpc_async_jobid *jobid, int *result);

int fastrpc_search_async_job(fastrpc_async_jobid jobid, struct fastrpc_async_job_node **async_node)
{
...
int fastrpc_async_get_status(fastrpc_async_jobid jobid, int timeout_us, int *result)
{
	...
	if (-1 == (lasync_node->pfd.fd = eventfd(0, 0))) {
		nErr = AEE_EFAILED;
		FARF(ERROR,
		     "Error 0x%x: %s failed to create poll event for jobid 0x%" PRIx64 " (%s)\n",
...
int fastrpc_remove_async_job(fastrpc_async_jobid jobid, bool dsp_invoke_done)
{
	...
	pthread_mutex_unlock(&me->mut);
	if (lasync_node->async_desc.type == FASTRPC_ASYNC_POLL && lasync_node->pfd.fd != -1) {
...
bail:
	if (nErr) {
		FARF(ERROR, "Error 0x%x: %s failed for domain %d and jobid 0x%" PRIx64 " (%s)\n",
                           ...
                           fastrpc_async_descriptor_t *desc)
{
	...
	pthread_mutex_unlock(&me->mut);
	FARF(RUNTIME_RPC_HIGH, "adsprpc: %s : Saving job with jobid 0x%" PRIx64 "", __func__,
	...
	for (i = 0; i < FASTRPC_ASYNC_QUEUE_LIST_LEN; i++) {
		...
		while (!QList_IsEmpty(&me->ql[i])) {
			...
			if (lasync_node->async_desc.type == FASTRPC_ASYNC_CALLBACK) {
				...
				FARF(RUNTIME_RPC_HIGH,
				     "adsprpc: %s poll jobid 0x%" PRIx64 " and result 0x%x",
				     __func__, lasync_node->async_desc.jobid, lasync_node->result);
				if (lasync_node->pfd.fd != -1) {
					eventfd_write(lasync_node->pfd.fd,
...
static void *async_fastrpc_thread(void *arg)
{
	...
	do {
		...
		FARF(RUNTIME_RPC_HIGH,
		     "adsprpc: %s received async response for jobid 0x%" PRIx64 " and result 0x%x",
		...
		QLIST_NEXTSAFE_FOR_ALL(&me->ql[hash], pn, pnn)
		{
			...
			if (lasync_node->async_desc.jobid == jobid) {
				...
				case FASTRPC_ASYNC_NO_SYNC:
					FARF(RUNTIME_RPC_HIGH,
					     "adsprpc: %s nosync jobid 0x%" PRIx64
					     " and result 0x%x",
				...
				case FASTRPC_ASYNC_CALLBACK:
					pthread_mutex_unlock(&me->mut);
					FARF(RUNTIME_RPC_HIGH,
					     "adsprpc: %s callback jobid 0x%" PRIx64
					     " and result 0x%x",
					     __func__, lasync_node->async_desc.jobid, result);
					lasync_node->async_desc.cb.fn(
					    lasync_node->async_desc.jobid,
					    lasync_node->async_desc.cb.context, result);
					break;
				default:
					pthread_mutex_unlock(&me->mut);
					FARF(RUNTIME_RPC_HIGH,
					     "adsprpc: %s Invalid job type for jobid 0x%" PRIx64 "",
...
int fastrpc_async_domain_init(int domain)
{
	...
	VERIFY(AEE_SUCCESS == (nErr = fastrpc_get_cap(domain, ASYNC_FASTRPC_SUPPORT, &capability)));
	...
	VERIFY(AEE_SUCCESS
	       == (nErr = pthread_create(&me->thread, 0, async_fastrpc_thread, (void *)me)));
