...
static __inline uint32_t std_dtoa_clz32(uint32_t ulVal)
//
...
static __inline uint32_t std_dtoa_clz64(uint64_t ulVal)
//
...
double fp_pow_10(int nPow)
{
	...
	double aTableNeg[] = { 0, 1e-1, 1e-2, 1e-4, 1e-8, 1e-16, 1e-32, 1e-64, 1e-128, 1e-256 };
...
int std_dtoa_hex(double dNumber, int nPrecision, char cFormat,
                 char acIntegerPart[STD_DTOA_FORMAT_INTEGER_SIZE],
                 char acFractionPart[STD_DTOA_FORMAT_FRACTION_SIZE], int *pnExponent)
{
	int nError = AEE_SUCCESS;
	uint64_t ullMantissa = 0;
	uint64_t ullSign = 0;
	...
	for (nI = 0, nF = 0, bFirstDigit = true; ullMantissa != 0; ullMantissa <<= 4) {
