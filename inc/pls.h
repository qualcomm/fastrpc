...
unsigned int atomic_CompareOrAdd(atomic_uint *puDest, unsigned int uCompare, unsigned int nAdd)
{
	...
	do {
		...
		result = current;
		if (current != uCompare) {
			atomic_compare_exchange_strong(puDest, &previous, current + nAdd);
			if (previous == current) {
				result = current + nAdd;
			}
		}
...
static __inline int pls_thread_init(struct pls_table *me, uintptr_t tid)
{
	...
	while (1) {
		...
		if (atomic_compare_exchange_strong(&me->uRefs, &current, current + 1))
			return 0;
	}
}
 ...
 */
static __inline int pls_add(struct pls_table *me, uintptr_t type, uintptr_t key, int size,
                            int (*ctor)(void *ctx, void *data), void *ctx, void (*dtor)(void *data),
                            void **ppo)
{
	int nErr = 0;
	struct PLS *pls = 0;
	...
	do {
		...
		prev = pls->next;
		atomic_compare_exchange_strong(&me->lst, &prev, (uintptr_t)pls);
	} while (prev != pls->next);
	if (ppo) {
		*ppo = (void *)pls->data;
	}
 ...
 */
static __inline int pls_add_lookup_singleton(struct pls_table *me, uintptr_t type, uintptr_t key,
...
{
	struct PLS *lst;
	for (lst = (struct PLS *)atomic_load(&me->lst); me->uRefs != 0 && lst != 0;
	     lst = (struct PLS *)lst->next) {
