...
/**
 *  @file rpcmem.h
 *  @brief APIs used to manage memory allocated by the application processor and shared with the
 * DSP.
 */
...
/**
 * The FastRPC library tries to map buffers allocated with this flag to the remote process of all
 * current and new FastRPC sessions. In case of failure to map, the FastRPC library ignores the
 * error and continues to open the session without pre-mapping the buffer. In case of success,
 * buffers allocated with this flag will be pre-mapped to reduce the latency of upcoming FastRPC
 * calls. This flag is recommended only for buffers that are used with latency-critical FastRPC
 * methods. Pre-mapped buffers will be unmapped during either buffer free or session close.
 */
