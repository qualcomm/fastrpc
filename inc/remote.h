...
#define REMOTE_SCALARS_LENGTH(sc)                                                                  \
	(REMOTE_SCALARS_INBUFS(sc) + REMOTE_SCALARS_OUTBUFS(sc) + REMOTE_SCALARS_INHANDLES(sc)     \
...
enum remote_rpc_latency_flags {
	...
	/** DSP driver predicts completion time of a method and send CPU wake up signal to reduce
	 * wake up latency. Recommended for moderate latency sensitive use cases. It is more power
...
typedef struct remote_dsp_capability {
	uint32_t domain; /** @param[in]: DSP domain ADSP_DOMAIN_ID, SDSP_DOMAIN_ID, CDSP_DOMAIN_ID,
	                    or GDSP_DOMAIN_ID */
	uint32_t attribute_ID; /** @param[in]: One of the DSP/kernel attributes from enum
	                          remote_dsp_attributes */
	uint32_t capability;   /** @param[out]: Result of the DSP/kernel capability query based on
	                          attribute_ID */
} fastrpc_capability;
...
struct remote_control_custom_path {
	...
	char *value; /** value which will be used for file operations when the corresponding key is
	                specified in the file URI */
};
...
enum handle_control_req_id {
	...
	DSPRPC_GET_DOMAIN,       /** Request ID to get the default domain or domain associated to an
	                            exisiting handle */
...
struct remote_rpc_relative_thread_priority {
	...
	int relative_thread_priority; /** the value by which the default thread priority needs to
	                               * increase/decrease DSP thread priorities run from 1 to 255
	                               * with 1 being the highest thread priority. So a negative
	                               * relative thread priority value will 'increase' the thread
	                               * priority, a positive value will 'decrease' the thread
	                               * priority.
	                               */
};
...
typedef enum remote_rpc_status_flags {
	...
	FASTRPC_USER_PD_FORCE_KILL, /** DSP user process forcefully killed. Happens when DSP
...
typedef struct remote_rpc_notif_register {
	...
	int domain;    /** @param[in]: DSP domain ADSP_DOMAIN_ID, SDSP_DOMAIN_ID, CDSP_DOMAIN_ID, or
	                  GDSP_DOMAIN_ID */
...
struct remote_rpc_pd_initmem_size {
	int domain; /** Remote subsystem domain ID, pass -1 to set params for all domains **/
	uint32_t pd_initmem_size; /** Initial memory allocated for remote userpd, minimum value :
...
typedef struct remote_rpc_reserve_new_session {
	char *
	    domain_name; /** @param[in]: Domain name of DSP, on which session need to be reserved */
	...
	uint32_t effective_domain_id; /** @param[out]: Effective Domain ID is the identifier of the
	                               * session. Effective Domain ID is the unique identifier
	                               * representing the session(PD) on DSP. Effective Domain ID
	                               * needs to be used in place of Domain ID when application has
	                               * multiple sessions.
	                               */
	uint32_t session_id;          /** @param[out]: Session ID of the reserved session.
	                               ...
	                               * session_id 0 is the default session. Clients can reserve session
	                               * starting from 1.	  Currently only 2 sessions are supported
	                               * session_id	  0 and session_id 1.
	                               */
...
typedef struct remote_rpc_effective_domain_id {
	...
	uint32_t domain_name_len; /** @param[in]: Domain name length, without NULL character */
	uint32_t session_id; /** @param[in]: Session ID of the reserved session. 0 can be used for
	                        Default session */
...
typedef struct remote_rpc_get_uri {
	...
	uint32_t session_id; /** @param[in]: Session ID of the reserved session. 0 can be used for
	                        Default session */
	char *
	    module_uri; /** @param[in]: URI of the module, found in the auto-generated header file*/
	...
	char *uri;               /** @param[out]: URI containing module, domain and session.
	                          ...
	                          * If size of uri is beyond session_uri_len, remote_session_control fails with
	                          * AEE_EBADSIZE
	                          */
...
enum session_control_req_id {
	...
	DSPRPC_CONTROL_UNSIGNED_MODULE, /** Handle the unsigned module offload request, to be called
	                                   before remote_handle_open() */
...
enum remote_mem_map_flags {
	 ...
	 * Same remoteVirtAddr will be assigned on remote process when fastrpc call made with local
	 * virtual address.
	 * @Map lifetime
	 * Life time of this mapping is until user unmap using remote_mem_unmap or session close.
	 * No reference counts are used. Behavior of mapping multiple times without unmap is
	 * undefined.
	 * @Cache maintenance
	 * Driver clean caches when virtual address passed through RPC calls defined in IDL as a
	 * pointer. User is responsible for cleaning cache when remoteVirtAddr shared to DSP and
	 * accessed out of fastrpc method invocations on DSP.
	 * @recommended usage
	 * Map buffers which are reused for long time or until session close. This helps to reduce
	 * fastrpc latency. Memory shared with remote process and accessed only by DSP.
	 */
...
#define FASTRPC_ATTR_FORCE_NOFLUSH                                                                 \
	32 /** Attribute to map buffer such that flush by driver is skipped for that particular    \
	      buffer client has to perform cache maintenance*/
#define FASTRPC_ATTR_FORCE_NOINVALIDATE                                                            \
	64 /** Attribute to map buffer such that invalidate by driver is skipped for that          \
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_handle_open)(
    __QAIC_IN_CHAR const char *name, __QAIC_OUT remote_handle *ph) __QAIC_REMOTE_ATTRIBUTE;
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_handle64_open)(
    __QAIC_IN_CHAR const char *name, __QAIC_OUT remote_handle64 *ph) __QAIC_REMOTE_ATTRIBUTE;
 ...
 *
 * @param req [in] Request ID specifying the control parameter to set, defined in enum
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_handle64_control)(
    __QAIC_IN remote_handle64 h, __QAIC_IN uint32_t req, __QAIC_IN_LEN(datalen) void *data,
    __QAIC_IN uint32_t datalen) __QAIC_REMOTE_ATTRIBUTE;
 ...
 */
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_handle_invoke_async)(
    __QAIC_IN remote_handle h, __QAIC_IN fastrpc_async_descriptor_t *desc,
    __QAIC_IN uint32_t dwScalars, __QAIC_IN remote_arg *pra) __QAIC_REMOTE_ATTRIBUTE;
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_handle64_invoke_async)(
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(fastrpc_async_get_status)(
    __QAIC_IN fastrpc_async_jobid jobid, __QAIC_IN int timeout_us, __QAIC_OUT int *result);
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_mmap)(
    __QAIC_IN int fd, __QAIC_IN uint32_t flags, __QAIC_IN uint32_t vaddrin, __QAIC_IN int size,
    __QAIC_OUT uint32_t *vaddrout) __QAIC_REMOTE_ATTRIBUTE;
 ...
 */
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_munmap)(
    __QAIC_IN uint32_t vaddrout, __QAIC_IN int size) __QAIC_REMOTE_ATTRIBUTE;
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_mem_map)(
    __QAIC_IN int domain, __QAIC_IN int fd, __QAIC_IN int flags, __QAIC_IN uint64_t virtAddr,
    __QAIC_IN size_t size, __QAIC_OUT uint64_t *remoteVirtAddr) __QAIC_REMOTE_ATTRIBUTE;
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_mmap64)(
    __QAIC_IN int fd, __QAIC_IN uint32_t flags, __QAIC_IN __QAIC_INT64PTR vaddrin,
    __QAIC_IN int64_t size, __QAIC_OUT __QAIC_INT64PTR *vaddrout) __QAIC_REMOTE_ATTRIBUTE;
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_munmap64)(
    __QAIC_IN __QAIC_INT64PTR vaddrout, __QAIC_IN int64_t size) __QAIC_REMOTE_ATTRIBUTE;
 ...
 * and CACHE WRITEBACK configuration. Driver will clean cache when buffer is passed in a FastRPC
 * call.
 *
 * @param domain [in] DSP domain ID of a fastrpc session. Use -1 for default domain based on linked
 * library. Valid domains are ADSP_DOMAIN_ID, MDSP_DOMAIN_ID, SDSP_DOMAIN_ID, CDSP_DOMAIN_ID, or
 * GDSP_DOMAIN_ID.
 * @param fd [in] DMA memory file descriptor obtained from dma_alloc_fd() or similar DMA allocation
 * APIs.
 * @param addr [in] Virtual address of the buffer on CPU side. Must be the same address returned by
 * mmap() when mapping the DMA fd.
 ...
 * @param flags [in] Controls mapping functionality on DSP. See enum fastrpc_map_flags for valid
 * flags: FASTRPC_MAP_CACHE_WRITEBACK - Map with writeback cache configuration (default)
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(fastrpc_mmap)(
    __QAIC_IN int domain, __QAIC_IN int fd, __QAIC_IN void *addr, __QAIC_IN int offset,
    __QAIC_IN size_t length, __QAIC_IN enum fastrpc_map_flags flags) __QAIC_REMOTE_ATTRIBUTE;
 ...
 * @param addr [in] Virtual address of the buffer on CPU side. Must match the address used in
 * fastrpc_mmap().
 * @param length [in] Size of buffer in bytes to unmap. Must match the length used in
 * fastrpc_mmap().
 *
 * @return 0 on success, error code on failure:
 *         AEE_EBADPARM - Invalid parameters (null pointers, unaligned sizes, etc)
 ...
 * @param fd [in] File descriptor for the buffer. Use -1 to deregister a previously registered
 * buffer.
...
__QAIC_REMOTE_EXPORT __QAIC_RETURN void __QAIC_REMOTE(remote_register_buf)(
    __QAIC_IN_LEN(size) void *buf, __QAIC_IN int size, __QAIC_IN int fd) __QAIC_REMOTE_ATTRIBUTE;
 ...
 *         ENOSYS - API not supported on this platform
 */
__QAIC_REMOTE_EXPORT __QAIC_RETURN void __QAIC_REMOTE(remote_register_buf_attr2)(
    __QAIC_IN_LEN(size) void *buf, __QAIC_IN size_t size, __QAIC_IN int fd,
    __QAIC_IN int attr) __QAIC_REMOTE_ATTRIBUTE;
 ...
 *
 * @param fd [in] File descriptor for the ION buffer. Use -1 to deregister a previously registered
 * buffer.
 ...
 *         ENOSYS - API not supported on this platform
 */
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_register_dma_handle)(
    __QAIC_IN int fd, __QAIC_IN uint32_t len) __QAIC_REMOTE_ATTRIBUTE;
__QAIC_REMOTE_EXPORT __QAIC_RETURN int __QAIC_REMOTE(remote_register_dma_handle_attr)(
    __QAIC_IN int fd, __QAIC_IN uint32_t len, __QAIC_IN uint32_t attr) __QAIC_REMOTE_ATTRIBUTE;
