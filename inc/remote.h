...
enum remote_rpc_latency_flags {
	...
	/** DSP driver predicts completion time of a method and send CPU wake
	 * up signal to reduce wake up latency. Recommended for moderate
	 * latency sensitive use cases. It is more power efficient compared to
	 * pm_qos control.
	 */
	RPC_ADAPTIVE_QOS,

	/**
	 * After sending invocation to DSP, CPU will enter polling mode instead
	 * of waiting for a glink response. This will boost fastrpc performance
	 * by reducing the CPU wakeup and scheduling times. Enabled only for
	 * sync RPC calls. Using this option also enables PM QoS with a latency
	 * of 100 us.
	 */
	RPC_POLL_QOS,
};
...
struct remote_rpc_control_latency {
	 ...
	 * Use remote_rpc_latency_flags */
	uint32_t enable;

	/**
	 * Latency in microseconds.
	 *
	 * When used with RPC_PM_QOS or RPC_ADAPTIVE_QOS, user should pass
	 * maximum RPC latency that can be tolerated. It is not guaranteed that
	 * fastrpc will meet this requirement. 0 us latency is ignored.
	 * Recommended value is 100.
	 *
	 * When used with RPC_POLL_QOS, user needs to pass the expected
	 * execution time of method on DSP. CPU will poll for a DSP response
	 * for that specified duration after which it will timeout and fall
	 * back to waiting for a glink response. Max value that can be passed
	 * is 10000 (10 ms)
	 */
	uint32_t latency;
};
...
typedef struct remote_dsp_capability {
	uint32_t
	    domain; /** @param[in]: DSP domain ADSP_DOMAIN_ID, SDSP_DOMAIN_ID,
	               CDSP_DOMAIN_ID, or GDSP_DOMAIN_ID */
	uint32_t attribute_ID; /** @param[in]: One of the DSP/kernel attributes
...
enum remote_dsp_attributes {
	...
	STATUS_NOTIFICATION_SUPPORT, /** DSP User PD status notification
	                                Support */
	MCID_MULTICAST,              /** Multicast widget programming */
	/** Update FASTRPC_MAX_DSP_ATTRIBUTES when adding new value to this
	   enum */
};
...
enum handle_control_req_id {
	...
	DSPRPC_GET_DSP_INFO, /** Request ID to get dsp capabilites from kernel
	                        and Hexagon */
	DSPRPC_CONTROL_WAKELOCK, /** Request ID to enable wakelock for the
	                            given domain */
...
struct remote_rpc_thread_params {
	int domain; /** Remote subsystem domain ID, pass -1 to set params for
	               all domains */
...
struct remote_rpc_control_unsigned_module {
	int domain; /** Remote subsystem domain ID, -1 to set params for all
	               domains */
...
struct remote_rpc_relative_thread_priority {
	int domain; /** Remote subsystem domain ID, pass -1 to update priority
	               for all domains */
	int relative_thread_priority; /** the value by which the default thread
	                               * priority needs to increase/decrease
	                               * DSP thread priorities run from 1 to
	                               * 255 with 1 being the highest thread
	                               * priority. So a negative relative
	                               * thread priority value will 'increase'
	                               * the thread priority, a positive value
	                               * will 'decrease' the thread priority.
	                               */
};
 ...
 * requestID and the appropriate remote domain ID. Once remote process is
 * successfully killed, before attempting to create new session, user is
 * expected to close all open handles for shared objects in case of domains.
...
struct remote_rpc_session_close {
	int domain; /** Remote subsystem domain ID, -1 to close all handles for
	               all domains */
};
...
typedef enum remote_rpc_status_flags {
	...
	FASTRPC_USER_PD_FORCE_KILL, /** DSP user process forcefully killed.
	                               Happens when DSP resources needs to be
	                               freed. */
	FASTRPC_USER_PD_EXCEPTION, /** Exception in the user process of DSP. */
	FASTRPC_DSP_SSR, /** Subsystem restart of the DSP, where user process
	                    is running. */
...
typedef struct remote_rpc_notif_register {
	...
	int domain; /** @param[in]: DSP domain ADSP_DOMAIN_ID, SDSP_DOMAIN_ID,
	               CDSP_DOMAIN_ID, or GDSP_DOMAIN_ID */
	fastrpc_notif_fn_t
...
struct remote_rpc_pd_initmem_size {
	...
	/** Unsupported for unsigned user PD, for unsigned user PD init mem
	 * size is fixed at 5MB **/
};
...
typedef struct remote_rpc_reserve_new_session {
	char *domain_name; /** @param[in]: Domain name of DSP, on which session
	                      need to be reserved */
	uint32_t domain_name_len;  /** @param[in]: Domain name length, without
	                              NULL character */
	char *session_name;        /** @param[in]: Session name of the reserved
	                              sesssion */
	uint32_t session_name_len; /** @param[in]: Session name length, without
	                              NULL character */
	uint32_t
	    effective_domain_id; /** @param[out]: Effective Domain ID is the
	                          * identifier of the session. Effective Domain
	                          * ID is the unique identifier representing
	                          * the session(PD) on DSP. Effective Domain ID
	                          * needs to be used in place of Domain ID when
	                          * application has multiple sessions.
	                          */
	uint32_t
	    session_id; /** @param[out]: Session ID of the reserved session.
	                 * An application can have multiple sessions(PDs)
	                 * created on DSP. session_id 0 is the default session.
	                 * Clients can reserve session starting from 1.
	                 * Currently only 2 sessions are supported session_id 0
	                 * and session_id 1.
	                 */
...
typedef struct remote_rpc_effective_domain_id {
	...
	uint32_t session_id;      /** @param[in]: Session ID of the reserved
	                             session. 0 can be used for Default session */
	uint32_t effective_domain_id; /** @param[out]: Effective Domain ID of
	                                 session */
...
typedef struct remote_rpc_get_uri {
	...
	uint32_t module_uri_len; /** @param[in]: Module URI length, without
	                            NULL character */
	char *uri; /** @param[out]: URI containing module, domain and session.
	            * Memory for uri need to be pre-allocated with
	            * session_uri_len size. Typically session_uri_len is 30
	            * characters more than Module URI length. If size of uri is
	            * beyond session_uri_len, remote_session_control fails with
	            * AEE_EBADSIZE
	            */
...
enum session_control_req_id {
	...
	FASTRPC_THREAD_PARAMS, /** Set thread parameters like priority and
	                          stack size */
	DSPRPC_CONTROL_UNSIGNED_MODULE, /** Handle the unsigned module offload
	                                   request, to be called before
	                                   remote_handle_open() */
	FASTRPC_RESERVED_2,             /** Reserved */
	FASTRPC_RELATIVE_THREAD_PRIORITY, /** To increase/decrease default
	                                     thread priority */
	...
	FASTRPC_REMOTE_PROCESS_EXCEPTION, /** Trigger Exception in the remote
	                                     process */
	FASTRPC_REMOTE_PROCESS_TYPE, /** Query type of process defined by enum
	                                fastrpc_process_type */
	FASTRPC_REGISTER_STATUS_NOTIFICATIONS, /** Enable DSP User process
	                                          status notifications */
	FASTRPC_PD_INITMEM_SIZE, /** Set signed userpd initial memory size  **/
	FASTRPC_RESERVE_NEW_SESSION,     /** Reserve new FastRPC session **/
	FASTRPC_GET_EFFECTIVE_DOMAIN_ID, /** Get effective domain ID of a
	                                    FastRPC session */
	FASTRPC_GET_URI, /** Creates the URI needed to load a module in the DSP
	                    User PD */
	FASTRPC_MAX_THREAD_PARAM, /** Set max thread value for unsigned PD */
	FASTRPC_CONTEXT_CREATE,  /** Create or attaches to remote session(s) on
	                            one or more domains */
...
enum remote_mem_map_flags {
	/**
	 * Create static memory map on remote process with default cache
	 * configuration (writeback). Same remoteVirtAddr will be assigned on
	 * remote process when fastrpc call made with local virtual address.
	 * @Map lifetime
	 * Life time of this mapping is until user unmap using remote_mem_unmap
	 * or session close. No reference counts are used. Behavior of mapping
	 * multiple times without unmap is undefined.
	 * @Cache maintenance
	 * Driver clean caches when virtual address passed through RPC calls
	 * defined in IDL as a pointer. User is responsible for cleaning cache
	 * when remoteVirtAddr shared to DSP and accessed out of fastrpc method
	 * invocations on DSP.
	 * @recommended usage
	 * Map buffers which are reused for long time or until session close.
	 * This helps to reduce fastrpc latency. Memory shared with remote
...
enum fastrpc_map_flags {
	 ...
	 * maintenance of CPU and DSP caches for the buffer. Get virtual
	 * address of buffer on DSP using HAP_mmap_get() and HAP_mmap_put()
	 * functions.
	 */
	FASTRPC_MAP_FD,
 ...
 * from IDL files. Applications should use the generated stub functions
 * instead.
 *
 ...
 *
 * @param ph [out] Pointer to store the opened remote handle. This handle
 * should be used in subsequent remote_handle_invoke() calls and must be closed
 * using remote_handle_close() when no longer needed.
 *
 ...
 *
 * @note For unsigned modules, DSPRPC_CONTROL_UNSIGNED_MODULE must be called
 * via remote_session_control() before opening the handle.
 *
 ...
 *                       - REMOTE_SCALARS_INHANDLES(sc): Number of input
 * handles
 ...
 * handle operations like latency requirements, wake lock control, domain info
 * etc.
 *
 ...
 * DEPRECATED: Use fastrpc_munmap() instead.
 *
 * Unmaps memory previously mapped using remote_mem_map() from a specific
 * remote domain process.
 *
 ...
 *
 * @note This API is deprecated and will be removed in a future release.
 *       Use fastrpc_munmap() instead which provides better integration with
 * the FastRPC framework.
 */
 ...
 * DEPRECATED: Use fastrpc_munmap() instead.
 *
 * Unmaps memory previously mapped using remote_mmap64() from the remote
 * domain. This is the 64-bit version of remote_munmap().
 *
 ...
 * This API maps the buffer with RW- permission and CACHE WRITEBACK
 * configuration. Driver will clean cache when buffer is passed in a FastRPC
 * call.
 *
 ...
 * MDSP_DOMAIN_ID, SDSP_DOMAIN_ID, CDSP_DOMAIN_ID, or GDSP_DOMAIN_ID.
...
/**
 * remote_register_fd
 * Register a file descriptor with FastRPC to enable zero-copy sharing of
 * memory with DSP. This API is useful when users have a file descriptor but no
 ...
 * - Invalid parameters (fd < 0, size = 0 or size >= 2GB) ENOMEM - Out of
 * memory in driver EBADF - Invalid file descriptor EBUSY - Buffer already
 * registered ENOSYS - API not supported on this platform
 *
 ...
 * Register a file descriptor with FastRPC to enable zero-copy sharing of
 * memory with DSP. This API is useful when users have a file descriptor but no
 * virtual address mapping. Unlike remote_register_fd(), this function supports
 ...
 * @return On success, returns a virtual address that can be used in FastRPC
 * calls. On failure, returns (void*)-1. Check errno for error details: EINVAL
 * - Invalid parameters (fd < 0, size = 0) ENOMEM - Out of memory in driver
 *         EBADF - Invalid file descriptor
 *         EBUSY - Buffer already registered
 *         ENOSYS - API not supported on this platform
 *
