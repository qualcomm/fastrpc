 ...
 *         - AEE_EQURTTHREADCREATE: Unable to create callback thread; the
 * system may have reached its thread limit.
 ...
 * @param [out] num_buffers Number of buffer references in packet
 * @param [out] message_length Packet message length in bytes
 * @param [out] timeout_us Timeout in microseconds; use DSPQUEUE_TIMEOUT_NONE
 * to block indefinitely until a packet is available or zero for non-blocking
 * behavior.
 *
 * @return 0 on success, error code on failure.
 *         - AEE_EEXPIRED: Request timed out
 ...
 *
 * @param [in] queue Queue handle from dspqueue_create() or dspqueue_import()
 * @param [in] wakeup_delay Wakeup time in microseconds; this indicates how
 * soon the real message packet should be available. Zero if not known. The
 * recipient can use this information to determine how to wait for the packet.
 * @param [in] packet_flags Flags for the upcoming packet if known.
 *                    The recipient can use this information to determine how
 * to wait for the packet. See enum #dspqueue_packet_flags
 *
 * @return 0 on success, error code on failure.
 *         - AEE_EWOULDBLOCK: The queue is full
 */
 ...
 * @param [out] value Statistic value. Reading a statistic will reset it to
 * zero
 *
 * @return 0 on success, error code on failure.
