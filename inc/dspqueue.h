...
enum dspqueue_stat {
	...
	DSPQUEUE_STAT_EARLY_WAKEUP_WAIT_TIME, /**< Total accumulated early wakeup wait time in
	                                         microseconds */
	DSPQUEUE_STAT_EARLY_WAKEUP_MISSES,    /**< Number accumulated of packets missed in the early
	                                         wakeup loop */
	DSPQUEUE_STAT_SIGNALING_PERF /**< Signaling performance; 0 or undefined indicates the first
	                                implementation, DSPQUEUE_SIGNALING_PERF_REDUCED_SIGNALING or
	                                higher a version if reduced signaling for polling clients.
	                              */
};
 ...
 *
 * @return 0 on success, error code on failure.
 *         - AEE_ERPC: Internal RPC error, e.g. The queue is open on the DSP when attempting to
 * close it on the host CPU
 */
 ...
 */
AEEResult dspqueue_import(uint64_t queue_id, dspqueue_callback_t packet_callback,
                          dspqueue_callback_t error_callback, void *callback_context,
                          dspqueue_t *queue);
 ...
 *         - AEE_ENOSUCHMAP: Attempt to refer to an unmapped buffer. Buffers must be mapped to the
 * DSP with fastrpc_mmap() before they can be used in queue packets.
 ...
 * @param [in] max_buffers The maximum number of buffer references that can fit in the "buffers"
 * parameter
 ...
 *
 * @return 0 on success, error code on failure.
 *         - AEE_ENOSUCHMAP: The packet refers to an unmapped buffer. Buffers must be mapped to the
 * DSP with fastrpc_mmap() before they can be used in queue packets.
