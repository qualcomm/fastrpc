...
static __inline void QList_Zero(QList *me) { me->n.pNext = me->n.pPrev = &me->n; }

static __inline void QList_Ctor(QList *me) { QList_Zero(me); }

static __inline int QList_IsEmpty(QList *me) { return me->n.pNext == &me->n; }

static __inline int QList_IsNull(QList *me) { return ((0 == me->n.pNext) && (0 == me->n.pPrev)); }

static __inline void QList_AppendNode(QList *me, QNode *pn) { QNode_InsPrev(&me->n, pn); }

static __inline void QList_PrependNode(QList *me, QNode *pn) { QNode_InsNext(&me->n, pn); }
...
#define QLIST_FOR_ALL(pList, pNode)                                                                \
	for ((pNode) = (pList)->n.pNext; (pNode) != &(pList)->n; (pNode) = (pNode)->pNext)

#define QLIST_FOR_REST(pList, pNode) for (; (pNode) != &(pList)->n; (pNode) = (pNode)->pNext)

#define QLIST_REV_FOR_ALL(pList, pNode)                                                            \
	for ((pNode) = (pList)->n.pPrev; (pNode) != &(pList)->n; (pNode) = (pNode)->pPrev)

#define QLIST_REV_FOR_REST(pList, pNode) for (; (pNode) != &(pList)->n; (pNode) = (pNode)->pPrev)

/* Allows dequeing QNodes during iteration */
#define QLIST_NEXTSAFE_FOR_ALL(pList, pNode, pNodeNext)                                            \
	for ((pNode) = (pList)->n.pNext, (pNodeNext) = (pNode)->pNext; (pNode) != &(pList)->n;     \
