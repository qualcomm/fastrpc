...
//! current postiion
static __inline char *sbuf_cur(struct sbuf *buf) { return (char *)sbuf_head(buf); }
 ...
 * consume any char
 */
static __inline int sbuf_any(struct sbuf *buf) { return sbuf_notchars(buf, ""); }
 ...
 */
static __inline int sbuf_many(struct sbuf *buf, int (*consume)(struct sbuf *buf))
{
	if (!sbuf_valid(buf)) {
		return 0;
	}
 ...
 */
static __inline int sbuf_many1(struct sbuf *buf, int (*consume)(struct sbuf *buf))
{
	if (!consume(buf)) {
		return 0;
	}
 ...
 */
static __inline int sbuf_until(struct sbuf *buf, int (*consume)(struct sbuf *buf),
