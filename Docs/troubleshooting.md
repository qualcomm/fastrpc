...
## "Permission denied" opening `/dev/fastrpc-*` (`EACCES` errno 13 / `0x72`)

**Symptom:** `remote_handle64_open()` (or any first FastRPC call) fails with
`EACCES` (errno 13 / `0x72`), or the device node itself is not readable by
your user.
...
## "skel not found" / DSP module fails to load (`AEE_EUNABLETOLOAD` `0x80000406`)

**Symptom:** `remote_handle64_open()` fails with `AEE_EUNABLETOLOAD`
(`0x80000406`), and DSP/kernel logs (or `fastrpc-healthcheck`) show the
shell or skel library could not be located. The underlying OS error is
typically `ENOENT` (errno 2 — "no such file or directory").
