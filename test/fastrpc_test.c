...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "d:U:t:a:")) != -1) {
		...
		case 't':
			...
			if (strcmp(target, "linux") != 0 && strcmp(target, "android") != 0) {
				printf("\nERROR: Invalid target platform (-t). Must be linux or "
				       "android.\n");
				print_usage();
				return -1;
			}
		...
		case 'a':
			...
			if (strcmp(arch_version, "v68") != 0 && strcmp(arch_version, "v75") != 0) {
				printf("\nERROR: Invalid architecture version (-a). Must be v68 or "
				       "v75.\n");
				print_usage();
				return -1;
			}
	...
	if (current_ld_lib_path) {
		snprintf(ld_lib_path, sizeof(ld_lib_path), "%s;%s", current_ld_lib_path,
		         testlibdir);
	} else {
	...
	if (current_dsp_lib_path) {
		snprintf(dsp_lib_path, sizeof(dsp_lib_path), "%s;%s/%s", current_dsp_lib_path,
	...
	while ((entry = readdir(dir)) != NULL) {
		if (entry->d_type == DT_REG && strstr(entry->d_name, ".so")) {
			snprintf(full_lib_path, sizeof(full_lib_path), "%s/%s", testlibdir,
			         entry->d_name);
