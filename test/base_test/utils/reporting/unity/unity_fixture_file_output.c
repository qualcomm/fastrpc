                                   ...
                                   unsigned int line)
{
     ...
     * before the body runs, even if the body crashes or hangs. This
     * live-visibility guarantee is traded away below when --silent is
     * active: the header, body output, and footer are buffered together
     * and only flushed to the real console if the test does not pass. */
    if (g_test_config.silent_mode) {
        fflush(stdout);
    ...
    /* Restore the real stdout and, only if the test did not pass, copy the
     * buffered header/body/footer block to it verbatim. On a pass the
     * temp file is simply discarded, producing no output at all. */
    if (capture_file) {
        fflush(stdout);
        ...
        if (!test_passed) {
            ...
            while ((n = fread(buf, 1, sizeof(buf), capture_file)) > 0)
